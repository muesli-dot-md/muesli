//! The y-websocket wire protocol (docs/design/sync-protocol.md): binary frames of
...
pub fn decode_awareness(payload: &[u8]) -> Result<Vec<AwarenessEntry>, ProtoError> {
    ...
    for _ in 0..count {
        ...
        entries.push(AwarenessEntry { client_id, clock, json });
    }
    Ok(entries)
}
...
mod tests {
    ...
    fn var_u64_round_trip() {
        for n in [0u64, 1, 127, 128, 300, 16383, 16384, u32::MAX as u64, u64::MAX / 2] {
            let mut buf = Vec::new();
    ...
    fn awareness_round_trip() {
        let entries = vec![
            AwarenessEntry { client_id: 42, clock: 7, json: r#"{"user":{"name":"jb"}}"#.into() },
            AwarenessEntry { client_id: 1337, clock: 1, json: "null".into() },
        ];
