...
//! diff (see internal/design/ingest-and-materialization.md).
...
/// replacement (internal/design/ingest-and-materialization.md).
...
impl MuesliDoc {
    ...
    /// (internal/design/ingest-and-materialization.md). Caller is responsible for the hash-based
    ...
    pub fn create_anchor(&self, start: usize, end: usize) -> Result<Anchor, CoreError> {
        ...
        if !current.is_char_boundary(start) || !current.is_char_boundary(end) {
            return Err(CoreError::Anchor(format!(
                "range {start}..{end} splits a character"
            )));
        }
...
mod tests {
    ...
    fn ingest_small_edit_lands_exactly() {
        ...
        match out {
            IngestOutcome::Applied { inserted, deleted } => {
                ...
                assert!(
                    inserted <= 8,
                    "inserted {inserted} bytes, expected a small refined edit"
                );
                assert!(
                    deleted <= 4,
                    "deleted {deleted} bytes, expected a small refined edit"
                );
            }
    ...
    fn delete_only_updates_report_changed() {
        ...
        assert!(
            b.apply_update_changed(&delta).unwrap(),
            "delete-only update must count as a change"
        );
    ...
    fn apply_edits_with_multibyte_text() {
        ...
        let text = doc.materialize();
        let (ws, we) = (
            text.find("wörld").unwrap(),
            text.find("wörld").unwrap() + "wörld".len(),
        );
        doc.apply_edits(&[
            (0, "Hällo".len(), "Hello 🌍".into()),
            (ws, we, "world".into()),
        ])
        .unwrap();
    ...
    fn apply_edits_is_one_atomic_update() {
        ...
        let update = doc
            .apply_edits(&[(0, 5, "ALPHA".into()), (11, 16, "GAMMA".into())])
            .unwrap();
    ...
    fn apply_edits_rejects_bad_ops() {
        ...
        // overlapping
        assert!(doc
            .apply_edits(&[(0, 5, "a".into()), (3, 8, "b".into())])
            .is_err());
