//! Text-diff computation for out-of-band ingest (docs/design/ingest-and-materialization.md):
...
impl Block {
    fn into_edit(self) -> Edit {
        Edit { start: self.start, old_len: self.old_len, insert: self.insert }
    }
}
...
fn grouped_blocks<'a>(diff: &TextDiff<'a, 'a, '_, str>) -> Vec<Block> {
    ...
    for change in diff.iter_all_changes() {
        ...
        match change.tag() {
            ...
            ChangeTag::Delete => {
                let b = current.get_or_insert_with(|| Block { start: old_pos, old_len: 0, insert: String::new() });
                b.old_len += v.len();
                old_pos += v.len();
            }
            ChangeTag::Insert => {
...
mod tests {
    ...
    fn intra_line_edit_is_char_precise() {
        ...
        assert!(touched < 12, "expected char-level refinement, touched {touched} bytes: {edits:?}");
