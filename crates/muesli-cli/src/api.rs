...
//! (docs/design/local-agent-cli.md, mcp-and-agent-auth.md).
...
pub async fn device_flow(issuer: &str, client_id: &str) -> Result<String> {
    ...
    let url = auth.verification_uri_complete.clone().unwrap_or_else(|| auth.verification_uri.clone());
...
pub async fn cli_login(server: &str, id_token: &str, label: &str) -> Result<CliLoginResponse> {
    ...
    if !res.status().is_success() {
        bail!("server rejected login ({}): {}", res.status(), res.text().await.unwrap_or_default());
    }
...
pub async fn create_share(server: &str, token: &str, doc: &str, role: &str) -> Result<ShareLink> {
    let res = reqwest::Client::new()
        ...
        .await?;
    if !res.status().is_success() {
        bail!("share failed ({}): {}", res.status(), res.text().await.unwrap_or_default());
    }
...
fn join_api_path(base: &str, path: &str) -> Result<reqwest::Url> {
    ...
    let base_url = reqwest::Url::parse(base).with_context(|| format!("invalid server URL {base:?}"))?;
...
pub async fn api_request(
    ...
    Ok(RawResponse { status, body: value })
}
...
mod api_path_tests {
    ...
    fn accepts_rooted_same_origin_paths() {
        ...
        .unwrap();
        assert_eq!(u.as_str(), "http://localhost:8787/api/documents/notes/comments?mentions=me");
    }
