...
impl SyncShared {
    ...
    pub fn set_hash(&self, doc_id: &str, text: &str) {
        self.doc_hashes.lock().unwrap().insert(doc_id.to_string(), text_hash(text));
    }
...
pub enum SessionMode {
    Open { web: String },
    Sync { label: String, shared: Arc<SyncShared> },
}
...
impl SessionCtx {
    ...
    fn on_materialize(&self) {
        match &self.mode {
            SessionMode::Open { .. } => info!("materialized remote edits → {}", self.file.display()),
            SessionMode::Sync { label, shared } => {
...
impl FileSession {
    ...
    pub async fn run(
        ...
        loop {
            ...
            match self.run_once(fs_rx, stop_rx, bridge_ctl_rx, idle_timeout, &mut synced).await {
    ...
    async fn run_once(
        ...
        let disk_text = match std::fs::read(file) {
            Ok(bytes) => String::from_utf8(bytes).context("file is not valid UTF-8 — refusing to sync")?,
        ...
        if let Some(token) = &ctx.token {
            request
                .headers_mut()
                .insert(AUTHORIZATION, format!("Bearer {token}").parse().context("token header")?);
        }
        ...
        sink.send(Message::Binary(frame_sync(SYNC_STEP1, &replica.state_vector()))).await?;
        ...
        let idle_timer = tokio::time::sleep(idle_timeout.unwrap_or(Duration::from_secs(86_400 * 365)));
...
/// First-sync reconciliation of room state vs disk state (docs/design/ingest-and-
...
fn reconcile(
    ...
    if room_text.is_empty() && !disk_text.is_empty() {
        ...
        return Ok(Some(replica.diff_update(&sv).map_err(|e| anyhow::anyhow!(e.to_string()))?));
    }
    ...
    *last_written = disk_text.to_string();
    Ok(Some(replica.diff_update(&sv).map_err(|e| anyhow::anyhow!(e.to_string()))?))
}
...
pub fn atomic_write(path: &Path, text: &str) -> Result<()> {
    ...
    let tmp = dir.join(format!(
        ".{}.muesli-tmp",
        path.file_name().map(|s| s.to_string_lossy().to_string()).unwrap_or_else(|| "out".into())
    ));
