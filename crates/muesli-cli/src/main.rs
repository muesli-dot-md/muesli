//! `muesli` — the Local Agent (ADR 0014; internal/design/local-agent-cli.md).
//!
...
//! remote edits back to disk (internal/design/ingest-and-materialization.md). Hands-off git.
...
#[command(
    name = "muesli",
    version,
    about = "Muesli local agent — live multiplayer for plain .md files"
)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
...
async fn main() -> Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "muesli=info".into()),
        )
        .init();
    ...
    match Cli::parse().cmd {
        ...
        Cmd::Open {
            ...
            doc,
            web,
        } => open(file, server, doc, web).await,
        Cmd::Sync {
            dir,
            server,
            prefix,
            web,
        } => sync::sync(dir, server, prefix, web).await,
        Cmd::Share {
            target,
            role,
            server,
            web,
        } => share(target, role, server, web).await,
...
// ---------------------------------------------------------------------------
// mcp: stdio ⇄ POST /mcp proxy (internal/design/mcp-and-agent-auth.md)
// ---------------------------------------------------------------------------
...
async fn mcp_proxy(server: String) -> Result<()> {
    ...
    if token.is_none() {
        eprintln!(
            "muesli mcp: no stored token for {} (fine on an open server)",
            store::http_base(&server)
        );
    }
...
async fn login(server: String) -> Result<()> {
    ...
    if config.mode == "open" {
        println!(
            "✓ {} runs in open mode — no sign-in needed.",
            store::http_base(&server)
        );
        return Ok(());
    }
    let issuer = config
        .issuer
        .context("server reported oidc mode but no issuer")?;
    let client_id = config
        .cli_client_id
        .context("server reported no CLI client id")?;
...
async fn open(file: PathBuf, server: String, doc: Option<String>, web: String) -> Result<()> {
    ...
    let url = format!(
        "{}/{}",
        store::ws_base(&server).trim_end_matches('/'),
        doc_id
    );
    let token = store::load_token(&server);
    ...
    match session
        .run(&mut fs_rx, &mut stop_rx, &mut bridge_ctl_rx, None)
        .await?
    {
