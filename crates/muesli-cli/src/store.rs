//! Local state (docs/design/local-agent-cli.md): the API token lives in the OS keychain
...
fn config_dir() -> Result<PathBuf> {
    let dir = dirs::config_dir().context("no config dir for this OS user")?.join("muesli");
...
fn data_dir() -> Result<PathBuf> {
    let dir = dirs::data_dir().context("no data dir for this OS user")?.join("muesli");
    std::fs::create_dir_all(&dir)?;
    Ok(dir)
}
...
fn migrate_legacy_json_in(dir: &Path) -> Result<()> {
    ...
    drop(conn);
    std::fs::rename(&legacy, dir.join("links.json.migrated")).context("renaming links.json → links.json.migrated")?;
    warn!("migrated links.json → index.db ({} link(s)); old file kept as links.json.migrated", entries.len());
    write_mirror_in(dir)?;
    Ok(())
}
...
mod tests {
    ...
    fn tmp_dir(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("muesli-store-test-{tag}-{}", std::process::id()));
    ...
    fn fresh_index_roundtrip_and_mirror() {
        ...
        let a = load_links_in(&dir).unwrap().into_iter().find(|l| l.doc == "doc-a").unwrap();
        assert!(a.last_synced.is_some());

        // the legacy mirror exists, carries a marker + both entries
        let mirror: Vec<serde_json::Value> =
            serde_json::from_str(&std::fs::read_to_string(dir.join("links.json")).unwrap()).unwrap();
        assert!(mirror[0].get("_generated").is_some());
        assert_eq!(mirror.len(), 3);

        let removed = remove_link_in(&dir, Path::new("/tmp/a.md")).unwrap().unwrap();
    ...
    fn migrates_legacy_links_json() {
        ...
        assert!(dir.join("links.json.migrated").exists(), "original kept as .migrated");
    ...
    fn rebind_moves_the_path_and_keeps_the_doc() {
        ...
        rebind_link_in(&dir, "kept-doc", "http://localhost:8787", Path::new("/tmp/new.md")).unwrap();
        let links = load_links_in(&dir).unwrap();
        assert_eq!(links.len(), 1);
    ...
    fn keychain_gate_defaults_open() {
        let _lock = KEYCHAIN_GATE_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
