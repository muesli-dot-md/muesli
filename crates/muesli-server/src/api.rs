...
async fn room_call<T>(
    ...
    rx.await.map_err(|_| err(StatusCode::INTERNAL_SERVER_ERROR, "room dropped the request"))
}
...
pub(crate) async fn ctx(
    ...
    Ok(ApiCtx { access, room, persistence, document_id })
}
...
pub async fn list_comments(
    ...
    for cm in comments {
        by_thread.entry(cm.thread_id).or_default().push(CommentJson {
            id: cm.id,
            body: cm.body,
...
pub async fn create_comment(
    ...
    match c.persistence.create_thread(c.document_id, &anchor, c.access.user_id, &req.body).await {
...
async fn record_body_mentions(
    ...
    let recipients: Vec<Uuid> =
        recipients.into_iter().filter(|r| audience.contains(r)).collect();
    if recipients.is_empty() {
        return;
    }
...
async fn thread_in_doc(c: &ApiCtx, thread_id: Uuid) -> Result<String, Response> {
    match c.persistence.thread_ref(thread_id).await {
        ...
        Ok(_) => Err(err(StatusCode::NOT_FOUND, "no such thread on this document")),
        Err(e) => Err(err500(e)),
    }
}
...
pub async fn reply_comment(
    ...
    match c.persistence.add_comment(thread_id, c.access.user_id, &req.body).await {
        Ok(comment_id) => {
            record_body_mentions(&state, &c, &slug, thread_id, comment_id, &req.body).await;
...
async fn suggestion_json(c: &ApiCtx, s: SuggestionRow) -> Result<SuggestionJson, Response> {
    let range =
        room_call(&c.room, |reply| RoomMsg::ResolveAnchor { anchor: s.anchor.clone(), reply })
            .await?;
    Ok(SuggestionJson {
        id: s.id,
...
pub async fn list_suggestions(
    ...
    let mut rows = match c
        .persistence
        .list_suggestions(c.document_id, params.get("status").map(String::as_str), None)
        .await
    {
        Ok(rows) => rows,
        Err(e) => return err500(e),
...
pub async fn create_suggestion(
    ...
    for e in &edits {
        ...
        {
            return err(StatusCode::BAD_REQUEST, format!("bad edit range {}..{}", e.start, e.end));
        }
        let anchor = match room_call(&c.room, |reply| RoomMsg::CreateAnchor {
            start: e.start,
            end: e.end,
    ...
    match c
        .persistence
        .insert_suggestions(c.document_id, change_set_id, &items, c.access.user_id, req.note.as_deref())
        .await
    {
        Ok(ids) => Json(json!({
...
async fn get_doc_suggestion(c: &ApiCtx, id: Uuid) -> Result<SuggestionRow, Response> {
    match c.persistence.get_suggestion(id).await {
        ...
        Ok(_) => Err(err(StatusCode::NOT_FOUND, "no such suggestion on this document")),
        Err(e) => Err(err500(e)),
    }
}
...
pub async fn accept_suggestion(
    ...
    if s.status != "pending" {
        return err(StatusCode::CONFLICT, format!("suggestion is already {}", s.status));
    }
...
pub async fn reject_suggestion(
    ...
    if !(c.access.role.can_edit() || is_author) {
        return err(StatusCode::FORBIDDEN, "only editors (or the author) can reject");
    }
    if s.status != "pending" {
        return err(StatusCode::CONFLICT, format!("suggestion is already {}", s.status));
    }
...
pub async fn accept_change_set(
    ...
    if rows.is_empty() {
        return err(StatusCode::NOT_FOUND, "no pending suggestions in this change set");
    }
...
pub async fn reject_change_set(
    ...
    let is_author = c.access.user_id.is_some()
        && rows.iter().all(|s| c.access.user_id == s.author.as_ref().map(|a| a.id));
    if !(c.access.role.can_edit() || is_author) {
        return err(StatusCode::FORBIDDEN, "only editors (or the author) can reject");
    }
...
pub async fn history(
    ...
    match c.persistence.history(c.document_id, limit, before_seq).await {
...
pub async fn text(
    ...
    let want_seq = match params.get("seq").map(|s| s.parse::<i64>()) {
        ...
        Some(_) => return err(StatusCode::BAD_REQUEST, "seq must be a non-negative integer"),
        None => None,
    };
