//! Cross-document link graph (ADR 0015; internal/design/wikilinks-and-link-graph.md).
//!
//! Three parts:
//!
...
fn parse_md_link(s: &str) -> Option<(Option<ExtractedLink>, usize)> {
    ...
    let dest = dest
        ...
        .unwrap_or_else(|| dest.split_whitespace().next().unwrap_or(""));
    ...
    let Some(stem) = path_part
        .strip_suffix(".md")
        .or_else(|| path_part.strip_suffix(".MD"))
    else {
        return Some((None, consumed));
    };
...
impl LinkIndexer {
    pub fn spawn(persistence: Arc<Persistence>) -> Arc<LinkIndexer> {
        ...
        let indexer = Arc::new(LinkIndexer {
            persistence,
            handle: LinkHandle { tx },
        });
    ...
    async fn current_text(&self, document_id: Uuid) -> anyhow::Result<String> {
        ...
        if let Some(snap) = snapshot {
            doc.apply_update(&snap)
                .map_err(|e| anyhow::anyhow!("corrupt snapshot: {e}"))?;
        }
        for u in &updates {
            doc.apply_update(u)
                .map_err(|e| anyhow::anyhow!("corrupt update: {e}"))?;
        }
        Ok(doc.materialize())
    }
    ...
    async fn reindex(&self, document_id: Uuid) -> anyhow::Result<()> {
        ...
        let (inserted, deleted) = self
            .persistence
            .update_document_links(document_id, &links)
            .await?;
        self.persistence
            .upsert_search_text(document_id, &text)
            .await?;
    ...
    async fn on_hydrated(&self, document_id: Uuid) -> anyhow::Result<()> {
        ...
        let resolved = self
            .persistence
            .resolve_links_to(document_id, &slug)
            .await?;
        if resolved > 0 {
...
async fn event_loop(indexer: Arc<LinkIndexer>, mut rx: mpsc::UnboundedReceiver<LinkEvent>) {
    ...
    while let Some(event) = rx.recv().await {
        ...
        let indexer = indexer.clone();
        tokio::spawn(async move {
            // Every edit restarts the window; quiet (or shutdown) falls through to index.
            while let Ok(Some(())) = tokio::time::timeout(LINK_DEBOUNCE, reset.recv()).await {}
...
pub async fn document_links(
    ...
    Json(json!({
        "outgoing": outgoing
            .iter()
            .filter(|l| l.dst_id.is_none_or(&can_see))
            .map(|l| json!({
...
mod tests {
    ...
    fn extracts_wikilinks_with_labels_and_fragments() {
        let links =
            extract_links("See [[Other Page|a label]] and [[The Slug]] and [[Guide#Setup]].\n");
        assert_eq!(
            links
                .iter()
                .map(|l| (l.raw_target.as_str(), l.target_slug.as_str()))
                .collect::<Vec<_>>(),
            vec![
