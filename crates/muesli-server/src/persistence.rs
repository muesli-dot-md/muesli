...
impl Persistence {
    ...
    pub async fn load(&self, slug: &str) -> Result<DocState> {
        ...
        let (snap_seq, snapshot) = match snap {
            Some(r) => (r.get::<i64, _>("up_to_seq"), Some(r.get::<Vec<u8>, _>("snapshot_blob"))),
            None => (0, None),
        };
        ...
        .fetch_all(&self.pool)
        .await?;
        let last_seq = rows.last().map(|r| r.get::<i64, _>("seq")).unwrap_or(snap_seq);
        let tail = rows.into_iter().map(|r| r.get::<Vec<u8>, _>("update_blob")).collect();
    ...
    pub async fn load_at(&self, document_id: Uuid, seq: i64) -> Result<(Option<Vec<u8>>, Vec<Vec<u8>>)> {
        let snap = sqlx::query(
            "select up_to_seq, snapshot_blob from crdt_snapshots
        ...
        .fetch_all(&self.pool)
        .await?;
        Ok((snapshot, rows.into_iter().map(|r| r.get::<Vec<u8>, _>("update_blob")).collect()))
    }

    pub async fn save_snapshot(&self, document_id: Uuid, up_to_seq: i64, blob: &[u8]) -> Result<()> {
        sqlx::query(
    ...
    pub async fn prune_history(&self, document_id: Uuid) -> Result<(u64, u64)> {
        let latest: Option<i64> = sqlx::query_scalar(
            "select max(up_to_seq) from crdt_snapshots where document_id = $1",
        )
        .bind(document_id)
        .fetch_one(&self.pool)
        .await?;
        ...
        let updates = sqlx::query(
            "delete from crdt_updates where document_id = $1 and seq <= $2",
        )
        .bind(document_id)
        .bind(latest)
        .execute(&self.pool)
        ...
        let snaps = sqlx::query(
            "delete from crdt_snapshots where document_id = $1 and up_to_seq < $2",
        )
        .bind(document_id)
        .bind(latest)
        .execute(&self.pool)
    ...
    pub async fn ensure_document_owned(
        ...
        tx.commit().await?;
        Ok(EnsuredDoc { id: document_id, workspace_id: row.get("workspace_id"), created })
    }
    ...
    pub async fn user_role(
        ...
        let acl = sqlx::query(
            "select role from document_acl where document_id = $1 and user_id = $2",
        )
        .bind(document_id)
        .bind(user_id)
        .fetch_optional(&self.pool)
    ...
    pub async fn create_agent_user(&self, display_name: &str) -> Result<Uuid> {
        let row = sqlx::query(
            "insert into users (kind, display_name) values ('agent', $1) returning id",
        )
        .bind(display_name)
        .fetch_one(&self.pool)
        .await?;
    ...
    pub async fn create_thread(
        ...
        .execute(&mut *tx)
        .await?;
        sqlx::query("insert into comments (id, thread_id, author_id, body) values ($1, $2, $3, $4)")
            .bind(comment_id)
            .bind(thread_id)
            .bind(author_id)
            .bind(body)
    ...
    pub async fn add_comment(&self, thread_id: Uuid, author_id: Option<Uuid>, body: &str) -> Result<Uuid> {
        let comment_id = Uuid::now_v7();
        sqlx::query("insert into comments (id, thread_id, author_id, body) values ($1, $2, $3, $4)")
            .bind(comment_id)
            .bind(thread_id)
    ...
    pub async fn record_mentions(
        ...
        for recipient in recipient_ids {
            ...
            let in_app_on =
                channels.iter().any(|c| c == crate::notifications::CHANNEL_IN_APP);
    ...
    pub async fn list_comments(&self, document_id: Uuid) -> Result<Vec<CommentRow>> {
        ...
        .fetch_all(&self.pool)
        .await?;
        Ok(rows.into_iter().map(|r| CommentRow {
            id: r.get("id"),
    ...
    // -----------------------------------------------------------------------
    // MCP façade lookups (ADR 0008; docs/design/mcp-and-agent-auth.md)
    // -----------------------------------------------------------------------
    ...
    pub async fn workspace_role(&self, workspace_id: Uuid, user_id: Uuid) -> Result<Option<String>> {
        let row = sqlx::query(
    ...
    pub async fn add_membership(&self, workspace_id: Uuid, user_id: Uuid, role: &str) -> Result<()> {
        sqlx::query(
    ...
    pub async fn set_member_role(&self, workspace_id: Uuid, user_id: Uuid, role: &str) -> Result<bool> {
        let res = sqlx::query(
    ...
    pub async fn claim_invites(&self, user_id: Uuid, email: &str) -> Result<Vec<(Uuid, String)>> {
        ...
        .await?;
        tx.commit().await?;
        Ok(rows.into_iter().map(|r| (r.get("workspace_id"), r.get("role"))).collect())
    }
    ...
    pub async fn list_storage_connections(&self, workspace_id: Uuid) -> Result<Vec<StorageConnRow>> {
        let rows = sqlx::query(
    ...
    pub async fn delete_storage_connection(&self, id: Uuid, workspace_id: Uuid) -> Result<bool> {
        ...
        .await?;
        let res = sqlx::query(
            "delete from storage_connections where id = $1 and workspace_id = $2",
        )
        .bind(id)
        .bind(workspace_id)
        .execute(&mut *tx)
    ...
    pub async fn workspace_meta(&self, workspace_id: Uuid) -> Result<Option<WorkspaceMeta>> {
        let row = sqlx::query(
            "select status, storage_conn_id, retention from workspaces where id = $1",
        )
        .bind(workspace_id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn activate_workspace_with_storage(
        ...
        sqlx::query(
            "update workspaces set storage_conn_id = $2, status = 'active' where id = $1",
        )
        .bind(workspace_id)
        .bind(conn_id)
        .execute(&self.pool)
    ...
    pub async fn unattached_document(&self, document_id: Uuid) -> Result<Option<UnattachedDocFull>> {
        let row = sqlx::query(
    ...
    pub async fn update_document_links(
        ...
        let existing: Vec<String> = sqlx::query(
            "select raw_target from document_links where src_document_id = $1",
        )
        ...
        let gone: Vec<String> =
            existing.iter().filter(|t| !wanted.contains(t.as_str())).cloned().collect();
    ...
    pub async fn has_links_from(&self, document_id: Uuid) -> Result<bool> {
        let row = sqlx::query("select 1 as one from document_links where src_document_id = $1 limit 1")
            .bind(document_id)
            .fetch_optional(&self.pool)
            .await?;
        Ok(row.is_some())
    ...
    pub async fn live_folder_parents(
        ...
        .fetch_all(&self.pool)
        .await?;
        Ok(rows.into_iter().map(|r| (r.get("id"), r.get("parent_id"))).collect())
    }
    ...
    pub async fn folder_chain_names(&self, folder_id: Option<Uuid>) -> Result<Vec<String>> {
        let Some(folder_id) = folder_id else { return Ok(Vec::new()) };
        let rows = sqlx::query(
    ...
    pub async fn set_document_folder(&self, document_id: Uuid, folder_id: Option<Uuid>) -> Result<()> {
    ...
    pub async fn workspace_sso_configs(&self) -> Result<Vec<(Uuid, serde_json::Value)>> {
        let rows = sqlx::query("select id, sso from workspaces where sso is not null")
            .fetch_all(&self.pool)
            .await?;
        Ok(rows.into_iter().map(|r| (r.get("id"), r.get("sso"))).collect())
    }
...
mod tests {
    ...
    async fn onboarded_at_stamps_once_and_round_trips() {
        let Some(p) = test_db().await else {
            eprintln!("skipping: set TEST_DATABASE_URL to run onboarded_at_stamps_once_and_round_trips");
            return;
        };
        ...
        let first = p
            ...
            .onboarded_at
            .expect("stamped");
        assert!(first.contains('T') && first.ends_with('Z'), "house ISO shape: {first}");

        // Idempotent: a second stamp (e.g. finishing on a second device) keeps
        // the FIRST timestamp.
        assert!(p.set_user_onboarded(user).await.unwrap());
        let second = p.get_user(user).await.unwrap().unwrap().onboarded_at.unwrap();
    ...
    async fn record_mentions_is_idempotent_across_reparse() {
        ...
        let (thread_id, comment_id) =
            p.create_thread(doc.id, &serde_json::json!({"k": "v"}), Some(actor), &body).await.unwrap();

        // First record: the recipient is newly mentioned → one dispatch context enqueued.
        let first = p
            .record_mentions(
                doc.id, thread_id, comment_id, Some(actor), Some("Actor"), &slug, "Doc", &[recipient],
            )
            .await
            .unwrap();
        assert_eq!(first.len(), 1, "first parse inserts the mention + one notification");
        ...
        let second = p
            ...
            .await
            .unwrap();
        assert!(second.is_empty(), "re-parse must not double-insert or re-enqueue");
        ...
        .await
        .unwrap();
        assert_eq!(mention_count, 1, "exactly one mention row after two records");

        let notif_count: i64 = sqlx::query_scalar(
            "select count(*) from notification where recipient_id = $1",
        )
        .bind(recipient)
        .fetch_one(&p.pool)
        .await
        .unwrap();
        assert_eq!(notif_count, 1, "exactly one notification per recipient per comment");
    }
    ...
    async fn notifications_are_isolated_per_recipient() {
        let Some(p) = test_db().await else {
            eprintln!("skipping: set TEST_DATABASE_URL to run notifications_are_isolated_per_recipient");
            return;
        };
        ...
        let dispatch = p
            .record_mentions(
                doc.id, thread_id, comment_id, Some(actor), Some("Actor"), &slug, "Doc", &[user_a],
            )
            .await
            .unwrap();
        assert_eq!(dispatch.len(), 1, "A is newly mentioned → one notification enqueued");
        let a_notification_id =
            dispatch[0].notification_id.expect("in-app default on → a notification row exists");

        // B sees nothing: empty list, zero unread count.
        let b_list = p.list_notifications(user_b, false, None, 100).await.unwrap();
        assert!(b_list.is_empty(), "user B must not see user A's notifications");
        assert_eq!(
        ...
        let marked = p.mark_notification_read(a_notification_id, user_b).await.unwrap();
        assert!(!marked, "user B marking A's notification read must affect no rows");
        ...
        assert_eq!(a_list.len(), 1, "A still sees their own unread notification");
    ...
    async fn is_valid_timestamptz_accepts_real_timestamps_and_rejects_garbage() {
        ...
        assert!(p.is_valid_timestamptz("2026-06-26T12:00:00.123456Z").await.unwrap());
        assert!(p.is_valid_timestamptz("2026-06-26T12:00:00Z").await.unwrap());
        // Malformed cursors are rejected (→ 400), not surfaced as a 500.
        assert!(!p.is_valid_timestamptz("not-a-timestamp").await.unwrap());
        assert!(!p.is_valid_timestamptz("2026-13-99T99:99:99Z").await.unwrap());
    }
    ...
    async fn self_mention_creates_no_notification() {
        let Some(p) = test_db().await else {
            eprintln!("skipping: set TEST_DATABASE_URL to run self_mention_creates_no_notification");
            return;
        };
        ...
        let (thread_id, comment_id) =
            p.create_thread(doc.id, &serde_json::json!({"k": "v"}), Some(user_a), &body).await.unwrap();
        let dispatch = p
            .record_mentions(
                doc.id, thread_id, comment_id, Some(user_a), Some("A"), &slug, "Doc", &[user_a],
            )
            .await
            .unwrap();
        assert!(dispatch.is_empty(), "a self-mention enqueues no notification dispatch");
    ...
    async fn record_mentions_skips_notification_when_in_app_disabled() {
        ...
        p.set_notification_preference(recipient, EVENT_MENTION, CHANNEL_IN_APP, false).await.unwrap();
        p.set_notification_preference(recipient, EVENT_MENTION, CHANNEL_EMAIL, false).await.unwrap();
        // ensure_document_owned no longer auto-creates a workspace (BYO storage) — the
        // actor needs one up front.
        active_workspace(&p, "Actor WS", actor).await;
        let doc = p
        ...
        assert!(dispatch.is_empty(), "both channels off → no dispatch context produced");
        ...
        .await
        .unwrap();
        assert_eq!(mention_count, 1, "mention row recorded even with in-app disabled");
    ...
    async fn record_mentions_creates_notification_by_default() {
        ...
        assert!(dispatch[0].notification_id.is_some(), "in-app default on → a notification row id");
        ...
        .await
        .unwrap();
        assert_eq!(notif_count, 1, "default in-app on → exactly one notification row");
    }
    ...
    async fn record_mentions_dispatches_email_when_in_app_off_but_email_on() {
        ...
        assert_eq!(dispatch.len(), 1, "email enabled → a dispatch context is produced");
        assert_eq!(dispatch[0].recipient_id, recipient);
        assert_eq!(dispatch[0].recipient_email.as_deref(), Some("recipient@example.com"));
        assert!(dispatch[0].notification_id.is_none(), "in-app off → no notification row id");
        ...
        assert!(channels.iter().any(|c| c == CHANNEL_EMAIL), "email channel still resolved on");
        assert!(!channels.iter().any(|c| c == CHANNEL_IN_APP), "in-app not resolved");
        ...
        .await
        .unwrap();
        assert_eq!(notif_count, 0, "in-app off → no notification row, email handled out-of-band");
    }
    ...
    async fn workspace_lifecycle_pending_to_active() {
        let Some(p) = test_db().await else {
            eprintln!("skipping: set TEST_DATABASE_URL to run workspace_lifecycle_pending_to_active");
            return;
        };
        let owner = p.create_agent_user("owner").await.unwrap();
        let ws = p.create_workspace("BYO Test", owner).await.unwrap();

        let meta = p.workspace_meta(ws).await.unwrap().expect("workspace exists");
        assert_eq!(meta.status, "pending_storage");
        assert_eq!(meta.storage_conn_id, None);
        assert_eq!(meta.retention, None);

        let conn = p
            .create_storage_connection(ws, "s3", &serde_json::json!({"endpoint": "https://x", "bucket": "b"}))
            .await
            .unwrap();
        p.activate_workspace_with_storage(ws, conn).await.unwrap();
    ...
    async fn pending_workspaces_are_hidden_from_listings() {
        ...
        assert!(!p.list_workspaces(owner).await.unwrap().iter().any(|w| w.id == ws));
    ...
    async fn primary_workspace_lookup_never_creates() {
        let Some(p) = test_db().await else {
            eprintln!("skipping: set TEST_DATABASE_URL to run primary_workspace_lookup_never_creates");
            return;
        };
        ...
        let membership_count: i64 =
            sqlx::query_scalar("select count(*) from memberships where user_id = $1")
                ...
                .await
                .unwrap();
        assert_eq!(membership_count, 0, "the lookup must not auto-create a workspace/membership");
    ...
    async fn delete_storage_connection_clears_workspace_binding() {
        ...
        assert!(p.delete_storage_connection(conn, ws).await.unwrap(), "delete should succeed");

        let meta = p.workspace_meta(ws).await.unwrap().unwrap();
        assert_eq!(meta.storage_conn_id, None, "binding must be cleared");
        assert_eq!(meta.status, "active", "workspace stays active, grandfathered-unbound");
    }
    ...
    async fn workspace_meta_distinguishes_bound_from_grandfathered() {
        ...
        assert_eq!(meta.storage_conn_id, None, "grandfathered: connection exists, no binding");
        ...
        assert!(meta.storage_conn_id.is_some(), "bound workspace carries a storage_conn_id");
    }
    ...
    async fn prune_history_keeps_latest_snapshot_and_tail() {
        ...
        let doc = p.ensure_document_owned(&format!("prune-{}", Uuid::now_v7()), actor, actor)
            .await.unwrap();
        for seq in 1..=10i64 {
            p.append_update(doc.id, seq, b"u", "test", None, None).await.unwrap();
        }
        ...
        assert_eq!(updates_gone, 8, "seq 1..=8 pruned (covered by snapshot up_to 8)");
        assert_eq!(snaps_gone, 1, "snap4 pruned, snap8 kept");

        let remaining: i64 = sqlx::query_scalar(
            "select count(*) from crdt_updates where document_id = $1")
            .bind(doc.id).fetch_one(&p.pool).await.unwrap();
        assert_eq!(remaining, 2, "seq 9,10 survive");
        let snaps: i64 = sqlx::query_scalar(
            "select count(*) from crdt_snapshots where document_id = $1")
            .bind(doc.id).fetch_one(&p.pool).await.unwrap();
        assert_eq!(snaps, 1);

        // No snapshot at all → nothing is ever pruned (safety: full log IS the doc).
        let doc2 = p.ensure_document_owned(&format!("prune2-{}", Uuid::now_v7()), actor, actor)
            .await.unwrap();
        p.append_update(doc2.id, 1, b"u", "test", None, None).await.unwrap();
    ...
    async fn purge_only_touches_abandoned_pending_workspaces() {
        ...
        assert!(purged >= 1);
        assert!(p.workspace_meta(old_pending).await.unwrap().is_none(), "old pending purged");
        assert!(p.workspace_meta(fresh_pending).await.unwrap().is_some(), "fresh pending kept");
    }
}
