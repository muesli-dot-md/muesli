...
impl Ctx {
    ...
    async fn require_workspace(&self, workspace_id: Option<Uuid>) -> Result<(), Response> {
        ...
        let Some(ws) = workspace_id else {
            return Err(err(StatusCode::FORBIDDEN, "this folder belongs to no workspace"));
        };
...
async fn doc_editor(
    ...
    let restricted = principal.document_restriction.is_some_and(|d| d != doc.id)
        || principal.workspace_restriction.is_some_and(|w| doc.workspace_id != Some(w));
    if restricted {
        return Err(err(StatusCode::FORBIDDEN, "your token is restricted elsewhere"));
    }
...
async fn relocate_all(state: &AppState, doc_ids: Vec<Uuid>) -> Vec<String> {
    let Some(mgr) = state.storage.clone() else { return Vec::new() };
...
fn relocation_response(payload: serde_json::Value, errors: Vec<String>) -> Response {
    if errors.is_empty() {
        ...
        err(
            ...
            format!("updated, but storage relocation failed: {}", errors.join("; ")),
        )
    }
}
...
pub async fn create_folder(
    ...
    if !valid_folder_name(name) {
        return err(StatusCode::BAD_REQUEST, "folder name must be non-empty and contain no '/'");
    }
    ...
    match c.persistence.create_folder(workspace_id, parent.as_ref().map(|p| p.id), name).await {
        Ok(folder) => {
            audit::record(
                &c.persistence,
...
pub async fn update_folder(
    ...
    if folder.deleted_at.is_some() {
        return err(StatusCode::CONFLICT, "this folder is in the trash (restore it first)");
    }
    ...
    if let Some(Some(new_parent)) = req.parent_id {
        ...
        if parent.workspace_id != folder.workspace_id {
            return err(StatusCode::BAD_REQUEST, "cannot move a folder across workspaces");
        }
    ...
    match c.persistence.update_folder(id, name.as_deref(), req.parent_id).await {
        Ok(true) => {}
    ...
    audit::record(
        ...
        AuditEvent::new("folder_updated")
            ...
            .actor(c.user)
            .detail(json!({ "folder_id": id, "name": updated.name, "parent_id": updated.parent_id })),
    );
...
pub async fn update_document(
    ...
    if let Some(t) = &req.title {
        let t = t.as_deref().map(str::trim).filter(|s| !s.is_empty()).map(str::to_string);
    ...
    let errors = if moved { relocate_all(&state, vec![doc.id]).await } else { Vec::new() };
...
pub async fn restore_document(
    ...
    let errors =
        if folder_id != doc.folder_id { relocate_all(&state, vec![doc.id]).await } else { Vec::new() };
    audit::record(
        &p,
