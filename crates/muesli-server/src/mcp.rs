//! MCP façade over the sync server (ADR 0008; internal/design/mcp-and-agent-auth.md).
//!
...
pub async fn method_not_allowed() -> Response {
    (
        ...
        "use POST with a single JSON-RPC request",
    )
        .into_response()
}
...
async fn room_call<T>(
    ...
    room.send(make(tx))
        .map_err(|_| "room is gone".to_string())?;
...
impl Caller<'_> {
    ...
    async fn doc_by_slug(
        ...
        if let Some(p) = self.state.persistence.as_ref() {
            ...
            if !must_exist && exists {
                return Err(
                    "cannot create document: the slug is already in use or access is denied"
                        .to_string(),
                );
            }
        }
...
async fn announce_presence(room: &mpsc::UnboundedSender<RoomMsg>, name: String) {
    if let Ok(generation) = room_call(room, |reply| RoomMsg::AgentPresenceSet { name, reply }).await
    {
        let room = room.clone();
...
fn gate(c: &Caller<'_>, tool: &'static str) -> Result<(), String> {
    ...
    if let Some(p) = c.state.persistence.as_ref() {
        let action = if allowed {
            "mcp_gated_action_allowed"
        } else {
            "mcp_gated_action_denied"
        };
        audit::record(
            p,
...
fn audit_mcp(ctx: &DocCtx, c: &Caller<'_>, action: &'static str, mut detail: Value) {
    let Some(p) = ctx.persistence.as_ref() else {
        return;
    };
...
pub(crate) fn build_ops(
    ...
    for (i, e) in edits.iter().enumerate() {
        ...
        let op = if let Some(anchor) = e.get("anchor_text").and_then(Value::as_str) {
            ...
            let (s, end) =
                resolve_anchor_text(text, anchor).map_err(|m| format!("edit {i}: {m}"))?;
            match (insert, delete) {
                (None, false) => {
                    return Err(format!(
                        "edit {i}: anchor_text needs insert and/or delete:true"
                    ))
                }
...
async fn read_document(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    match args.get("version").and_then(Value::as_i64) {
        ...
        Some(version) if version >= 0 => {
            ...
            for u in &updates {
                doc.apply_update(u)
                    .map_err(|e| format!("corrupt update during replay: {e}"))?;
            }
...
async fn get_history(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    let limit = args
        ...
        .clamp(1, 500);
    let rows = db
        .history(document_id, limit, None)
        .await
        .map_err(internal)?;
...
async fn edit_document(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    let min_role = if requested == "direct" {
        Role::Editor
    } else {
        Role::Commenter
    };
    ...
    if requested == "direct" {
        ...
        out["reason"] = json!(
            "a human is co-present — your edit was stored as suggestions for review (ADR 0007)"
        );
    }
    Ok(out)
}
...
async fn reply_comment(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    let comment_id = db
        .add_comment(thread_id, ctx.access.user_id, body)
        .await
        .map_err(internal)?;
...
async fn resolve_comment(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    let (ctx, _) = thread_ctx(c, thread_id, Role::Commenter).await?;
    let (db, _) = ctx.persistence()?;
    db.set_thread_status(thread_id, "resolved")
        .await
        .map_err(internal)?;
    audit_mcp(
        &ctx,
...
async fn list_suggestions(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    let mut rows = db
        .list_suggestions(document_id, status, None)
        .await
        .map_err(internal)?;
...
async fn accept_suggestion(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    .await??;
    let (db, _) = ctx.persistence()?;
    db.set_suggestion_status(id, "accepted")
        .await
        .map_err(internal)?;
    audit_mcp(
        &ctx,
...
async fn reject_change_set(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    let is_author = ctx.access.user_id.is_some()
        && rows
            .iter()
            .all(|s| ctx.access.user_id == s.author.as_ref().map(|a| a.id));
    if !(ctx.access.role.can_edit() || is_author) {
        return Err("forbidden: only editors (or the author) can reject".into());
    }
    let (db, _) = ctx.persistence()?;
    ...
    for s in &rows {
        db.set_suggestion_status(s.id, "rejected")
            .await
            .map_err(internal)?;
        rejected.push(s.id);
    }
...
async fn update_document(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    rest_doc_result(
        crate::folders::update_document(c.app(), Path(slug), c.jar(), c.headers(), Json(req)).await,
    )
    .await
}
...
async fn trash_document(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    let slug = c.slug_from_args(args).await?;
    rest_doc_result(
        crate::folders::delete_document(c.app(), Path(slug), c.jar(), c.headers()).await,
    )
    .await
}
...
async fn notif_list(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    let rows = db
        .list_notifications(
            ...
            unread_only,
            before,
            crate::notifications_api::INBOX_LIMIT,
        )
        .await
        .map_err(internal)?;
...
async fn notif_mark_read(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    match db
        .mark_notification_read(id, user)
        .await
        .map_err(internal)?
    {
...
async fn ws_set_role(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    rest_result(
        crate::workspace::set_member_role(
            c.app(),
            Path((ws, user)),
            c.jar(),
            c.headers(),
...
async fn storage_delete(c: &Caller<'_>, args: &Value) -> Result<Value, String> {
    ...
    rest_result(
        crate::workspace::delete_storage_connection(
            c.app(),
            Path((ws, conn)),
            c.jar(),
            c.headers(),
...
mod tests {
    ...
    fn build_ops_range_and_replace_all() {
        ...
        let ops = build_ops(
            text,
            &[json!({ "range": { "start": 2, "end": 4 }, "insert": "X" })],
        )
        .unwrap();
        ...
        let err = build_ops(
            text,
            &[json!({ "replace_all": "a" }), json!({ "replace_all": "b" })],
        )
        .unwrap_err();
