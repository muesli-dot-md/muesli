...
pub(crate) fn q_file_in_folder(name: &str, folder_id: &str) -> String {
    format!("name='{}' and '{}' in parents and trashed=false", escape_q(name), escape_q(folder_id))
}

/// The `q` that finds the app folder at connect time.
pub(crate) fn q_app_folder() -> String {
    format!("name='{}' and mimeType='{FOLDER_MIME}' and trashed=false", escape_q(FOLDER_NAME))
}
...
fn load_client_file(path: &str) -> Result<ClientFileWeb> {
    ...
    let parsed: ClientFile =
        serde_json::from_str(&raw).with_context(|| format!("parsing {path} (expected {{\"web\":{{client_id,client_secret,…}}}})"))?;
    Ok(parsed.web)
}
...
impl GoogleCtx {
    ...
    fn from_env(public_url: &str) -> Result<Option<Self>> {
        ...
        if client_id.is_none() || client_secret.is_none() {
            ...
            let path = explicit
                .clone()
                .or_else(|| std::path::Path::new("muesli.json").exists().then(|| "muesli.json".to_string()));
            if let Some(path) = path {
                match load_client_file(&path) {
                    ...
                    Err(e) => warn!(%e, "ignoring unreadable ./muesli.json (google drive stays unconfigured)"),
                }
            }
        }
        ...
        let redirect_uri =
            format!("{}/auth/storage/google/callback", public_url.trim_end_matches('/'));
        Ok(Some(Self::new(client_id, client_secret, auth_uri, token_uri, api_base, redirect_uri)))
    }
    ...
    pub(crate) fn begin_at(
        ...
        pending.insert(token.clone(), PendingConnect { workspace_id, user_id, created, wizard });
        token
    }
    ...
    pub(crate) fn take_pending(&self, state: &str) -> Option<(Uuid, Uuid, bool)> {
        ...
        (p.created.elapsed() < CONNECT_ATTEMPT_TTL)
            .then_some((p.workspace_id, p.user_id, p.wizard))
    }
    ...
    fn invalidate_file_id(&self, folder_id: &str, name: &str) {
        self.file_ids.lock().unwrap().remove(&(folder_id.to_string(), name.to_string()));
    }
}
...
impl GdriveBackend {
    ...
    pub fn from_conn(kind: &str, config: &Value) -> Result<Self> {
        ...
        Ok(Self { ctx, refresh_token, folder_id: field("folder_id")? })
    }

    #[cfg(test)]
    fn for_tests(ctx: Arc<GoogleCtx>, refresh_token: &str, folder_id: &str) -> Self {
        Self { ctx, refresh_token: refresh_token.into(), folder_id: folder_id.into() }
    }
    ...
    async fn send_authed(
        ...
        for attempt in 0..2 {
            let token = self.ctx.access_token(&self.refresh_token, attempt > 0).await?;
            let mut req = self.ctx.http.request(method.clone(), url).bearer_auth(&token);
    ...
    async fn resolve(&self, rel_path: &str) -> Result<Option<(String, String)>> {
        ...
        let v: Value = res.json().await?;
        let Some(file) = v.pointer("/files/0") else { return Ok(None) };
        let id = file
            .get("id")
            .and_then(Value::as_str)
            .ok_or_else(|| anyhow!("drive files.list entry has no id"))?
            .to_string();
        let md5 = file.get("md5Checksum").and_then(Value::as_str).unwrap_or_default().to_string();
    ...
    async fn create_multipart(&self, rel_path: &str, bytes: &[u8]) -> Result<String> {
        ...
        let url = format!("{}?uploadType=multipart&fields=id,md5Checksum", self.upload_url());
        let res = self
            .send_authed(
                reqwest::Method::POST,
                &url,
        ...
        if !res.status().is_success() {
            let status = res.status();
            let body = res.text().await.unwrap_or_default();
            return Err(anyhow!("drive multipart create {rel_path}: {status} {body}"));
        }
        let v: Value = res.json().await?;
        if let Some(id) = v.get("id").and_then(Value::as_str) {
            self.ctx.cache_file_id(&self.folder_id, &name, id);
        }
        Ok(v.get("md5Checksum").and_then(Value::as_str).unwrap_or_default().to_string())
    }
}

impl StorageBackend for GdriveBackend {
    async fn read(&self, rel_path: &str) -> Result<Option<(Vec<u8>, String)>> {
        let Some((id, md5)) = self.resolve(rel_path).await? else { return Ok(None) };
        ...
        if res.status() == reqwest::StatusCode::NOT_FOUND {
            ...
            self.ctx.invalidate_file_id(&self.folder_id, &drive_name(rel_path));
            return Ok(None);
        }
        if !res.status().is_success() {
            let status = res.status();
    ...
    async fn list(&self, prefix: &str) -> Result<Vec<(String, String)>> {
        let q = format!("'{}' in parents and trashed=false", escape_q(&self.folder_id));
        let url = format!(
        ...
        Ok(files
            ...
            .filter_map(|f| {
                ...
                let md5 = f.get("md5Checksum").and_then(Value::as_str).unwrap_or_default();
    ...
    async fn delete(&self, rel_path: &str) -> Result<()> {
        ...
        let Some((id, _md5)) = self.resolve(rel_path).await? else { return Ok(()) };
        let url = format!("{}/{}", self.files_url(), uri_encode(&id, true));
        let res = self.send_authed(reqwest::Method::DELETE, &url, None).await?;
        self.ctx.invalidate_file_id(&self.folder_id, &drive_name(rel_path));
...
pub async fn start(
    ...
    if principal.workspace_restriction.is_some_and(|w| w != workspace_id) {
        return err(StatusCode::FORBIDDEN, "your token is restricted to another workspace");
    }
    match p.workspace_role(workspace_id, principal.role_user).await {
        Ok(Some(role)) if role == "admin" => {}
        Ok(_) => return err(StatusCode::FORBIDDEN, "requires the admin role on this workspace"),
...
pub async fn callback(
    ...
    if let Some(e) = params.error {
        ...
        let target = error_redirect(&auth.web_origin, pending.map(|(ws, _user, wizard)| (ws, wizard)));
    ...
    let Some((workspace_id, user_id, wizard)) = google.take_pending(&state_token) else {
        return err(StatusCode::BAD_REQUEST, "unknown or expired connect attempt");
    };
    ...
    let result: Result<Uuid> = async {
        ...
        let refresh = refresh.ok_or_else(|| {
            anyhow!("google returned no refresh_token (access_type=offline + prompt=consent expected)")
        })?;
        ...
        if workspace_already_bound(meta.as_ref()) {
            return Err(anyhow!("{ALREADY_BOUND} (bound during the oauth round-trip)"));
        }
        let id = p.create_storage_connection(workspace_id, "gdrive", &config).await?;
    ...
    match result {
        Ok(_) if wizard => {
            Redirect::to(&wizard_redirect(&auth.web_origin, workspace_id, "connected"))
                .into_response()
        }
        ...
        Err(e) if wizard => {
            ...
            Redirect::to(&wizard_redirect(&auth.web_origin, workspace_id, "error"))
                .into_response()
        }
        Err(e) => {
...
mod tests {
    ...
    fn rebind_guard_blocks_only_bound_workspaces() {
        ...
        // must NOT be blocked.
        assert!(!workspace_already_bound(Some(&meta("pending_storage", None))));
        ...
        assert!(workspace_already_bound(Some(&meta("active", Some(Uuid::now_v7())))));
    }
