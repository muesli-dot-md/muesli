...
pub enum RoomMsg {
    Join { conn: ConnId, tx: Outbound, can_edit: bool, author_id: Option<Uuid>, author_is_agent: bool },
    Leave { conn: ConnId },
    Inbound { conn: ConnId, data: Vec<u8> },
    /// Pin a byte range with sticky indices (ADR 0019); replies with the anchor's json form.
    CreateAnchor { start: usize, end: usize, reply: oneshot::Sender<Result<serde_json::Value, String>> },
    /// Current byte range of a stored anchor: None = unresolvable, start==end = text gone.
    ResolveAnchor { anchor: serde_json::Value, reply: oneshot::Sender<Option<(usize, usize)>> },
    GetText { reply: oneshot::Sender<String> },
    GetSeq { reply: oneshot::Sender<i64> },
    ...
    GetDocumentId { reply: oneshot::Sender<Option<Uuid>> },
    ...
    AgentPresenceSet { name: String, reply: oneshot::Sender<u64> },
    AgentPresenceClear { generation: u64 },
}
...
async fn run_room(
    ...
    while let Some(msg) = rx.recv().await {
        match msg {
            RoomMsg::Join { conn, tx, can_edit, author_id, author_is_agent } => {
            ...
            RoomMsg::ApplyEdit { ops, author_id, change_set_id, origin, reply } => {
                let _ = reply.send(room.on_apply_edit(ops, author_id, change_set_id, &origin).await);
            }
...
impl Room {
    ...
    async fn hydrate(&mut self) {
        let Some(p) = self.persistence.clone() else { return };
    ...
    async fn persist(
        ...
        let (Some(p), Some(document_id)) = (self.persistence.as_ref(), self.document_id) else { return };
        self.seq += 1;
        if let Err(e) = p.append_update(document_id, self.seq, update, origin, author_id, change_set_id).await {
    ...
    fn on_join(
        ...
        // with step 2 and sends its own step 1 (docs/design/sync-protocol.md).
        ...
        self.clients.insert(conn, ClientConn { tx, can_edit, author_id, is_agent });
    }
    ...
    fn on_leave(&mut self, conn: ConnId) {
        ...
        if let Some(states) = self.awareness.remove(&conn) {
            let removals: Vec<AwarenessEntry> = states
                .into_iter()
                .map(|(client_id, (clock, _))| AwarenessEntry { client_id, clock: clock + 1, json: "null".into() })
                .collect();
    ...
    async fn on_inbound(&mut self, conn: ConnId, data: &[u8]) {
        ...
        let Ok(msg_type) = c.read_var_u64() else { return };
        match msg_type {
            MSG_SYNC => {
                let Ok(subtype) = c.read_var_u64() else { return };
            ...
            MSG_QUERY_AWARENESS => {
                ...
                if let Some(client) = self.clients.get(&conn) {
                    let _ = client.tx.send(frame_awareness(&encode_awareness(&snapshot)));
                }
            }
            other => warn!(doc = %self.doc_id, conn, msg_type = other, "ignoring unknown message type"),
        }
    }
    ...
    fn on_awareness(&mut self, conn: ConnId, payload: &[u8]) {
        ...
        let accepted: Vec<AwarenessEntry> =
            entries.into_iter().filter(|e| self.awareness_entry_ok(conn, e)).collect();
    ...
    fn awareness_entry_ok(&self, conn: ConnId, e: &AwarenessEntry) -> bool {
        ...
        if !is_agent_conn {
            ...
            if state.pointer("/user/kind").and_then(serde_json::Value::as_str) == Some("agent") {
    ...
    async fn on_apply_edit(
        ...
        self.persist(&update, origin, author_id, change_set_id).await;
    ...
    fn agent_presence_set(&mut self, name: &str) -> u64 {
        ...
        let entry = AwarenessEntry { client_id: self.agent_client_id, clock: self.agent_clock, json };
    ...
    fn human_present(&self) -> bool {
        self.clients.iter().any(|(conn, c)| {
            !c.is_agent && self.awareness.get(conn).is_some_and(|s| !s.is_empty())
        })
    }
    ...
    fn awareness_snapshot(&self) -> Vec<AwarenessEntry> {
        self.awareness
            .values()
            .flat_map(|states| {
                states.iter().map(|(&client_id, (clock, json))| AwarenessEntry {
                    client_id,
                    clock: *clock,
                    json: json.clone(),
                })
