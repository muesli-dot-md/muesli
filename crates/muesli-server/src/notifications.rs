...
pub fn resolve_channels(event_type: &str, prefs: &[PreferenceRow]) -> Vec<String> {
    ALL_CHANNELS
        .iter()
        .filter(|&&channel| {
            match prefs.iter().find(|p| p.event_type == event_type && p.channel == channel) {
                Some(p) => p.enabled,
...
impl SmtpConfig {
    ...
    pub fn from_env() -> Option<Self> {
        ...
        Some(SmtpConfig {
            host,
            port: std::env::var("MUESLI_SMTP_PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(587),
...
impl Dispatcher {
    pub fn new(sender: Arc<dyn EmailSender>) -> Self {
        Self { email: EmailChannel { sender } }
    }
...
mod tests {
    ...
    fn pref(event: &str, channel: &str, enabled: bool) -> PreferenceRow {
        PreferenceRow { event_type: event.into(), channel: channel.into(), enabled }
    }
    ...
    fn resolve_uses_defaults_when_no_prefs() {
        ...
        assert_eq!(resolve_channels(EVENT_MENTION, &[]), vec![CHANNEL_IN_APP, CHANNEL_EMAIL]);
    }
    ...
    fn resolve_honors_disabled_email() {
        ...
        assert_eq!(resolve_channels(EVENT_MENTION, &prefs), vec![CHANNEL_IN_APP]);
    }
    ...
    fn resolve_keeps_in_app_on_by_default() {
        ...
        assert!(resolve_channels(EVENT_MENTION, &[]).iter().any(|c| c == CHANNEL_IN_APP));
    }
    ...
    fn resolve_honors_disabled_in_app() {
        ...
        assert!(!channels.iter().any(|c| c == CHANNEL_IN_APP), "in-app opted out → not present");
        // email is independent and stays on (its mention default).
        assert!(channels.iter().any(|c| c == CHANNEL_EMAIL), "email unaffected by in-app toggle");
    }
    ...
    fn resolve_in_app_and_email_are_independent() {
        ...
        assert_eq!(resolve_channels(EVENT_MENTION, &only_email), vec![CHANNEL_EMAIL]);
        // email off but in-app on.
        let only_in_app = vec![pref(EVENT_MENTION, CHANNEL_EMAIL, false)];
        assert_eq!(resolve_channels(EVENT_MENTION, &only_in_app), vec![CHANNEL_IN_APP]);
        // both off → fully muted.
        let neither = vec![
            pref(EVENT_MENTION, CHANNEL_IN_APP, false),
            pref(EVENT_MENTION, CHANNEL_EMAIL, false),
        ];
        assert!(resolve_channels(EVENT_MENTION, &neither).is_empty(), "both off → no channels");
    }
    ...
    fn resolve_honors_explicit_email_enable_for_non_default_type() {
        ...
        assert_eq!(resolve_channels("comment_reply", &prefs), vec![CHANNEL_IN_APP, CHANNEL_EMAIL]);
    }
    ...
    async fn dispatcher_skips_email_when_disabled() {
        ...
        assert!(sender.sent.lock().unwrap().is_empty(), "email opted out → no send");
    }
    ...
    async fn dispatcher_skips_email_when_recipient_has_no_address() {
        ...
        dispatcher.dispatch(&n, &[]).await;
        assert!(sender.sent.lock().unwrap().is_empty(), "no address → no send");
    }
}
