...
pub(crate) async fn ctx(
    ...
    Ok(WsCtx { persistence, user_id: principal.role_user, principal })
}
...
impl WsCtx {
    ...
    async fn member_role(&self, workspace_id: Uuid) -> Result<String, Response> {
        if self.principal.workspace_restriction.is_some_and(|w| w != workspace_id) {
            return Err(err(StatusCode::FORBIDDEN, "your token is restricted to another workspace"));
        }
        match self.persistence.workspace_role(workspace_id, self.user_id).await {
            Ok(Some(role)) => Ok(role),
            Ok(None) => Err(err(StatusCode::FORBIDDEN, "you are not a member of this workspace")),
            Err(e) => Err(err500(e)),
        }
    }
    ...
    pub(crate) async fn require_admin(&self, workspace_id: Uuid) -> Result<(), Response> {
        if self.member_role(workspace_id).await? != "admin" {
            return Err(err(StatusCode::FORBIDDEN, "requires the admin role on this workspace"));
        }
        Ok(())
    }
    ...
    async fn primary_workspace(&self) -> Result<Uuid, Response> {
        let list = self.persistence.list_workspaces(self.user_id).await.map_err(err500)?;
        list.first()
            .map(|w| w.id)
...
pub async fn list_workspaces(
    ...
    match c.persistence.list_workspaces(c.user_id).await {
        Ok(list) => Json(json!({
            "workspaces": list
                .iter()
                .filter(|w| !c.principal.workspace_restriction.is_some_and(|r| r != w.id))
                .map(|w| json!({
...
async fn workspace_detail(c: &WsCtx, workspace_id: Uuid, role: &str) -> Result<Value, Response> {
    ...
    let members = c.persistence.list_members(workspace_id).await.map_err(err500)?;
    let mut out = json!({
        "id": workspace_id,
        "name": name,
        "role": role,
    ...
    if role == "admin" {
        let invites = c.persistence.list_invites(workspace_id).await.map_err(err500)?;
        ...
        if let Some(sso) = c.persistence.workspace_sso(workspace_id).await.map_err(err500)? {
...
async fn invite_in(c: &WsCtx, workspace_id: Uuid, req: &InviteReq) -> Response {
    let Some(role) = parse_member_role(&req.role) else {
        return err(StatusCode::BAD_REQUEST, "role must be \"admin\" or \"member\"");
    };
    ...
    match c.persistence.find_user_by_email(&email).await {
        Err(e) => err500(e),
        Ok(Some(user_id)) => match c.persistence.add_membership(workspace_id, user_id, role).await {
            Ok(()) => {
                audit::record(
                    &c.persistence,
        ...
        Ok(None) => {
            match c.persistence.create_invite(workspace_id, &email, role, c.user_id).await {
                Ok(invite_id) => {
                    audit::record(
                        &c.persistence,
                        AuditEvent::new("invite_created")
...
pub async fn delete_invite(
    ...
    match c.persistence.delete_invite(workspace_id, invite_id).await {
        ...
        Ok(false) => err(StatusCode::NOT_FOUND, "no such pending invite on this workspace"),
        Err(e) => err500(e),
    }
}
...
pub async fn set_member_role(
    ...
    if last_admin_violation(&target_role, Some(role), admins) {
        return err(StatusCode::CONFLICT, "cannot demote the last admin of a workspace");
    }
    match c.persistence.set_member_role(workspace_id, member_id, role).await {
        Ok(true) => {
            audit::record(
                &c.persistence,
...
pub async fn remove_member(
    ...
    if caller_role != "admin" && member_id != c.user_id {
        return err(StatusCode::FORBIDDEN, "members may only remove themselves (leave)");
    }
    let target_role = match c.persistence.workspace_role(workspace_id, member_id).await {
        Ok(Some(r)) => r,
        Ok(None) => return err(StatusCode::NOT_FOUND, "no such member in this workspace"),
    ...
    if last_admin_violation(&target_role, None, admins) {
        return err(StatusCode::CONFLICT, "cannot remove the last admin of a workspace");
    }
...
pub async fn list_documents(
    ...
    let trashed = params.get("trashed").is_some_and(|v| v == "true" || v == "1");
    let docs = match p
...
async fn validate_storage_url(raw: &str) -> Result<(), String> {
    ...
    if let Some(list) = allowlist {
        ...
        return if allowed {
            ...
            Err(format!("storage host {host:?} is not on MUESLI_STORAGE_HOST_ALLOWLIST"))
        };
    }
    ...
    if url.scheme() != "https" {
        return Err(
            "storage endpoints must use https (or the host must be on \
    ...
    if let Ok(ip) = bare.parse::<std::net::IpAddr>() {
        if ip_is_internal(ip) {
            return Err(format!("storage endpoint address {bare} is private/loopback"));
        }
        return Ok(());
    }
    ...
    for addr in addrs {
        if ip_is_internal(addr.ip()) {
            return Err(format!("storage host {bare:?} resolves to a private/loopback address"));
        }
    }
    Ok(())
}
...
pub async fn create_storage_connection(
    ...
    let (kind, config) = match req.kind.as_str() {
        "s3" => {
            let access_key_id = req.access_key_id.as_deref().unwrap_or("").trim().to_string();
            ...
            let endpoint =
                req.endpoint.as_deref().unwrap_or("").trim_end_matches('/').to_string();
        ...
        "github" => {
            ...
            let api_base =
                req.api_base.as_deref().unwrap_or("").trim_end_matches('/').to_string();
            ...
            match crate::msgraph::SharePointBackend::from_conn("sharepoint", &config) {
                Ok(backend) => {
                    ...
                    if let Err(e) = backend.probe().await {
                        return err(
                            StatusCode::BAD_GATEWAY,
                            format!("storage probe failed: {e:#}{}", crate::msgraph::graph_hint(&e)),
                        );
                    }
                }
                Err(e) => return err(StatusCode::BAD_REQUEST, e.to_string()),
    ...
    match c.persistence.create_storage_connection(workspace_id, kind, &config).await {
        Ok(id) => {
            audit::record(
                &c.persistence,
...
pub async fn s3_policy(
    ...
    let bucket = params.get("bucket").map(String::as_str).unwrap_or("").trim();
    if bucket.is_empty() {
        return err(StatusCode::BAD_REQUEST, "bucket is required");
    }
    let prefix = params.get("prefix").map(String::as_str).unwrap_or("").trim();
    ...
    if let Some(obj) = config.as_object_mut() {
        match kind {
            "gdrive" => {
                ...
                obj.insert("has_refresh_token".into(), Value::Bool(had_plain || had_enc));
            }
            "s3" => {
            ...
            "sharepoint" => {
                ...
                obj.insert(
                    "credentials".into(),
                    Value::String(
                        if had_secret || had_key { "workspace" } else { "server-app" }.into(),
                    ),
                );
            }
            _ => {}
...
pub async fn set_workspace_sso(
    ...
    if issuer.is_empty() || client_id.is_empty() || client_secret.is_empty() {
        return err(StatusCode::BAD_REQUEST, "issuer, client_id, and client_secret are required");
    }
    ...
    if let Err(e) = auth.probe_issuer(&issuer, &client_id, &client_secret).await {
        ...
        return err(StatusCode::BAD_GATEWAY, format!("issuer discovery failed: {e:#}"));
    }
    ...
    match c.persistence.set_workspace_sso(workspace_id, Some(&sso)).await {
        Ok(true) => {}
...
pub async fn delete_workspace_sso(
    ...
    if !had {
        return err(StatusCode::NOT_FOUND, "this workspace has no SSO configuration");
    }
...
pub async fn delete_storage_connection(
    ...
    let conn = match c.persistence.get_storage_connection(conn_id, workspace_id).await {
        Ok(Some(s)) => s,
        Ok(None) => return err(StatusCode::NOT_FOUND, "no such storage connection on this workspace"),
        Err(e) => return err500(e),
    };
    ...
    if attached > 0 {
        return (StatusCode::CONFLICT, Json(json!({ "attached_documents": attached })))
            .into_response();
    }
    match c.persistence.delete_storage_connection(conn_id, workspace_id).await {
        Ok(true) => {
            audit::record(
                &c.persistence,
        ...
        Ok(false) => err(StatusCode::NOT_FOUND, "no such storage connection on this workspace"),
        Err(e) => err500(e),
    }
}
...
pub async fn storage_status(
    ...
    let kind = match c.persistence.get_storage_connection(conn_id, workspace_id).await {
...
pub async fn attach_document_storage(
    ...
    let conn = match p.get_storage_connection(req.storage_conn_id, doc_workspace).await {
        Ok(Some(s)) => s,
    ...
    if let Err(e) = crate::storage::validate_rel_path(&rel_path) {
        return err(StatusCode::BAD_REQUEST, format!("rel_path must be a clean relative path: {e}"));
    }
    ...
    match manager.materialize(doc.id).await {
        ...
        Err(e) => {
            ...
            err(StatusCode::BAD_GATEWAY, format!("failed to write to the storage backend: {e}"))
        }
    }
}
...
mod tests {
    ...
    fn gdrive_config_is_redacted_for_members() {
        let gdrive = json!({"refresh_token": "1//secret", "folder_id": "f1", "folder_name": "Muesli"});
    ...
    fn s3_and_github_configs_are_redacted_for_members() {
        ...
        assert_eq!(public.get("access_key_id").and_then(Value::as_str), Some("…MPLE"));
        assert_eq!(public.get("credentials").and_then(Value::as_str), Some("workspace"));
        ...
        assert_eq!(public.get("credentials").and_then(Value::as_str), Some("server-env"));
    }
    ...
    fn internal_addresses_are_recognized() {
        ...
        for a in internal {
            assert!(ip_is_internal(a.parse::<IpAddr>().unwrap()), "{a} must read as internal");
        }
        let external = ["8.8.8.8", "172.32.0.1", "140.82.112.3", "2606:4700::1111"];
        for a in external {
            assert!(!ip_is_internal(a.parse::<IpAddr>().unwrap()), "{a} must read as external");
        }
    }
    ...
    async fn storage_url_guard_blocks_ssrf_shapes() {
        ...
        assert!(validate_storage_url("https://127.0.0.1:9000").await.is_err());
        assert!(validate_storage_url("https://10.1.2.3").await.is_err());
        assert!(validate_storage_url("https://169.254.169.254/latest").await.is_err());
        assert!(validate_storage_url("https://[::1]:9000").await.is_err());
        assert!(validate_storage_url("https://localhost:9000").await.is_err());
        assert!(validate_storage_url("https://minio.localhost").await.is_err());
    ...
    fn sso_config_is_redacted_like_gdrive() {
        ...
        assert_eq!(public.get("client_secret"), None, "the secret must never be echoed");
    ...
    fn sp_req(
        creds: Option<(&str, Option<&str>, Option<(&str, &str)>)>, // (client_id, secret, (cert, key))
    ) -> CreateStorageReq {
    ...
    fn sharepoint_config_encrypts_ws_credentials() {
        ...
        assert!(config.get("client_secret").is_none(), "the request field never lands in config");

        // cert path wins over a simultaneously-sent secret
        let req = sp_req(Some(("ws-cid", Some("ws-secret"), Some(("CERT-PEM", "KEY-PEM")))));
        let config = sharepoint_config_from_req(&req, true).unwrap();
        assert_eq!(config["client_certificate_pem"], json!("CERT-PEM"), "cert is public, plaintext");
        let key_enc = config["client_private_key_enc"].as_str().expect("encrypted");
        assert_eq!(crate::secrets::decrypt_secret(key_enc).unwrap(), "KEY-PEM");
        assert!(config.get("client_secret_enc").is_none(), "cert wins over secret");
    }
    ...
    fn sharepoint_config_is_redacted_for_members() {
        ...
        assert_eq!(public["site_url"], json!("https://contoso.sharepoint.com/sites/eng"));
        ...
        assert_eq!(public_config("sharepoint", &server_app)["credentials"], json!("server-app"));
    }
    ...
    async fn sharepoint_connection_row_round_trips() {
        let Ok(url) = std::env::var("TEST_DATABASE_URL") else {
            eprintln!("skipping: set TEST_DATABASE_URL to run sharepoint_connection_row_round_trips");
            return;
        };
        ...
        let id = p.create_storage_connection(ws, "sharepoint", &config).await.unwrap();
