...
pub(crate) fn pem_blocks(pem: &str) -> Vec<(String, Vec<u8>)> {
    ...
    for line in pem.lines() {
        let line = line.trim();
        if let Some(l) = line.strip_prefix("-----BEGIN ").and_then(|r| r.strip_suffix("-----")) {
...
pub(crate) fn parse_private_key_pem(pem: &str) -> Result<rsa::RsaPrivateKey> {
    ...
    Err(anyhow!("no PRIVATE KEY / RSA PRIVATE KEY block found in the PEM input"))
}
...
pub(crate) fn resolve_credential(
    ...
    let field = |name: &str| config.get(name).and_then(Value::as_str).filter(|s| !s.is_empty());
    if let Some(client_id) = field("client_id") {
        if let (Some(cert_pem), Some(key_enc)) =
            (field("client_certificate_pem"), field("client_private_key_enc"))
        {
            ...
            return Ok(MsCredential {
                ...
                auth: MsAuth::Certificate { cert_der, key },
            });
        }
...
pub(crate) enum MsAuth {
    Secret(String),
    Certificate { cert_der: Vec<u8>, key: rsa::RsaPrivateKey },
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            MsAuth::Certificate { cert_der, .. } => {
                write!(f, "Certificate {{ cert_der: <{} bytes>, key: <redacted> }}", cert_der.len())
            }
        }
    }
}
...
impl MsCtx {
    pub(crate) fn new(login_base: String, graph_base: String, env_app: Option<MsCredential>) -> Self {
        Self {
    ...
    fn from_env() -> Result<Self> {
        ...
        let auth = if let Some(path) = env("MUESLI_MS_CLIENT_CERT_FILE") {
            let raw = std::fs::read_to_string(&path)
                .with_context(|| format!("MUESLI_MS_CLIENT_CERT_FILE is set but unreadable: {path}"))?;
            let cert_der =
                parse_cert_pem(&raw).context("MUESLI_MS_CLIENT_CERT_FILE has no CERTIFICATE block")?;
            let key = parse_private_key_pem(&raw)
                .context("MUESLI_MS_CLIENT_CERT_FILE has no private key block")?;
            Some(MsAuth::Certificate { cert_der, key })
        } else {
    ...
    pub(crate) fn store_access(&self, tenant: &str, client_id: &str, access_token: &str, expires_in: u64) {
    ...
    pub(crate) async fn access_token(&self, tenant: &str, cred: &MsCredential, force: bool) -> Result<String> {
        if force {
            self.tokens.lock().unwrap().remove(&(tenant.to_string(), cred.client_id.clone()));
        ...
        if !status.is_success() {
            ...
            let desc: String = v
                ...
                .take(200)
                .collect();
            return Err(anyhow!("microsoft token endpoint answered {status} {code}: {desc}"));
        }
        let v: Value =
...
async fn err_detail(res: reqwest::Response) -> String {
    res.text().await.unwrap_or_default().chars().take(300).collect()
}
...
impl SharePointBackend {
    ...
    pub fn from_conn(kind: &str, config: &Value) -> Result<Self> {
        if kind != "sharepoint" {
            return Err(anyhow!("SharePointBackend cannot serve storage kind {kind:?}"));
        }
        let ctx = msctx()
        ...
        let mut prefix =
            config.get("prefix").and_then(Value::as_str).unwrap_or("").trim_matches('/').to_string();
        if !prefix.is_empty() {
            validate_rel_path(&prefix)?;
            prefix.push('/');
        }
        let cred = resolve_credential(config, ctx.env_app.as_ref())?;
        Ok(Self { cred, tenant: field("tenant_id")?, drive_id: field("drive_id")?, prefix, ctx })
    }
    ...
    pub(crate) fn for_tests(
        ...
        Self { ctx, cred, tenant: tenant.into(), drive_id: drive_id.into(), prefix }
    }
    ...
    fn item_url(&self, rel_path: &str) -> String {
        format!(
            ...
            uri_encode(&format!("{}{}", self.prefix, rel_path.trim_start_matches('/')), false),
        )
    }
    ...
    async fn write_via_session(&self, rel_path: &str, bytes: &[u8]) -> Result<String> {
        ...
        for (i, chunk) in bytes.chunks(UPLOAD_CHUNK).enumerate() {
            ...
            let res = self
                ...
                .map_err(|e| {
                    anyhow!("sharepoint chunk upload {rel_path} ({start}-{end}): {}", e.without_url())
                })?;
            let status = res.status();
            if !status.is_success() {
                return Err(anyhow!(
            ...
            if status == reqwest::StatusCode::OK || status == reqwest::StatusCode::CREATED {
                ...
                item = Some(res.json().await.map_err(|e| {
                    anyhow!("sharepoint upload session driveItem for {rel_path}: {}", e.without_url())
                })?);
            }
        }
        let item = item
...
impl StorageBackend for SharePointBackend {
    async fn read(&self, rel_path: &str) -> Result<Option<(Vec<u8>, String)>> {
        ...
        let res = self.send_authed(reqwest::Method::GET, &self.item_url(rel_path), None).await?;
        if res.status() == reqwest::StatusCode::NOT_FOUND {
            return Ok(None);
        }
        if !res.status().is_success() {
            let status = res.status();
            return Err(anyhow!("sharepoint GET {rel_path}: {status} {}", err_detail(res).await));
        }
        let v: Value = res.json().await?;
        ...
        let res = self
            ...
            .send()
            .await
            .map_err(|e| anyhow!("sharepoint content download for {rel_path}: {}", e.without_url()))?;
        if !res.status().is_success() {
            let status = res.status();
    ...
    async fn write(&self, rel_path: &str, bytes: &[u8]) -> Result<String> {
        ...
        if bytes.len() <= SIMPLE_UPLOAD_MAX {
            ...
            if !res.status().is_success() {
                let status = res.status();
                return Err(anyhow!("sharepoint PUT {rel_path}: {status} {}", err_detail(res).await));
            }
            let v: Value = res.json().await?;
    ...
    async fn list(&self, prefix: &str) -> Result<Vec<(String, String)>> {
        ...
        let full =
            format!("{}{}", self.prefix, rel_prefix).trim_matches('/').to_string();
        ...
        loop {
            ...
            if !res.status().is_success() {
                let status = res.status();
                return Err(anyhow!("sharepoint LIST {prefix}: {status} {}", err_detail(res).await));
            }
            let v: Value = res.json().await?;
            for item in v.get("value").and_then(Value::as_array).into_iter().flatten() {
    ...
    async fn delete(&self, rel_path: &str) -> Result<()> {
        validate_rel_path(rel_path)?;
        let res = self.send_authed(reqwest::Method::DELETE, &self.item_url(rel_path), None).await?;
        // Already absent = done (idempotent, matching the trait contract).
        if !res.status().is_success() && res.status() != reqwest::StatusCode::NOT_FOUND {
            let status = res.status();
            return Err(anyhow!("sharepoint DELETE {rel_path}: {status} {}", err_detail(res).await));
        }
        Ok(())
    }
}
...
pub(crate) fn site_path_of(site_url: &str) -> Result<(String, String)> {
    let url = reqwest::Url::parse(site_url)
        .with_context(|| format!("invalid site url {site_url:?}"))?;
    let host = url
        .host_str()
    ...
    let raw_path = site_url
        ...
        .map_or("", |(_, p)| p);
    let raw_path = raw_path.split(['?', '#']).next().unwrap_or("").trim_end_matches('/');
    // "".split('/') yields a single empty element, not zero — an empty (root) path is
    // valid and must not be mistaken for an empty segment.
    if !raw_path.is_empty() && raw_path.split('/').any(|seg| seg.is_empty() || seg == "." || seg == "..") {
        return Err(anyhow!(
...
pub(crate) async fn resolve_site(
    ...
    let url = if path.is_empty() {
        ...
        format!("{}/sites/{}:{}", ctx.graph_base, uri_encode(&host, true), uri_encode(&path, false))
    };
    let res = send_authed_with(ctx, cred, tenant, reqwest::Method::GET, &url, None).await?;
    if !res.status().is_success() {
        let status = res.status();
        return Err(anyhow!("graph site resolve {site_url}: {status} {}", err_detail(res).await));
    }
    let v: Value = res.json().await?;
    let site_id = v
        .get("id")
...
pub(crate) async fn list_libraries(
    ...
    let url = format!("{}/sites/{}/drives", ctx.graph_base, uri_encode(site_id, true));
    let res = send_authed_with(ctx, cred, tenant, reqwest::Method::GET, &url, None).await?;
    if !res.status().is_success() {
        let status = res.status();
        return Err(anyhow!("graph drive list {site_id}: {status} {}", err_detail(res).await));
    }
    let v: Value = res.json().await?;
    let drives = v.get("value").and_then(Value::as_array).cloned().unwrap_or_default();
    let default_id = async {
        let url = format!("{}/sites/{}/drive", ctx.graph_base, uri_encode(site_id, true));
        ...
        res.json::<Value>().await.ok()?.get("id").and_then(Value::as_str).map(str::to_string)
    }
    .await;
    Ok(drives
        .iter()
...
pub(crate) fn ephemeral_credential(
    ...
    env_app.cloned().ok_or_else(|| {
        anyhow!("this server has no Microsoft app configured — provide your own Entra app credentials")
    })
}
...
pub async fn list_libraries_endpoint(
    ...
    let Some(ctx) = msctx() else {
        return err(StatusCode::SERVICE_UNAVAILABLE, "microsoft graph support is not initialized");
    };
    let tenant = req.tenant.trim().to_string();
    if !valid_tenant(&tenant) {
        return err(StatusCode::BAD_REQUEST, "tenant must be a GUID or a domain ([A-Za-z0-9.-]+)");
    }
    ...
    let libraries = match list_libraries(&ctx, &cred, &tenant, &site.site_id).await {
        Ok(l) => l,
        Err(e) => {
            return err(StatusCode::BAD_GATEWAY, format!("library list failed: {e:#}{}", graph_hint(&e)))
        }
    };
    Json(serde_json::json!({
...
mod tests {
    ...
    fn from_env_reads_the_ms_client() {
        ...
        for k in ["MUESLI_MS_CLIENT_ID", "MUESLI_MS_CLIENT_SECRET", "MUESLI_MS_LOGIN_BASE",
                  "MUESLI_MS_GRAPH_BASE", "MUESLI_MS_CLIENT_CERT_FILE"] {
    ...
    async fn secret_token_acquisition_and_cache() {
        ...
        let cred = MsCredential { client_id: "cid".into(), auth: MsAuth::Secret("cs".into()) };
        assert_eq!(ctx.access_token("tenant-a", &cred, false).await.unwrap(), "tok-tenant-a-1");
        // cached: no second POST
        assert_eq!(ctx.access_token("tenant-a", &cred, false).await.unwrap(), "tok-tenant-a-1");
        assert_eq!(calls.load(Ordering::SeqCst), 1);
        // another tenant is a separate cache entry
        assert_eq!(ctx.access_token("tenant-b", &cred, false).await.unwrap(), "tok-tenant-b-2");
        // force drops the cached entry first (the 401-retry path)
        assert_eq!(ctx.access_token("tenant-a", &cred, true).await.unwrap(), "tok-tenant-a-3");
    }
    ...
    async fn token_errors_surface_the_aadsts_code() {
        ...
        let base = serve(app).await;
        let ctx = MsCtx::new(base.clone(), format!("{base}/v1.0"), None);
        let cred = MsCredential { client_id: "cid".into(), auth: MsAuth::Secret("bad".into()) };
        let err = ctx.access_token("t", &cred, false).await.unwrap_err().to_string();
    ...
    fn pem_parsing_and_x5t() {
        ...
        assert_eq!(parse_private_key_pem(&pkcs1).unwrap().to_public_key(), key.to_public_key());
    ...
    fn client_assertion_is_a_valid_rs256_jwt() {
        ...
        assert!(claims["jti"].as_str().is_some_and(|j| uuid::Uuid::parse_str(j).is_ok()));
    ...
    fn from_env_prefers_certificate_over_secret_and_fails_fast() {
        ...
        let path = std::env::temp_dir().join(format!("muesli-ms-cert-{}.pem", uuid::Uuid::new_v4()));
        ...
        let ctx = MsCtx::from_env().unwrap();
        assert!(
            matches!(ctx.env_app.as_ref().unwrap().auth, MsAuth::Certificate { .. }),
    ...
    async fn certificate_token_acquisition_sends_a_client_assertion() {
        ...
        let cred = MsCredential {
            client_id: "cid".into(),
            auth: MsAuth::Certificate { cert_der: b"dummy-cert-der-bytes".to_vec(), key },
        };
        assert_eq!(ctx.access_token("t", &cred, false).await.unwrap(), "tok-cert");
    }
    ...
    fn credential_resolution_precedence_matrix() {
        ...
        let env_secret = MsCredential { client_id: "env-cid".into(), auth: MsAuth::Secret("env-s".into()) };
        ...
        let err = resolve_credential(&json!({}), None).unwrap_err().to_string();
    ...
    fn test_cred() -> MsCredential {
        MsCredential { client_id: "cid".into(), auth: MsAuth::Secret("cs".into()) }
    }
        ...
        let app = axum::Router::new().fallback(move |req: axum::extract::Request| {
            ...
            async move {
                ...
                if method == axum::http::Method::POST && path == "/t1/oauth2/v2.0/token" {
                    ...
                    return axum::Json(json!({ "access_token": format!("t{n}"), "expires_in": 3600 }))
                        .into_response();
                }
                // Everything below is Graph: enforce the bearer (t2 when flaky, else any t*).
                let ok = if flaky_401 { auth == "Bearer t2" } else { auth.starts_with("Bearer t") };
                ...
                match (method.as_str(), path.as_str()) {
                    ...
                    .into_response(),
                    _ => (StatusCode::NOT_FOUND, format!("unmocked {method} {path}")).into_response(),
                }
            }
        });
        tokio::spawn(async move { axum::serve(listener, app).await.unwrap() });
    ...
    async fn sharepoint_read_enforces_the_ingest_cap_from_metadata() {
        use crate::storage::StorageBackend as _;
        let (base, _) = read_mock(false).await;
        let err = backend_at(&base).read("huge.md").await.unwrap_err().to_string();
    ...
    async fn sharepoint_download_transport_failure_never_leaks_the_preauth_url() {
        ...
        assert!(!err.contains('?'), "the pre-authenticated query string must not survive: {err}");
    }
    ...
    async fn sharepoint_read_refreshes_and_retries_on_401() {
        ...
        assert_eq!(bytes, b"# from graph\n");
        assert_eq!(token_calls.load(Ordering::SeqCst), 2, "one mint + one forced refresh");
    ...
    async fn sharepoint_list_pages_through_next_links_and_skips_folders() {
        ...
        assert_eq!(listed, vec![("a.md".into(), "c-a".into()), ("b.md".into(), "e-b".into())]);
    }
    ...
    async fn write_mock() -> (String, Arc<std::sync::Mutex<HashMap<String, Vec<u8>>>>, Arc<std::sync::Mutex<Vec<String>>>) {
        ...
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let base = format!("http://{}", listener.local_addr().unwrap());
        let (b, s, r, sb) = (base.clone(), store.clone(), ranges.clone(), session_buf.clone());
        let app = axum::Router::new().fallback(move |req: axum::extract::Request| {
            ...
            async move {
                ...
                if let Some(rest) = path.strip_prefix(ITEM) {
                    ...
                    if method == axum::http::Method::POST && rest.ends_with(":/createUploadSession") {
                        return axum::Json(json!({ "uploadUrl": format!("{base}/upload/session-1") }))
                            .into_response();
                    }
                ...
                if method == axum::http::Method::PUT && path == "/upload/session-1" {
                    ...
                    return if end + 1 == total {
                        ...
                        (StatusCode::CREATED, axum::Json(json!({ "cTag": "c-session" })))
                            .into_response()
                    } else {
                        (
                            StatusCode::ACCEPTED,
    ...
    async fn sharepoint_small_write_uses_the_simple_put() {
        ...
        assert_eq!(store.lock().unwrap().get("pre/doc.md").unwrap(), b"# hello\n");
        assert!(ranges.lock().unwrap().is_empty(), "no upload session for small writes");
    }
    ...
    async fn sharepoint_write_boundary_at_simple_upload_max() {
        ...
        assert!(ranges.lock().unwrap().is_empty(), "exactly SIMPLE_UPLOAD_MAX must use the simple PUT");
    ...
    async fn site_resolve_and_library_list() {
        ...
        let ctx = MsCtx::new(base.clone(), format!("{base}/v1.0"), None);
        let cred = test_cred();
        let site = resolve_site(&ctx, &cred, "t1", "https://contoso.sharepoint.com/sites/eng/")
            .await
            .unwrap();
        assert_eq!(site.site_id, "contoso.sharepoint.com,g1,g2");
        assert_eq!(site.site_name, "Engineering");
        let libs = list_libraries(&ctx, &cred, "t1", &site.site_id).await.unwrap();
    ...
    fn site_path_of_rejects_dot_and_empty_segments() {
        let err = site_path_of("https://host/sites/../../v1.0/anything").unwrap_err().to_string();
        assert!(err.contains("invalid segments"), "{err}");
        let err = site_path_of("https://host/sites/./eng").unwrap_err().to_string();
        assert!(err.contains("invalid segments"), "{err}");
        let err = site_path_of("https://host/sites//eng").unwrap_err().to_string();
        assert!(err.contains("invalid segments"), "{err}");
        let err = site_path_of("https://host/sites/a/../b").unwrap_err().to_string();
        assert!(err.contains("invalid segments"), "{err}");
    ...
    fn tenant_validation_guid_or_domain() {
        ...
        assert!(!valid_tenant("bad?tenant"));
        assert!(!valid_tenant("bad#tenant"));
        assert!(!valid_tenant(&"x".repeat(300)), "absurd lengths are refused");
    }
    ...
    fn ephemeral_credential_matrix() {
        ...
        let cred = ephemeral_credential(
            Some("body-cid"), Some("body-secret"), Some(&cert_pem), Some(&key_pem), Some(&env),
        )
        .unwrap();
        ...
        let cred =
            ephemeral_credential(Some("body-cid"), Some("body-secret"), None, None, Some(&env))
                .unwrap();
        assert!(matches!(cred.auth, MsAuth::Secret(ref s) if s == "body-secret"));

        // no body creds → env app
        assert_eq!(ephemeral_credential(None, None, None, None, Some(&env)).unwrap().client_id, "cid");
    ...
    async fn sharepoint_endpoints_require_identity() {
        ...
        let res = setup(State(state.clone()), jar.clone(), axum::http::HeaderMap::new()).await;
