...
async fn main() -> anyhow::Result<()> {
    ...
    let persistence = match std::env::var("DATABASE_URL") {
        ...
        Err(_) => {
            warn!("DATABASE_URL not set — running VOLATILE (in-memory only, edits lost on restart)");
            None
        }
    };

    let web_origin = std::env::var("MUESLI_WEB_ORIGIN")
        .unwrap_or_else(|_| "http://localhost:5173".into());
    let public_url = std::env::var("MUESLI_PUBLIC_URL")
    ...
    let cors = CorsLayer::new()
        ...
        .allow_headers([header::CONTENT_TYPE, HeaderName::from_static("x-muesli-share")])
    ...
    let links = persistence.as_ref().map(|p| LinkIndexer::spawn(p.clone()).handle());
    ...
    let auth_rl =
        axum::middleware::from_fn_with_state(AuthRateLimiter::default(), auth_rate_limit);
    ...
    let mut app = Router::new()
        ...
        .route("/auth/login/select", get(auth::login_select).layer(auth_rl.clone()))
        ...
        // Account settings (docs/design/settings.md): delegated API keys + version probe.
        .route("/api/me/tokens", get(account::list_tokens).post(account::mint_token))
        .route("/api/me/tokens/{id}", axum::routing::delete(account::revoke_token))
        ...
        .route("/api/documents/{slug}/comments/{thread_id}/replies", post(api::reply_comment))
        .route("/api/documents/{slug}/comments/{thread_id}/resolve", post(api::resolve_thread))
        .route("/api/documents/{slug}/comments/{thread_id}/reopen", post(api::reopen_thread))
        .route(
            "/api/documents/{slug}/suggestions",
            get(api::list_suggestions).post(api::create_suggestion),
        )
        .route("/api/documents/{slug}/suggestions/{id}/accept", post(api::accept_suggestion))
        .route("/api/documents/{slug}/suggestions/{id}/reject", post(api::reject_suggestion))
        .route(
        ...
        .route("/api/notifications", get(notifications_api::list_notifications))
        .route("/api/notifications/unread-count", get(notifications_api::unread_count))
        .route("/api/notifications/{id}/read", post(notifications_api::mark_read))
        .route("/api/notifications/read-all", post(notifications_api::read_all))
        .route(
        ...
        .route("/api/documents/{slug}/restore", post(folders::restore_document))
        .route("/api/documents/{slug}/purge", axum::routing::delete(folders::purge_document))
        ...
        .route("/api/documents/{slug}/storage", post(workspace::attach_document_storage))
        .route(
            "/api/workspaces",
        ...
        .route("/api/workspaces/{id}/storage/status", get(workspace::storage_status))
        ...
        .route("/api/workspaces/{id}/audit", get(audit::list_workspace_audit))
        ...
        .route("/api/workspaces/{id}/storage/google/start", get(gdrive::start))
        .route(
        ...
        .route("/api/workspace/invites", post(workspace::invite_to_current_workspace))
        .route("/api/cli/auth-config", get(auth::cli_auth_config))
        .route("/api/cli/login", post(auth::cli_login).layer(auth_rl.clone()))
...
async fn handle_socket(socket: WebSocket, doc_id: String, state: AppState, access: Access) {
    ...
    while let Some(Ok(msg)) = stream.next().await {
        match msg {
            Message::Binary(data) => {
                if room.send(RoomMsg::Inbound { conn, data: data.into() }).is_err() {
                    break;
                }
            }
