...
use openidconnect::{
    ...
    EndpointNotSet, EndpointSet, IssuerUrl, JsonWebKeySet, JsonWebKeySetUrl, Nonce, NonceVerifier,
...
impl JwksCache {
    fn new(jwks_uri: JsonWebKeySetUrl, keys: CoreJsonWebKeySet) -> Self {
        Self {
            jwks_uri,
            inner: RwLock::new(JwksCacheInner { keys, last_refresh: None }),
        }
    }
    ...
    async fn refresh(&self, http: &reqwest::Client) -> Result<CoreJsonWebKeySet> {
        ...
        // The very first refresh after discovery is always allowed (last_refresh == None).
        {
            ...
            if inner.last_refresh.is_some_and(|at| at.elapsed() < JWKS_MIN_REFRESH_INTERVAL) {
...
impl IssuerRegistry {
    fn new(primary: Arc<IssuerHandle>) -> Self {
        Self { primary, extra: Mutex::new(HashMap::new()) }
    }
    ...
    pub(crate) fn set_workspace_issuers(&self, rows: Vec<(String, String, String)>) {
        ...
        for (issuer, client_id, client_secret) in rows {
            ...
            extra.insert(
                issuer.clone(),
                keep.unwrap_or_else(|| Arc::new(IssuerHandle::new(&issuer, &client_id, &client_secret))),
            );
        }
    }
}
...
pub(crate) fn issuer_for_domain<'a>(
    ...
    for cfg in configs {
        let Some(issuer) = cfg.get("issuer").and_then(Value::as_str) else { continue };
...
impl AuthCtx {
    ...
    pub(crate) async fn connect_issuer(
        ...
        let connected =
            Arc::new(ConnectedIssuer { oidc, issuer_url, jwks, client_id, client_secret });
    ...
    pub async fn reload_workspace_issuers(&self) -> Result<()> {
        ...
        let creds = rows.iter().filter_map(|(_, cfg)| sso_credentials(cfg)).collect();
    ...
    pub async fn authenticate(&self, jar: &CookieJar, headers: &axum::http::HeaderMap) -> Option<Principal> {
        if let Some(value) = headers.get(axum::http::header::AUTHORIZATION) {
            let raw = value.to_str().ok()?.strip_prefix("Bearer ")?;
            let info = self.persistence.lookup_api_token(&hash_token(raw)).await.ok()??;
...
pub async fn login(
    ...
    let handle = match params.get("issuer") {
        ...
        Some(iss) => match auth.issuers.lookup(iss) {
            Some(h) => h,
            None => {
                return (StatusCode::NOT_FOUND, "that issuer is not registered with this server")
                    .into_response()
            }
        },
    };
    let issuer = match auth.connect_issuer(&handle).await {
        ...
        Err(e) => {
            ...
            return (StatusCode::BAD_GATEWAY, "the identity provider is unreachable")
                .into_response();
        }
    };
...
pub async fn callback(
    ...
    if let Some(err) = params.error {
        ...
        return (StatusCode::BAD_REQUEST, format!("issuer error: {err} {desc}")).into_response();
    }
    ...
    let Some(pending) = auth.pending.lock().unwrap().remove(&csrf_state) else {
        return (jar, (StatusCode::BAD_REQUEST, "unknown or expired login attempt"))
            .into_response();
    };
...
async fn finish_login(
    ...
    let id_token = token_response.id_token().ok_or_else(|| anyhow!("issuer returned no id_token"))?;
...
async fn ensure_sso_memberships(auth: &AuthCtx, iss: &str, user_id: Uuid) {
    ...
    for (workspace_id, cfg) in configs {
        ...
        match auth.persistence.workspace_role(workspace_id, user_id).await {
            ...
            Ok(None) => {
                if let Err(e) = auth.persistence.add_membership(workspace_id, user_id, "member").await {
...
pub async fn me(
    ...
    let Some(auth) = state.auth.as_ref() else {
        return Json(MeResponse { mode: "open", user: None }).into_response();
    };
...
pub async fn cli_auth_config(State(state): State<AppState>) -> Response {
    match state.auth.as_ref() {
        None => Json(CliAuthConfig { mode: "open", issuer: None, cli_client_id: None })
            .into_response(),
...
pub async fn cli_login(
    ...
    let result: Result<CliLoginResponse> = async {
        ...
        let issuer = auth.connect_issuer(&handle).await.context("issuer discovery")?;
        ...
        let claims = issuer
            .validate_id_token(
                ...
                &id_token,
                move |keys| {
                    CoreIdTokenVerifier::new_public_client(cli_client_id.clone(), issuer_url.clone(), keys)
                },
                |_: Option<&Nonce>| Ok(()),
            )
            .await
        ...
        auth.persistence
            .insert_api_token(&hash_token(&secret), agent_id, Some(owner_id), &["read", "write"], None)
            .await?;
        ...
        audit::record(
            ...
            AuditEvent::new("agent_token_minted")
                .workspace(None)
                .actor(Some(owner_id))
                .detail(json!({ "agent_id": agent_id, "label": label, "scopes": ["read", "write"] })),
        );
        ...
        Ok(CliLoginResponse { token: secret, agent, owner_email: email.map(String::from) })
    }
    .await;
...
pub async fn create_share(
    ...
    let Some(role) = Role::parse(&req.role) else {
        return (StatusCode::BAD_REQUEST, "role must be viewer|commenter|editor").into_response();
    };
    ...
    let result: Result<ShareResponse> = async {
        ...
        if auth.persistence.primary_workspace_of(principal.role_user).await?.is_none() {
...
pub async fn resolve_access(
    ...
    let doc = match state.persistence.as_ref() {
        Some(p) => p.find_document(slug).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?,
        None => None,
    };
    ...
    let Some(auth) = state.auth.as_ref() else {
        ...
        return Ok(Access { user_id: None, role: Role::Editor, author_is_agent: false });
    };
...
mod tests {
    ...
    fn email_domain_extraction() {
        ...
        assert_eq!(domain_of("  padded@corp.example  "), Some("corp.example".into()));
    ...
    fn domain_to_issuer_lookup() {
        ...
        assert_eq!(issuer_for_domain(configs.iter().copied(), "unknown.example"), None);
    ...
    fn issuer_registry_picks_by_iss() {
        let primary = Arc::new(IssuerHandle::new("http://primary.example/dex", "muesli", "s1"));
        ...
        assert_eq!(reg.lookup("http://primary.example/dex").unwrap().client_id, "muesli");
        assert_eq!(reg.lookup("http://primary.example/dex/").unwrap().client_id, "muesli");
        // workspace issuer resolves both ways too
        assert_eq!(reg.lookup("http://corp.example/dex").unwrap().client_id, "corp-client");
        assert_eq!(reg.lookup("http://corp.example/dex/").unwrap().client_id, "corp-client");
    ...
    fn workspace_issuer_reload_keeps_unchanged_clients() {
        ...
        reg.set_workspace_issuers(vec![("http://corp.example".into(), "c".into(), "s2".into())]);
        let first = reg.lookup("http://corp.example").unwrap();
        // same credentials → same handle (cached discovery survives a reload)
        reg.set_workspace_issuers(vec![("http://corp.example".into(), "c".into(), "s2".into())]);
        assert!(Arc::ptr_eq(&first, &reg.lookup("http://corp.example").unwrap()));
        // changed secret → a fresh handle (stale client must not be reused)
        reg.set_workspace_issuers(vec![("http://corp.example".into(), "c".into(), "s3".into())]);
        assert!(!Arc::ptr_eq(&first, &reg.lookup("http://corp.example").unwrap()));
    }
    ...
    fn unverified_iss_reads_the_payload_only() {
        ...
        assert_eq!(unverified_iss(&jwt), Some("http://localhost:5558/dex".into()));
    ...
    async fn validate_refetches_jwks_on_unknown_kid() {
        ...
        assert_eq!(hits.load(std::sync::atomic::Ordering::SeqCst), 1, "exactly one refetch");
    ...
    async fn refresh_is_rate_limited_against_fetch_storms() {
        ...
        assert_eq!(hits.load(std::sync::atomic::Ordering::SeqCst), 1, "rate-limited to one fetch");
    }
    ...
    async fn non_key_failures_do_not_refetch() {
        ...
        let (url, hits, _srv) =
            spawn_jwks_server(serde_json::to_string(&CoreJsonWebKeySet::new(vec![])).unwrap()).await;
        let issuer = ConnectedIssuer {
            oidc: dummy_oidc(),
            issuer_url: IssuerUrl::new(TEST_ISSUER.to_string()).unwrap(),
            jwks: JwksCache::new(
        ...
        assert_eq!(hits.load(std::sync::atomic::Ordering::SeqCst), 0, "no network call on non-kid failure");
    }
    ...
    fn sso_credentials_require_all_three_fields() {
        ...
        for missing in ["issuer", "client_id", "client_secret"] {
            ...
            assert_eq!(sso_credentials(&cfg), None, "missing {missing} must not register");
        }
