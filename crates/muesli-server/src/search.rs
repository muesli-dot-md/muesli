...
//! matches). Ranking tiers: title prefix > title substring > content FTS (plainto_tsquery
//! + ts_rank) > content ILIKE (the short/partial-token fallback) — the SQL computes the
//! tier, this module turns rows into the response shape and builds snippets.
...
pub async fn search(
    ...
    let limit = params
        ...
        .clamp(1, 100);
    let rows = match p
        .search_documents(user, doc_restriction, ws_restriction, q, &escape_like(q), limit)
        .await
    {
        Ok(r) => r,
...
pub fn source_of(kind: Option<&str>, config: Option<&Value>) -> (String, String) {
    ...
    let label = match kind {
        "s3" => {
            ...
            if bucket.is_empty() { "S3".into() } else { bucket }
        }
        "github" => {
        ...
        "gdrive" => {
            ...
            if folder.is_empty() { "Google Drive".into() } else { folder }
        }
...
mod tests {
    ...
    fn source_labels_per_kind() {
        assert_eq!(source_of(None, None), ("native".into(), "Muesli Cloud".into()));
        assert_eq!(
            source_of(Some("s3"), Some(&json!({"endpoint": "http://m:9000", "bucket": "muesli-dev"}))),
            ("s3".into(), "muesli-dev".into())
        );
        assert_eq!(
            source_of(Some("github"), Some(&json!({"owner": "acme", "repo": "docs", "branch": "main"}))),
            ("github".into(), "acme/docs".into())
        );
        assert_eq!(
            source_of(Some("gdrive"), Some(&json!({"folder_id": "f1", "folder_name": "Muesli"}))),
            ("gdrive".into(), "Muesli".into())
        );
        // degraded configs still label by kind rather than panicking or leaking
        assert_eq!(source_of(Some("s3"), Some(&json!({}))), ("s3".into(), "S3".into()));
        assert_eq!(source_of(Some("gdrive"), None), ("gdrive".into(), "Google Drive".into()));
    }
    ...
    fn snippet_is_multibyte_safe() {
        let text = format!("{}これは日本語のコンテンツです{}", "あ".repeat(100), "ん".repeat(100));
