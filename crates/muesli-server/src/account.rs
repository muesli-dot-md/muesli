//! Account-level settings endpoints (docs/design/settings.md): the caller's own profile
...
pub(crate) async fn session_ctx(
    ...
    let Principal { is_agent: false, role_user, .. } = principal else {
...
pub(crate) fn onboard_only(req: &UpdateMeReq) -> bool {
    ...
    let UpdateMeReq { display_name, avatar_url, onboarded } = req;
...
pub async fn update_me(
    ...
    if req.display_name.is_none() && req.avatar_url.is_none() && req.onboarded.is_none() {
        return err(StatusCode::BAD_REQUEST, "pass display_name, avatar_url and/or onboarded");
    }
    ...
    if set_display || set_avatar {
        match p
            .update_user_overrides(user_id, set_display, display_name.as_deref(), set_avatar, avatar_url)
            .await
        {
            Ok(true) => {}
            Ok(false) => return err(StatusCode::NOT_FOUND, "no such user"),
...
pub async fn mint_token(
    ...
    let result: anyhow::Result<Response> = async {
        ...
        let (token_id, expires_at) = p
            .insert_api_token(&hash_token(&secret), agent_id, Some(user_id), scopes, expires_in_days)
            .await?;
...
mod tests {
    ...
    fn display_name_normalization() {
        ...
        assert_eq!(normalize_display_name(Some("  Ada  ")), Ok(Some("Ada".into())));
    ...
    fn avatar_data_url_validation() {
        for mime in ["webp", "png", "jpeg"] {
            assert_eq!(validate_avatar_data_url(&format!("data:image/{mime};base64,AAAA")), Ok(()));
        }
        ...
        assert!(validate_avatar_data_url("data:image/svg+xml;base64,AAAA").is_err(), "svg can script");
        assert!(validate_avatar_data_url("data:image/png,AAAA").is_err(), "must be base64");
    ...
    fn scope_presets_only() {
        ...
        assert_eq!(scope_preset(&s(&["read", "write"])), Some(&["read", "write"][..]));
        // order is forgiven; everything else is not
        assert_eq!(scope_preset(&s(&["write", "read"])), Some(&["read", "write"][..]));
        assert_eq!(scope_preset(&s(&[])), None);
        assert_eq!(scope_preset(&s(&["write"])), None, "write-only is not a preset");
        assert_eq!(scope_preset(&s(&["admin"])), None);
        assert_eq!(scope_preset(&s(&["read", "admin"])), None);
        assert_eq!(scope_preset(&s(&["read", "read"])), None, "duplicates rejected");
    ...
    fn onboard_only_is_exactly_the_stamp_body() {
        ...
        assert!(!onboard_only(&parse(r#"{"onboarded": true, "display_name": "Ada"}"#)));
        assert!(!onboard_only(&parse(r#"{"onboarded": true, "avatar_url": null}"#)));
    }
