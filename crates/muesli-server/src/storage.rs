...
pub(crate) fn validate_rel_path(rel_path: &str) -> Result<()> {
    ...
    if rel_path.split('/').any(|seg| seg.is_empty() || seg == "." || seg == "..") {
        return Err(anyhow!("rel_path {rel_path:?} contains an empty, '.' or '..' segment"));
    }
    Ok(())
}
...
pub(crate) async fn read_body_capped(mut res: reqwest::Response, what: &str) -> Result<Vec<u8>> {
    ...
    while let Some(chunk) =
        res.chunk().await.map_err(|e| anyhow!("{what}: body read: {}", e.without_url()))?
    {
...
impl S3Backend {
    ...
    fn object_uri(&self, rel_path: &str) -> String {
        format!("/{}/{}", uri_encode(&self.bucket, true), uri_encode(&self.key(rel_path), false))
    }
}
...
fn s3_conn_creds(config: &Value) -> Result<(String, String)> {
    let id = config.get("access_key_id").and_then(Value::as_str).filter(|s| !s.is_empty());
    let enc = config.get("secret_key_enc").and_then(Value::as_str).filter(|s| !s.is_empty());
    match (id, enc) {
...
impl StorageBackend for S3Backend {
    ...
    async fn write(&self, rel_path: &str, bytes: &[u8]) -> Result<String> {
        validate_rel_path(rel_path)?;
        let res = self
            .send(reqwest::Method::PUT, &self.object_uri(rel_path), "", Some(bytes.to_vec()))
            .await?;
        if !res.status().is_success() {
            let status = res.status();
            let body = res.text().await.unwrap_or_default();
    ...
    async fn list(&self, prefix: &str) -> Result<Vec<(String, String)>> {
        ...
        Ok(parse_list_objects(&xml)
            .into_iter()
            .map(|(key, etag)| (key.strip_prefix(&self.prefix).unwrap_or(&key).to_string(), etag))
            .collect())
    }
    ...
    async fn delete(&self, rel_path: &str) -> Result<()> {
        validate_rel_path(rel_path)?;
        let res = self
            .send(reqwest::Method::DELETE, &self.object_uri(rel_path), "", None)
            .await?;
...
pub(crate) fn parse_list_objects(xml: &str) -> Vec<(String, String)> {
    ...
    while let Some(at) = rest.find("<Contents>") {
        let Some(end) = rest[at..].find("</Contents>") else { break };
        let block = &rest[at..at + end];
        if let Some(key) = tag(block, "Key") {
            let etag = tag(block, "ETag").unwrap_or_default().trim_matches('"').to_string();
            out.push((key, etag));
        }
...
impl S3Backend {
    ...
    pub async fn probe(&self) -> Result<()> {
        ...
        self.write(&rel, payload).await.context("probe write failed")?;
        let read = self
            .read(&rel)
            .await
...
fn github_conn_token(config: &Value) -> Result<String> {
    match config.get("token_enc").and_then(Value::as_str).filter(|s| !s.is_empty()) {
...
impl GithubBackend {
    ...
    async fn send_contents(
        ...
        let res = self.request(method.clone(), &url).json(&body).send().await?;
        let code = res.status();
        if code.is_success() {
...
impl StorageBackend for GithubBackend {
    async fn read(&self, rel_path: &str) -> Result<Option<(Vec<u8>, String)>> {
        ...
        let encoding = v.get("encoding").and_then(Value::as_str).unwrap_or_default();
        if encoding != "base64" {
    ...
    async fn write(&self, rel_path: &str, bytes: &[u8]) -> Result<String> {
        ...
        for attempt in 0..2 {
            ...
            let outcome = match &sha {
                Some(s) => {
                    self.send_contents(reqwest::Method::PUT, &repo_path, bytes, Some(s)).await?
                }
                None => {
                    ...
                    match self.send_contents(reqwest::Method::PUT, &repo_path, bytes, None).await? {
                        Ok(etag) => Ok(etag),
                        Err(_) => {
    ...
    async fn list(&self, prefix: &str) -> Result<Vec<(String, String)>> {
        ...
        let url = format!(
            "{}?ref={}",
            contents_url(&self.api_base, &self.owner, &self.repo, dir.trim_matches('/')),
            uri_encode(&self.branch, true),
        );
        let res = self.request(reqwest::Method::GET, &url).send().await?;
        if res.status() == reqwest::StatusCode::NOT_FOUND {
    ...
    async fn delete(&self, rel_path: &str) -> Result<()> {
        ...
        let res = self.request(reqwest::Method::DELETE, &url).json(&body).send().await?;
        if res.status() == reqwest::StatusCode::NOT_FOUND {
...
fn sanitize_health_error(raw: &str) -> String {
    truncate_chars(&redact_aws_access_keys(&strip_url_queries(raw)), HEALTH_ERROR_MAX_CHARS)
}
...
fn redact_aws_access_keys(s: &str) -> String {
    ...
    while !rest.is_empty() {
        ...
        let key_shaped =
            candidate && rest[4..20].bytes().all(|b| b.is_ascii_uppercase() || b.is_ascii_digit());
        if key_shaped {
...
impl StorageManager {
    ...
    pub async fn materialize(&self, document_id: Uuid) -> Result<Option<String>> {
        ...
        let generation =
            self.pending_materialize.lock().unwrap().get(&document_id).copied();
        ...
        self.persistence.set_content_hash(document_id, &hash).await?;
        ...
        if let Some(retention) = retention {
            if retention == "bounded" {
                match self.persistence.prune_history(document_id).await {
                    Ok((0, 0)) => {}
                    Ok((u, s)) => debug!(%document_id, updates = u, snapshots = s, "pruned history (bounded retention)"),
    ...
    pub async fn relocate(&self, document_id: Uuid) -> Result<()> {
        ...
        let stem = doc.title.as_deref().filter(|t| !t.trim().is_empty()).unwrap_or(&att.slug);
        ...
        let Some(final_rel) =
            self.set_rel_path_with_fallback(document_id, &att.slug, &chain, &new_rel).await?
        else {
            return Ok(());
        };
        ...
        match backend.read(&att.rel_path).await? {
            Some((bytes, _etag)) => {
                ...
                self.persistence.set_content_hash(document_id, &sha256_hex(&bytes)).await?;
            }
            None => {
    ...
    async fn attach_doc_with_fallback(
        ...
        let chain = self.persistence.folder_chain_names(doc.folder_id).await?;
        let stem = doc.title.as_deref().filter(|t| !t.trim().is_empty()).unwrap_or(&doc.slug);
        let rel = rel_path_for_named(&chain, stem);
        match self.persistence.attach_document_storage(doc.id, conn_id, &rel).await {
            Ok(()) => Ok(Some(rel)),
            Err(e) if e.to_string().contains("documents_storage_path") => {
                let slug_rel = rel_path_for(&chain, &doc.slug);
                match self.persistence.attach_document_storage(doc.id, conn_id, &slug_rel).await {
                    Ok(()) => Ok(Some(slug_rel)),
    ...
    pub async fn attach_new_document(&self, document_id: Uuid) -> Result<bool> {
        ...
        let Some(ws_id) = doc.workspace_id else { return Ok(false) };
        let Some(meta) = self.persistence.workspace_meta(ws_id).await? else { return Ok(false) };
        let Some(conn_id) = meta.storage_conn_id else { return Ok(false) };
        let unattached = crate::persistence::UnattachedDoc {
            id: doc.id, slug: doc.slug, folder_id: doc.folder_id, title: doc.title,
        };
        if self.attach_doc_with_fallback(&unattached, conn_id).await?.is_none() {
            return Ok(false);
        }
    ...
    pub async fn bind_workspace(&self, workspace_id: Uuid, conn_id: Uuid) -> Result<usize> {
        self.persistence.activate_workspace_with_storage(workspace_id, conn_id).await?;
        let docs = self.persistence.unattached_documents_in_workspace(workspace_id).await?;
    ...
    async fn poll_one(&self, att: &AttachedDoc) -> Result<()> {
        if self.pending_materialize.lock().unwrap().contains_key(&att.document_id) {
        ...
        self.persistence.set_content_hash(att.document_id, &hash).await?;
        ...
        if att.rel_path.contains('/') {
            ...
            if dirs.iter().all(|d| crate::folders::valid_folder_name(d)) {
                let leaf =
                    self.persistence.ensure_folder_chain(Some(att.workspace_id), &dirs).await?;
                if leaf != att.folder_id {
                    self.persistence.set_document_folder(att.document_id, leaf).await?;
...
async fn debounce_loop(mgr: Arc<StorageManager>, mut rx: mpsc::UnboundedReceiver<Uuid>) {
    ...
    while let Some(doc_id) = rx.recv().await {
        ...
        *mgr.pending_materialize.lock().unwrap().entry(doc_id).or_insert(0) += 1;
        ...
        tokio::spawn(async move {
            loop {
                match tokio::time::timeout(DEBOUNCE, reset.recv()).await {
                    Ok(Some(())) => continue, // more edits — restart the window
                    _ => break,               // quiet (or shutdown): write now
                }
            }
...
async fn poll_loop(mgr: Arc<StorageManager>) {
    ...
    info!(every_secs = secs, "storage poll loop running (out-of-band ingest, ADR 0013)");
    loop {
...
mod tests {
    ...
    static S3_ENV_LOCK: StdMutex<()> = StdMutex::new(());
    ...
    fn sigv4_matches_the_aws_reference_vector() {
        ...
        let sts = string_to_sign("20150830T123600Z", "20150830/us-east-1/iam/aws4_request", &creq);
    ...
    fn conn_health_records_ok_and_error() {
        ...
        reg.record_ok(conn);
        assert!(reg.get(conn).unwrap().healthy, "recovery clears the error flag");
    }
    ...
    fn record_err_redacts_aws_keys_and_truncates() {
        ...
        assert!(!stored.contains("AKIAIOSFODNN7EXAMPLE"), "access key leaked: {stored}");
        assert!(stored.contains("…redacted…"), "redaction marker missing: {stored}");
        assert!(stored.contains("InvalidAccessKeyId"), "non-secret context should survive: {stored}");

        // A temporary-credential ASIA-prefixed key is redacted the same way.
        reg.record_err(conn, &anyhow!("denied for ASIAABCDEFGHIJKLMNOP".to_string()));
        let stored = reg.get(conn).unwrap().last_error.unwrap();
    ...
    fn sanitize_health_error_strips_url_queries() {
        ...
        assert_eq!(sanitize_health_error("plain error, no url"), "plain error, no url");
    ...
    fn s3_credentials_resolution_config_then_env() {
        ...
        let _sk_guard =
            crate::secrets::SECRET_KEY_ENV_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    ...
    async fn minio_probe_and_round_trip() {
        ...
        backend.probe().await.expect("probe cycle (write/read/delete)");
    ...
    fn github_contents_url_building() {
        assert_eq!(
            contents_url("https://api.github.com", "octo", "notes", "docs/My Notes.md"),
        ...
        assert_eq!(
            contents_url("http://localhost:3300/api/v1/", "muesli", "muesli-e2e", "a.md"),
    ...
    fn github_commit_message_choice() {
        assert_eq!(commit_message(false, "notes/a.md"), "muesli: create notes/a.md");
        assert_eq!(commit_message(true, "notes/a.md"), "muesli: update notes/a.md");
    }

    #[test]
    fn rel_path_from_folder_chain() {
        assert_eq!(rel_path_for(&[], "notes"), "notes.md");
        assert_eq!(rel_path_for(&["Projects".into()], "notes"), "Projects/notes.md");
        assert_eq!(
    ...
    fn rel_path_for_named_tracks_the_title_case_preserving() {
        ...
        assert_eq!(rel_path_for_named(&[], "my-doc"), rel_path_for(&[], "my-doc"));
        assert_eq!(
    ...
    fn iam_policy_is_bucket_and_prefix_scoped() {
        ...
        assert_eq!(statements[0]["Action"], serde_json::json!(["s3:ListBucket"]));
        assert_eq!(statements[0]["Resource"], serde_json::json!("arn:aws:s3:::my-bucket"));
        assert_eq!(
        ...
        assert_eq!(statements[1]["Resource"], serde_json::json!("arn:aws:s3:::my-bucket/*"));
    }
        ...
        let app = axum::Router::new().route(
            ...
            axum::routing::get(move || {
                ...
                async move {
                    ...
                    let sha = if puts.load(Ordering::SeqCst) == 0 { "stale" } else { "fresh" };
                    axum::Json(serde_json::json!({
...
<Contents><Key>notes/b &amp; c.md</Key><ETag>"def456"</ETag></Contents>
    ...
    fn memory_store() -> &'static StdMutex<HashMap<String, HashMap<String, Vec<u8>>>> {
        static STORE: std::sync::OnceLock<StdMutex<HashMap<String, HashMap<String, Vec<u8>>>>> =
    ...
    impl MemoryBackend {
        pub(crate) fn from_conn(config: &Value) -> Self {
            MemoryBackend {
                store: config.get("store").and_then(|v| v.as_str()).unwrap_or("default").to_string(),
            }
        }
    }
    ...
    impl StorageBackend for MemoryBackend {
        ...
        async fn write(&self, rel_path: &str, bytes: &[u8]) -> Result<String> {
            ...
            g.entry(self.store.clone()).or_default().insert(rel_path.to_string(), bytes.to_vec());
            Ok(sha256_hex(bytes))
        }
        async fn list(&self, prefix: &str) -> Result<Vec<(String, String)>> {
            let g = memory_store().lock().unwrap();
    ...
    fn memory_conn() -> (Value, MemoryBackend) {
        ...
        (serde_json::json!({ "store": store }), MemoryBackend { store })
    }
    ...
    async fn test_manager() -> Option<Arc<StorageManager>> {
        ...
        let p = Arc::new(Persistence::connect(&url).await.expect("connect TEST_DATABASE_URL"));
        Some(StorageManager::spawn(p, Default::default(), None, Default::default()))
    }
    ...
    async fn test_db() -> Option<Persistence> {
        let url = std::env::var("TEST_DATABASE_URL").ok()?;
        Some(Persistence::connect(&url).await.expect("connect TEST_DATABASE_URL"))
    }
    ...
    async fn active_workspace(p: &Persistence, name: &str, owner: Uuid) -> Uuid {
        ...
        let conn = p.create_storage_connection(ws, "memory", &serde_json::json!({})).await.unwrap();
    ...
    async fn seed_attached(
        ...
        let conn = p
            ...
            .await
            .unwrap();
        p.attach_document_storage(doc.id, conn, rel_path).await.unwrap();
        (doc.id, conn)
    }
    ...
    async fn title_change_moves_rel_path_to_the_title_based_path() {
        ...
        mgr.persistence.set_document_title(doc_id, Some("My Great Title")).await.unwrap();
        let doc = mgr.persistence.find_document(&slug).await.unwrap().unwrap();
        let chain = mgr.persistence.folder_chain_names(doc.folder_id).await.unwrap();
        ...
        let att = mgr.persistence.document_attachment(doc_id).await.unwrap().unwrap();
        assert_eq!(att.rel_path, "My Great Title.md");
        assert!(att.content_hash.is_none(), "content_hash reset so the next materialize writes the new path");
    }

    #[tokio::test]
    async fn empty_title_falls_back_to_slug_based_path() {
        let Some(mgr) = test_manager().await else {
            eprintln!("skipping: set TEST_DATABASE_URL to run empty_title_falls_back_to_slug_based_path");
            return;
        };
        let slug = format!("doc-{}", Uuid::now_v7());
        ...
        mgr.persistence.set_document_title(doc_id, None).await.unwrap();
        let doc = mgr.persistence.find_document(&slug).await.unwrap().unwrap();
        let stem = doc.title.as_deref().filter(|t| !t.trim().is_empty()).unwrap_or(&slug);
        let chain = mgr.persistence.folder_chain_names(doc.folder_id).await.unwrap();
        let new_rel = rel_path_for_named(&chain, stem);
    ...
    async fn title_collision_falls_back_to_slug_and_does_not_error() {
        ...
        let da = p.ensure_document_owned(&slug_a, owner, owner).await.unwrap();
        let conn = p
            .create_storage_connection(da.workspace_id.unwrap(), "s3", &fake_s3_config())
            .await
            .unwrap();
        p.attach_document_storage(da.id, conn, "Shared.md").await.unwrap();
        let db = p.ensure_document_owned(&slug_b, owner, owner).await.unwrap();
        p.attach_document_storage(db.id, conn, &format!("{slug_b}.md")).await.unwrap();
        ...
        assert_eq!(landed, Some(format!("{slug_b}.md")), "B fell back to its slug path");
    ...
    async fn unattached_title_change_is_a_clean_no_op() {
        let Some(mgr) = test_manager().await else {
            eprintln!("skipping: set TEST_DATABASE_URL to run unattached_title_change_is_a_clean_no_op");
            return;
        };
        let p = &mgr.persistence;
        let owner = p.create_agent_user("owner").await.unwrap();
        ...
        p.set_document_title(doc.id, Some("Anything")).await.unwrap();
        ...
        mgr.relocate(doc.id).await.expect("relocate on an unattached doc is Ok(())");
    ...
    async fn relocate_moves_bytes_old_to_new_on_title_change() {
        ...
        let doc = p.ensure_document_owned(&slug, owner, owner).await.unwrap();
        let conn =
            p.create_storage_connection(doc.workspace_id.unwrap(), "memory", &config).await.unwrap();
        p.attach_document_storage(doc.id, conn, &format!("{slug}.md")).await.unwrap();
        // Materialize the canonical file at the slug path.
        backend.write(&format!("{slug}.md"), b"# hello\n").await.unwrap();

        // Rename → relocate moves the bytes to "Renamed Doc.md".
        p.set_document_title(doc.id, Some("Renamed Doc")).await.unwrap();
        ...
        assert_eq!(moved.map(|(b, _)| b), Some(b"# hello\n".to_vec()), "bytes live at the new key");
    ...
    async fn relocate_collision_to_current_path_preserves_bytes() {
        ...
        let da = p.ensure_document_owned(&slug_a, owner, owner).await.unwrap();
        let conn =
            p.create_storage_connection(da.workspace_id.unwrap(), "memory", &config).await.unwrap();
        // A occupies "Shared.md"; B is materialized at its slug path.
        p.attach_document_storage(da.id, conn, "Shared.md").await.unwrap();
        backend.write("Shared.md", b"# A\n").await.unwrap();
        let db = p.ensure_document_owned(&slug_b, owner, owner).await.unwrap();
        p.attach_document_storage(db.id, conn, &format!("{slug_b}.md")).await.unwrap();
        backend.write(&format!("{slug_b}.md"), b"# B\n").await.unwrap();
        ...
        p.set_document_title(db.id, Some("Shared")).await.unwrap();
        mgr.relocate(db.id).await.expect("collision relocate must not error");

        // B's canonical file is intact at its slug path (NOT destroyed).
        let b_bytes = backend.read(&format!("{slug_b}.md")).await.unwrap();
        assert_eq!(b_bytes.map(|(b, _)| b), Some(b"# B\n".to_vec()), "B's bytes survived the collision");
        // A is untouched.
        let a_bytes = backend.read("Shared.md").await.unwrap();
        assert_eq!(a_bytes.map(|(b, _)| b), Some(b"# A\n".to_vec()), "A's bytes untouched");
    ...
    async fn bind_workspace_attaches_and_activates() {
        let Some(p) = test_db().await else {
            eprintln!("skipping: set TEST_DATABASE_URL to run bind_workspace_attaches_and_activates");
            return;
        };
        let p = Arc::new(p);
        ...
        p.create_document_in_workspace(&slug_a, ws, None, Some("Doc A"), owner).await.unwrap();
        p.create_document_in_workspace(&slug_b, ws, None, None, owner).await.unwrap();
        ...
        let mgr = StorageManager::spawn(
            p.clone(), Default::default(), None, crate::events::WorkspaceEvents::default(),
        );
        let attached = mgr.bind_workspace(ws, conn).await.unwrap();
        assert_eq!(attached, 2);
        assert_eq!(p.workspace_meta(ws).await.unwrap().unwrap().status, "active");
        assert_eq!(p.unattached_documents_in_workspace(ws).await.unwrap().len(), 0);
        // Titles drive the filename; missing title falls back to the slug.
        let atts = p.attached_documents().await.unwrap();
        assert!(atts.iter().any(|a| a.slug == slug_a && a.rel_path == "Doc A.md"));
        assert!(atts.iter().any(|a| a.slug == slug_b && a.rel_path == format!("{slug_b}.md")));
    }
    ...
    async fn new_documents_auto_attach_in_bound_workspaces() {
        ...
        let doc = p.create_document_in_workspace(&slug, ws, None, Some("Auto Doc"), owner)
            .await.unwrap();
        assert!(mgr.attach_new_document(doc.id).await.unwrap(), "bound workspace → attached");
        assert!(p.attached_documents().await.unwrap().iter()
        ...
        let doc2 = p.create_document_in_workspace(&slug2, ws2, None, None, owner).await.unwrap();
