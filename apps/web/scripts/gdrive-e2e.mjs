...
let stateToken;
{
  ...
  if (cb.status !== 302 && cb.status !== 303) fail(`callback → ${cb.status}: ${await cb.text()}`);
  const finalLoc = cb.headers.get("location");
  if (finalLoc !== `${WEB_ORIGIN}/?storage=connected#~settings/connections`)
    fail(
      `callback redirected to ${finalLoc}, expected ${WEB_ORIGIN}/?storage=connected#~settings/connections`,
    );
...
let connId, folderId;
{
  ...
  ok(
    `gdrive connection ${connId}: Muesli folder ${folderId}, refresh_token stored in db, redacted in api`,
  );
}
...
// --- 6. out-of-band update of the mock file → polled ingest -------------------------------
{
  ...
  const modified = f.content
    .toString()
    .replace("hello from dev.", "hello from dev, edited OUT-OF-BAND.");
  ...
  if (devClient.text.toString() !== modified)
    fail(
      `room text diverges from Drive after ingest: ${JSON.stringify(devClient.text.toString())}`,
    );
...
// --- 7. token expiry: 401 once → transparent refresh + retry ------------------------------
{
  ...
  if (mock.refreshGrants <= grantsBefore) fail("no refresh_token grant was issued after the 401");
  if (mock.expireOnce) fail("the mock's 401 was never consumed");
  ok(
    `forced 401 → server refreshed (grants ${grantsBefore}→${mock.refreshGrants}) and retried transparently`,
  );
}
