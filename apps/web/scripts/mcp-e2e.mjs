// MCP façade e2e (ADR 0008/0007; internal/design/mcp-and-agent-auth.md). Starts its own
...
// --- 1. protocol plumbing -----------------------------------------------------
{
  ...
  if (init.result?.protocolVersion !== "2025-03-26")
    fail(`bad protocolVersion: ${JSON.stringify(init.result)}`);
...
const BASE = "# MCP Doc\n\nhello brave world, this stays.\n";
{
  ...
  if (!agentEntries.every((e) => e.author?.kind === "agent"))
    fail("agent entry not attributed to an agent identity");
  if (!agentEntries.some((e) => e.change_set_id === edited.change_set_id))
    fail("edit's change set missing from history");
...
// --- 4. ambiguous anchors and stale base_seq are clear errors ---------------------
{
  ...
  if (!/ambiguous/.test(amb.text) || !/byte \d+/.test(amb.text))
    fail(`ambiguity error lacks offsets: ${amb.text}`);
  ...
  if (!missing.isError || !/not found/.test(missing.text))
    fail(`missing-anchor error: ${missing.text}`);
...
let threadId;
{
  ...
  if (!t || t.comments.length !== 2)
    fail(`expected the thread with 2 comments: ${JSON.stringify(list)}`);
...
let suggestionId, downgradeChangeSet;
{
  ...
  if (!pending.suggestions?.some((s) => s.id === suggestionId))
    fail("pending suggestion not listed");
...
serverProc = await startServer({
  MUESLI_AGENT_GATED_ACTIONS: "true",
  MUESLI_AGENT_DIRECT: "always",
});
...
// --- 8. change-set accept + pinned-direct presence under a co-present human -----------
{
  ...
  if (sugg.applied_mode !== "suggest" || sugg.suggestion_ids.length !== 2)
    fail("2-edit suggest failed");
  const set = await toolOk("accept_change_set", { change_set_id: sugg.change_set_id });
  if (set.accepted?.length !== 2 || set.conflicts?.length !== 0)
    fail(`change set accept: ${JSON.stringify(set)}`);
  const read = await toolOk("read_document", { slug });
  ...
  if (direct.applied_mode !== "direct")
    fail(`ALWAYS policy did not pin direct: ${JSON.stringify(direct)}`);
  let sawAgent = false;
  for (let i = 0; i < 30 && !sawAgent; i++) {
    sawAgent = [...human.provider.awareness.getStates().values()].some(
      (s) => s.user?.kind === "agent",
    );
    await sleep(100);
  }
  ...
  if (!human.text.toString().includes("WORLD"))
    fail("direct edit did not reach the live human client");
...
// --- 9. the stdio proxy: `muesli mcp` -------------------------------------------------
{
  ...
  send({
    ...
    id: 103,
    method: "tools/call",
    params: { name: "read_document", arguments: { slug } },
  });
  const read = await waitFor(103, "read_document");
  const payload = JSON.parse(read.result?.content?.[0]?.text ?? "{}");
  if (!payload.markdown?.includes("# MCP DOC"))
    fail(`stdio read_document: ${JSON.stringify(read)}`);

  await sleep(300); // give a stray notification response a chance to (wrongly) appear
  if (responses.length !== 3)
    fail(`stdio proxy wrote ${responses.length} lines, expected 3 (notification must be silent)`);
  child.kill();
