// `muesli sync` folder-daemon e2e (Phase 5, ADR 0014; internal/design/local-agent-cli.md).
...
const sql = (home, query) =>
  execFileSync("sqlite3", [indexDb(home), query], { encoding: "utf8" }).trim();
...
await until(
  "summary: 2 files linked, skip dirs excluded",
...
await until("room #notes carries notes.md content", () =>
  notes.text.toString().includes("Hello notes"),
);
await until("room #sub-deep carries sub/deep.md content", () =>
  deep.text.toString().includes("Deep content"),
);
...
if (readFileSync(join(tree, "notes.md"), "utf8").includes("WEB-EDIT-DEEP"))
  fail("edit leaked into notes.md");
...
await until("external append ingested into #notes", () =>
  notes.text.toString().includes("DISK-EDIT-NOTES"),
);
...
await until(
  "new file auto-linked (stdout)",
  () => cliOut.includes(`new file linked: sub/fresh.md → #${room("sub-fresh")}`),
  8_000,
);
const fresh = client("sub-fresh");
await until("room #sub-fresh carries the new file's content", () =>
  fresh.text.toString().includes("Brand new file"),
);
...
if (notes.text.toString() !== notesTextBefore)
  fail("room #notes changed after the file was deleted");
...
await until(
  "rename source linked + synced",
  () => cliOut.includes(`✓ synced rn-a.md ⇄ #${room("rn-a")}`),
  8_000,
);
renameSync(join(tree, "rn-a.md"), join(tree, "rn-b.md"));
await until(
  "rename re-bind logged",
  () => cliOut.includes(`re-linked (rename): rn-b.md → #${room("rn-a")}`),
  8_000,
);
...
cli.kill("SIGINT");
await until(
  "daemon exited cleanly on SIGINT",
  () => cliExited !== null && cliExited.code === 0,
  15_000,
);
...
// 8. links.json migration (fresh HOME pre-seeded with the old JSON index)
{
  ...
  if (
    sql(tmpHome2, `SELECT doc_id FROM links WHERE file_path = '/tmp/legacy.md'`) !== `legacy-${RUN}`
  )
    fail("legacy entry not imported into index.db");
  if (!existsSync(join(dataDir(tmpHome2), "links.json.migrated")))
    fail("links.json not renamed to .migrated");
