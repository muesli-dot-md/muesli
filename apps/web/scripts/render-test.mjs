...
check(
  "highlight keeps inner markdown",
  /<mark>highlighted <strong>bold<\/strong><\/mark>/.test(html),
);
...
check(
  "frontmatter table",
  /<div class="frontmatter[^"]*"><table class="table table-xs">/.test(html),
);
...
check(
  "code span not katex-ified",
  !/katex[^]*notmath|notmath[^]*?class="katex-error"/.test(
...
check(
  "sanitizer strips script tags",
  !renderMarkdown("hi <script>alert(1)</script>").includes("<script"),
);
