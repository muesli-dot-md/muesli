...
let chainLeaf;
{
  ...
  const chain = list.json.folders.find((f) => f.name === `chain-${run}` && f.parent_id === auto?.id);
  if (!auto || !chain) fail(`folder chain was not auto-created: ${JSON.stringify(list.json.folders)}`);
...
// --- 7. folder trash cascades over the subtree; restore undoes it --------------------------
{
  ...
  if (!trash.json.documents.some((d) => d.slug === slug)) fail("subtree doc missing from the trash");
