...
// --- 8. audit assertions (dev = admin) -------------------------------------------------
{
  ...
  if (ssoCfg.detail?.client_secret !== undefined)
    fail("audit detail leaked the sso client_secret!");
  if (ssoCfg.detail?.issuer !== DEX2_ISSUER)
    fail(`sso audit detail: ${JSON.stringify(ssoCfg.detail)}`);
  ...
  if (shareEntry.detail?.role !== "viewer")
    fail(`share audit detail: ${JSON.stringify(shareEntry.detail)}`);
  ...
  ok(
    "audit contains: sso config, sso login membership, login, document created, share link, suggestion accept",
  );
...
// --- 9. cleanup for reruns: drop corp's membership + the sso config --------------------
{
  ...
  if (detail.members.some((m) => m.user_id === corpMe.id))
    fail("corp still a member after removal");
