...
for (const text of SAMPLES) {
  ...
  if (allGood) ok(`round-trips for ${JSON.stringify(text.slice(0, 24))}${text.length > 24 ? "…" : ""}`);
}

// --- clamping: byte offsets inside a multi-byte char clamp to its start --------
{
  const text = "aöb"; // ö = 2 bytes at byte offset 1..3
  check(byteToUtf16(text, 2) === 1, `inside 'ö' should clamp to its start (got ${byteToUtf16(text, 2)})`);
...
// --- range helpers ----------------------------------------------------------------
{
  ...
  for (const target of ["Grüße", "wörld", "☕", "👩‍👩‍👧‍👦", "emoji"]) {
    ...
    check(
      bytes.start === refByteLen(text.slice(0, from)) && bytes.end === refByteLen(text.slice(0, to)),
    ...
    check(sliced === target, `range slice for ${JSON.stringify(target)} got ${JSON.stringify(sliced)}`);
  }
