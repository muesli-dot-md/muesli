...
const created = await must(
  "/comments",
  { method: "POST", body: { anchor_start: braveStart, anchor_end: braveStart + 5, body: "is this brave enough?" } },
  "create comment",
);
...
// --- 5. deleting the anchored text orphans the comment ----------------------------
{
  ...
  if (threads[0].status !== "orphaned")
    fail(`expected orphaned comment, got ${threads[0].status}`);
...
{
  const res = await must(`/suggestions/changesets/${changeSet}/accept`, { method: "POST" }, "accept set");
...
// --- 10. history: coalesced + attributed -------------------------------------------------
{
  ...
  if (entries.length >= raw) fail(`history not coalesced: ${entries.length} entries for ${raw} updates`);
