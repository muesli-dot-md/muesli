...
check(
  "strike markers hidden",
  hasHide(strikeAt, strikeAt + 2) && hasHide(strikeAt + 6, strikeAt + 8),
);
...
check(
  "link url part hidden",
  hasHide(linkAt + 6, linkAt + "[label](https://example.com/p)".length),
);
...
check(
  "wikilink found with render.ts slug",
  wiki?.slug === "the-slug" && wiki?.target === "The Slug",
);
check(
  "wikilink brackets+target hidden",
  hasHide(wikiAt, wikiAt + 11) && hasHide(wikiAt + 14, wikiAt + 16),
);
...
// cursor inside **bold** reveals ONLY the bold markers
{
  ...
  check(
    "…but the heading stays hidden",
    hid.some((h) => h.from === 0 && h.to === 2),
  );
...
// multiple selections: each reveals its own span
{
  ...
  check(
    "…while em stays hidden",
    hid.some((h) => h.from === emAt),
  );
}
...

{
  ...
  check(
    "toggle [ ] -> change spec",
    change?.from === 3 && change?.to === 4 && change?.insert === "x",
  );
...

{
  ...
  check(
    ...
    images.length === 1 &&
      images[0].url === "https://example.com/img.png" &&
      images[0].alt === "alt text",
  );
}
...

{
  ...
  check(
    "highlight extent + content",
    hls[0].from === at && hls[0].to === at + "==marked text==".length,
  );
  check(
  ...
  check(
    "collectInlineSpans yields a mark span",
    sp.some((s) => s.kind === "mark"),
  );
...

{
  ...
  check(
    "one inline-math span (currency + code + escape skipped)",
    ms.length === 1,
    JSON.stringify(ms.map((m) => m.source)),
  );
  ...
  check(
    "inline math reveal is the whole span",
    mathSpan.reveal.from === at && mathSpan.reveal.to === at + 7,
  );
}
