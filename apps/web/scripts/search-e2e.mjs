...
async function mintToken(email) {
  ...
  if (!body.id_token)
    fail(`dex password grant for ${email} returned no id_token: ${JSON.stringify(body)}`);
...
await sleep(300);
{
  let r = await apiA(`/api/documents/${slugT1}`, {
    method: "PATCH",
    body: { title: `Zq${run} Guide` },
  });
...
let results;
{
  ...
  if (r1.slug !== slugT2)
    fail(`title-substring match should rank second: ${JSON.stringify(results)}`);
  ...
  if (r0.match.snippet !== `Zq${run} Guide`)
    fail(`title snippet should be the title: ${r0.match.snippet}`);
  if (!r2.match.snippet.includes(zq)) fail(`content snippet misses the token: ${r2.match.snippet}`);
  if (/[\n\r\t]/.test(r2.match.snippet))
    fail(`snippet contains raw newlines: ${JSON.stringify(r2.match.snippet)}`);
  ...
  for (const r of results) {
    if (r.is_owner !== true)
      fail(`A owns these docs, is_owner should be true: ${JSON.stringify(r)}`);
    ...
    if (!r.document_id || !r.updated_at || r.folder_id !== null)
      fail(`row shape wrong: ${JSON.stringify(r)}`);
  }
...
// --- 3. multibyte content query + partial-token ILIKE fallback ------------------------
{
  ...
  if (
    phit.match.field !== "content" ||
    !phit.match.snippet.toLowerCase().includes(longWord.slice(2))
  )
...
// --- 4. empty q and missing auth -------------------------------------------------------
{
  ...
  if (r.status !== 200 || r.json.results.length !== 0)
    fail(`empty q should be {results:[]}: ${r.text}`);
  r = await apiA(`/api/search?q=${encodeURIComponent("   ")}`);
  if (r.status !== 200 || r.json.results.length !== 0)
    fail(`whitespace q should be {results:[]}: ${r.text}`);
  r = await apiA(`/api/search`);
  if (r.status !== 200 || r.json.results.length !== 0)
    fail(`missing q should be {results:[]}: ${r.text}`);
...
// --- 6. sharing: workspace membership flips visibility + is_owner -----------------------
{
  ...
  for (const hit of r.json.results) {
    if (hit.is_owner !== false)
      fail(`shared doc must have is_owner:false for B: ${JSON.stringify(hit)}`);
    if (hit.owner?.id !== userIdA)
      fail(`shared doc owner should be A: ${JSON.stringify(hit.owner)}`);
