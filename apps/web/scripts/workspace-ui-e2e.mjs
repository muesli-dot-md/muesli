...
// --- 5. rename (admin) + rename back ----------------------------------------------
{
  ...
  const { workspaces } = await dev.listWorkspaces();
  if (workspaces.find((w) => w.id === wsId)?.name !== tmp) fail("rename not reflected in the list");
...
let friendUserId = null;
{
  ...
  if (res.status === "added") {
    ...
    if (d2.invites.some((i) => i.email === "friend@muesli.md"))
...
// --- 10. documents list + query filter (doc created over ws, as the UI notes) -----------
{
  ...
  if (mine.workspace_id !== wsId) fail(`doc owned by ${mine.workspace_id}, expected ${wsId}`);
  ...
  if (none.documents.some((d) => d.slug === slug)) fail("query filter returned a non-matching doc");
...
// --- 11. cleanup: remove friend so reruns start from a known state -----------------------
{
  ...
  const d = await dev.getWorkspace(wsId);
  if (d.members.some((m) => m.user_id === friendUserId))
    fail("friend still a member after removal");
