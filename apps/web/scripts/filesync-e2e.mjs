...
async function until(desc, fn) {
  ...
  console.error("file text:", JSON.stringify(await readFile(file, "utf8").catch(() => "<unreadable>")));
  process.exit(1);
}

// 1. disk → room: CLI seeds the room from the file.
await until("room contains the file's original content", () => text.toString().includes("Hello from disk"));
...
await until("external disk edit ingested into the room", () => text.toString().includes("DISK-EDIT"));

// Sanity: room and file converge to the same text.
await until("room text == file text", async () => (await readFile(file, "utf8")) === text.toString());
