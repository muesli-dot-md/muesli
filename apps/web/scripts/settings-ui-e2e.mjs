...
// --- 2. patchMe: override set / clear / 400 ---------------------------------------------
{
  ...
  if (set.display_name !== "Ada Override")
    fail(`override not trimmed/applied: ${set.display_name}`);
  ...
  if (withAvatar.display_name !== "Ada Override")
    fail("display_name lost on avatar-only patch (absent ≠ null broken)");
  ...
  try {
    ...
    if (!(e instanceof AccountApiError) || e.status !== 400)
      fail(`expected AccountApiError 400, got ${e}`);
  }
...
// --- 3. token lifecycle ------------------------------------------------------------------
{
  ...
  if (!minted.token.startsWith("mua_"))
    fail(`expected mua_ secret, got ${minted.token.slice(0, 8)}…`);
  if (!minted.expires_at) fail("expires_at missing on a 90-day key");

  const asAgent = await fetch(`${SERVER}/api/me`, {
  ...
  if (!row || row.label !== "settings-ui key")
    fail(`minted key missing from listTokens(): ${JSON.stringify(listed)}`);
  if (JSON.stringify([...row.scopes].sort()) !== '["read","write"]')
    fail(`bad scopes: ${row.scopes}`);
  ...
  const revokedBearer = await fetch(`${SERVER}/api/me/tokens`, {
    headers: { authorization: `Bearer ${minted.token}` },
  });
...
// --- 4. storage connections through workspaceApi ------------------------------------------
{
  ...
  if (empty.google?.configured !== false)
    fail(`expected google.configured:false, got ${JSON.stringify(empty.google)}`);
  ...
  if (gone.connections.some((c) => c.id === created.storage_conn_id))
    fail("connection survived delete");
  ok(
    "listStorageConnections()/createStorageConnection()/deleteStorageConnection(): attach (probed), 502 inline error, disconnect",
  );
}
