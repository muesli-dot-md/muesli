...
// --- 3. limit + empty q -------------------------------------------------------------------
{
  ...
  if (both.results[0].slug !== slugTitle)
    fail(
      `title tier must outrank content tier: ${JSON.stringify(both.results.map((r) => r.slug))}`,
    );
...
// --- 6. invite B → shared-with-me set ---------------------------------------------------------
{
  ...
  if (before.documents.some((d) => d.slug === slugTitle)) fail("B sees A's docs before the invite");

  const inv = await apiA.createInvite(wsA, "friend@muesli.md", "member");
  if (inv.status !== "added")
    fail(`existing user should be added immediately: ${JSON.stringify(inv)}`);
  ...
  if (
    s.results.length !== 1 ||
    s.results[0].is_owner !== false ||
    s.results[0].owner?.id !== userIdA
  )
...
// --- 7. trash removes from shared listing and search ------------------------------------------
{
  ...
  if (list.documents.some((d) => d.slug === slugTitle)) fail("trashed doc still in B's listing");
  const s = await apiB.search(token1);
