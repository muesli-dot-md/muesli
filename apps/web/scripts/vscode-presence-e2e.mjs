...
try {
  ...
  if (
    bSeen &&
    bSeen.cursor.anchor === B_ANCHOR &&
    bSeen.cursor.head === B_HEAD &&
    bSeen.name === "Berry 42"
  ) {
    ok(
      `A decoded B's cursor: name=${JSON.stringify(bSeen.name)} anchor=${bSeen.cursor.anchor} head=${bSeen.cursor.head} (UTF-16 offsets)`,
    );
  } else {
    fail(
      `A did not decode B's cursor {anchor:${B_ANCHOR}, head:${B_HEAD}} (got ${JSON.stringify(bSeen)})`,
    );
  }
  ...
  const aCursorAtB = await waitFor(() => {
    for (const [id, state] of b.provider.awareness.getStates()) {
      ...
      const head = Y.createAbsolutePositionFromRelativePosition(
        Y.createRelativePositionFromJSON(c.head),
        b.doc,
      );
  ...
  if (aCursorAtB && aCursorAtB.anchor === A_ANCHOR && aCursorAtB.head === A_HEAD) {
    ...
    fail(
      `A's cursor did not round-trip to B (expected {anchor:${A_ANCHOR}, head:${A_HEAD}}, got ${JSON.stringify(aCursorAtB)})`,
    );
  }
