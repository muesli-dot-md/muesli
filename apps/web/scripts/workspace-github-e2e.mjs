...
try {
  execFileSync(
    ...
    ["compose", "exec", "-T", "gitea", "gitea", "admin", "user", "create",
      "--admin", "--username", ADMIN.user, "--password", ADMIN.pass,
      "--email", "gitea@muesli.md", "--must-change-password=false"],
...
// --- 2. storage connection kind github (probe: bad repo → 502, good → 200) ----------
{
  const bad = await dev(`/api/workspaces/${devWs}/storage`, {
    method: "POST",
    body: { kind: "github", api_base: API, owner: ADMIN.user, repo: "no-such-repo", branch: BRANCH },
  });
...
// --- 5. out-of-band commit → polled ingest into the live room --------------------------
{
  ...
  if (devClient.text.toString() !== modified)
    fail(`room text diverges from the file after ingest: ${JSON.stringify(devClient.text.toString())}`);
