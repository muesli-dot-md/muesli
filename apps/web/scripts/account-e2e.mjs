// Account settings e2e (internal/design/settings.md): PATCH /api/me profile overrides,
...
async function dexIdToken(email) {
  ...
  if (!body.id_token)
    fail(`dex password grant (${email}) returned no id_token: ${JSON.stringify(body)}`);
...
function userId(email) {
  const out = psql(TEST_DB, `select id from users where kind = 'human' and email = '${email}'`, [
    "-t",
    "-A",
  ]).trim();
...
// --- 2. PATCH /api/me — profile overrides ------------------------------------------------
{
  ...
  const set = await dev("/api/me", {
    method: "PATCH",
    body: { display_name: "  Account E2E Custom  " },
  });
  if (set.status !== 200 || set.json.display_name !== "Account E2E Custom")
    fail(`PATCH display_name → ${set.status} ${set.text}`);
  const me = await dev("/api/me");
  if (me.json.user.display_name !== "Account E2E Custom")
    fail("GET /api/me does not reflect the override");
  ...
  if (av.status !== 200 || av.json.avatar_url !== avatar)
    fail(`PATCH avatar → ${av.status} ${av.text}`);
  ...
  const tooLong = await dev("/api/me", {
    method: "PATCH",
    body: { display_name: "x".repeat(121) },
  });
  ...
  const clear = await dev("/api/me", {
    method: "PATCH",
    body: { display_name: null, avatar_url: null },
  });
...
let mintedId;
{
  ...
  if (JSON.stringify([...cli.scopes].sort()) !== '["read","write"]')
    fail(`cli scopes ${cli.scopes}`);
  ...
  if (foreign.status !== 404)
    fail(`friend revoking dev's key → ${foreign.status} (want 404, hidden)`);
  const ghost = await dev(`/api/me/tokens/00000000-0000-7000-8000-000000000000`, {
    method: "DELETE",
  });
...
// --- 5. storage connections: readiness flag + disconnect -----------------------------------
{
  ...
  const mkConn = () =>
    dev(`/api/workspaces/${ws.id}/storage`, {
      ...
      body: {
        ...
        bucket: "muesli-dev",
        prefix: `account-e2e/${run}`,
      },
    });
  let conn = await mkConn();
  if (conn.status !== 200) fail(`create s3 conn → ${conn.status} ${conn.text}`);
  const del = await dev(`/api/workspaces/${ws.id}/storage/${conn.json.storage_conn_id}`, {
    method: "DELETE",
  });
  if (del.status !== 200 || del.json.deleted !== true)
    fail(`disconnect → ${del.status} ${del.text}`);
  ...
  const share = await dev(`/api/documents/${slug}/share`, {
    method: "POST",
    body: { role: "viewer" },
  });
  if (share.status !== 200) fail(`create doc via share → ${share.status} ${share.text}`);
  const attach = await dev(`/api/documents/${slug}/storage`, {
    method: "POST",
  ...
  if (blocked.status !== 409 || blocked.json.attached_documents !== 1)
    fail(
      `disconnect with attached doc → ${blocked.status} ${blocked.text} (want 409 {attached_documents:1})`,
    );
  ok("disconnect with an attached document → 409 {attached_documents: 1}");

  const notMember = await friend(`/api/workspaces/${ws.id}/storage/${connId}`, {
    method: "DELETE",
  });
