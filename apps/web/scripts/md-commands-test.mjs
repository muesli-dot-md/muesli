...
// bold toggle over a multibyte selection (emoji = 2 UTF-16 units, é = 1)
{
  ...
  check("bold wraps multibyte selection", st.doc.toString() === "café **😀toast** ok", st.doc.toString());
  check(
...
// empty selection on a word wraps the word
{
  ...
  check("word under cursor wrapped in em", st.doc.toString() === "hello *wörld* here", st.doc.toString());
}
...
// heading cycle: normal -> h1 -> h2 -> quote -> normal
{
  ...
  check("h1 -> h2 replaces the marker", st.doc.toString() === "## plain title\nbody", st.doc.toString());
  st = mkState(st.doc.toString(), sel(4));
  ...
  check("quote -> normal strips markers", st.doc.toString() === "plain title\nbody", st.doc.toString());
}
...
// multi-line selection styles every touched line, skipping blanks
{
  ...
  check("multi-line h3 skips blank lines", st.doc.toString() === "### one\n\n### two", st.doc.toString());
}
...
// code block wrap + unwrap
{
  ...
  check("code block wraps the line", st.doc.toString() === "alpha\n```\nbeta\n```\ngamma", st.doc.toString());
...

{
  ...
  check("bullet -> numbered renumbers from 1", st.doc.toString() === "1. one\n2. two\n3. three", st.doc.toString());
  st = mkState(st.doc.toString(), sel(0, st.doc.length));
  st = apply(st, toggleList(st, "ordered"));
...
// task list keeps existing text, strips other list markers
{
  ...
  check("bullet -> task rewrites the prefix", st.doc.toString() === "- [ ] already bullet", st.doc.toString());
...

{
  ...
  check("isProbablyUrl accepts https + www", isProbablyUrl("https://a.b") && isProbablyUrl("www.a.b"));
...
// snippet on an empty line inserts in place (no leading blank)
{
  ...
  check("hr on empty line inserts in place", st.doc.toString() === "above\n---\n\nbelow", JSON.stringify(st.doc.toString()));
}
...
// wikilink wraps the selection / opens empty brackets
{
  ...
  check("wikilink wraps selection", st.doc.toString() === "Visit [[The Page]] now", st.doc.toString());
  st = mkState("x ", sel(2));
  st = apply(st, insertWikilink(st));
  check("empty wikilink leaves cursor between brackets", st.doc.toString() === "x [[]]" && st.selection.main.head === 4);
}
...

{
  ...
  check("outline skips fenced code + frontmatter", !items.some((i) => i.text.includes("not a heading")));
  check("outline strips closing hashes", items[2].text === "Three");
  check("outline offsets point at the heading line", doc.slice(items[1].from, items[1].from + 2) === "##");
}
