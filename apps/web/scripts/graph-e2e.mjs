// Link-graph e2e (ADR 0015; internal/design/wikilinks-and-link-graph.md): drives the EXACT
...
const BINARY = join(repoRoot, "target", "debug", "muesli-server");
...
if (OWN_SERVER) {
  // Own scratch database, recreated every run (same pattern as search-e2e): the shared
  // dev database accumulates docs + broken storage bindings from other runs, which both
  // pollutes the open-mode graph and slows the server with failing storage polls.
  const TEST_DB = "muesli_graph_e2e";
  const psql = (db, sql) =>
    execFileSync(
      "docker",
      [
  ...
  try {
    ...
    fail(`recreating ${TEST_DB} failed (is compose postgres up?): ${e.stderr || e.message}`);
  }
  serverProc = spawn(BINARY, [], {
    // cwd OUTSIDE the repo: dotenvy walks parent dirs, and the repo .env must not
    // leak its OIDC/storage config (or its DATABASE_URL) into this open-mode server.
    cwd: tmpdir(),
    env: {
...
async function waitForGraph(what, predicate) {
  ...
  const g = await ourGraph();
  fail(
    `${what}: timed out; last graph = ${JSON.stringify({
      nodes: [...g.nodes.keys()],
      edges: g.edges,
