<script lang="ts" module>
  ...
  export type InfoTarget =
    { kind: "doc"; doc: DocumentSummary } | { kind: "folder"; folder: FolderSummary };
</script>
...
<script lang="ts">
  ...
  import { WorkspaceApiError, type ShareRole, type WorkspaceApi } from "./workspaceApi";
    ...
    {:else}
      ...
      <div class="flex flex-wrap items-center gap-1">
        <button class="link-hover link" onclick={() => gotoHome()}>
          {workspaceId
            ? (workspaceNames[workspaceId] ?? t("common.shared"))
            : t("home.myDocuments")}
        </button>
