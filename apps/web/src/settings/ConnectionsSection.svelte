...
{#if auth.mode === "open" || degraded === "open"}
  <p class="text-sm text-[var(--text-muted)]" style="text-wrap: pretty;">{t("ws.unavailableOpenMode")}</p>
...
{:else}
  ...
  {#if workspaces.length > 1}
    <div class="mb-4 flex flex-wrap gap-1" role="tablist" aria-label={t("settings.conn.workspaceLabel")}>
      {#each workspaces as w (w.id)}
        <button
          ...
          style={w.id === selectedId ? "background: var(--lift); box-shadow: var(--shadow-lift);" : ""}
