<script lang="ts">
  ...
  // left, the control right-aligned. Rows stack with a hairline divider drawn
  // by the row's top border (the first row in a card has none). When `stacked`
  // is set the control drops to its own full-width line below the label — used
  // by the connect/attach forms and tables that need the full width.
  // Presentational only.
  ...
  let {
    ...
    title?: string;
    description?: string;
    /** Put the control on its own line under the label (wide forms/tables). */
    stacked?: boolean;
    /** Optional leading icon/avatar snippet, vertically centered. */
    leading?: Snippet;
    /** The right-aligned (or, when stacked, below-the-label) control. */
    control?: Snippet;
    /** Free-form body when a row is more than a label+control (e.g. forms). */
    children?: Snippet;
  } = $props();
</script>
