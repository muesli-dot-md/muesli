<script lang="ts">
  ...
  function fail(e: unknown) {
    ...
    else if (e instanceof WorkspaceApiError && e.status === 403) toast(t("ws.notAllowed"), "warning");
    else toast(t("common.errorWithDetail", { detail: e instanceof Error ? e.message : String(e) }), "warning");
  }
...
{:else}
  ...
  <SettingsCard heading={t("ws.members")}>
    {#each detail.members as m (m.user_id)}
      <div class="flex flex-wrap items-center gap-x-3 gap-y-2 border-t border-base-300/60 px-5 py-3 first:border-t-0">
        <span
        ...
        <div class="min-w-0 flex-1">
          <div class="flex flex-wrap items-center gap-1.5">
            <span class="truncate text-sm font-medium">{m.display_name ?? m.email ?? m.user_id}</span>
            ...
            {#each detail.invites as inv (inv.id)}
              <li class="flex items-center gap-2 rounded-field bg-base-200 px-3 py-1.5 text-sm">
                ...
                {#if busyInvite === inv.id}
                  <span class="loading loading-spinner loading-xs"></span>
                {:else}
                  <button class="btn btn-ghost btn-xs text-error" onclick={() => void revokeInvite(inv.id)}>
                    {t("ws.revoke")}
                  </button>
                {/if}
              </li>
    ...
    <SettingsCard heading={t("ws.audit")}>
      <div class="px-5 py-4">
        ...
        {:else}
          <table class="table table-xs">
            ...
            <tbody>
              {#each auditEntries as e (e.id)}
                <tr>
                  <td><span class="badge badge-ghost badge-xs font-mono">{actionLabel(e.action)}</span></td>
                  <td>
                    <span class="flex items-center gap-1">
                      {auditActorName(e)}
                      {#if e.actor?.kind === "agent"}
                        <span class="badge badge-secondary badge-xs" title={t("ws.agentIdentity")}>🤖</span>
                      {/if}
                    </span>
                  </td>
                  <td class="text-right text-xs tabular-nums text-[var(--text-muted)]" title={e.created_at}>
          ...
          {#if !auditDone}
            <button class="btn btn-ghost btn-xs mt-1" disabled={auditBusy} onclick={() => void loadAudit(false)}>
