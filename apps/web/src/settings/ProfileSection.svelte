<script lang="ts">
  ...
  async function saveAvatar() {
    ...
    try {
      ...
      toast(
        e instanceof AccountApiError && e.status === 400 ? e.message : errMsg(e),
        "warning",
      );
    } finally {
      savingAvatar = false;
...
{#if auth.mode === "open"}
  <p class="text-sm text-[var(--text-muted)]" style="text-wrap: pretty;">{t("settings.openMode")}</p>
{:else if !auth.user}
...
{:else}
  ...
  <SettingsCard>
    <SettingRow title={t("settings.profile.email")} description={t("settings.profile.emailManaged")}>
      {#snippet control()}
