<script lang="ts">
  ...
  let { section, embedded = false }: { section: SettingsSection; embedded?: boolean } =
    $props();
...
{#snippet navItem(item: SettingsNavItem, rail: boolean)}
  ...
  <button
    ...
    style={section === item.section ? "background: var(--lift); box-shadow: var(--shadow-lift);" : ""}
      ...
      <nav class="flex flex-col gap-5" aria-label={t("common.settings")}>
        {#each groups as group (group.id)}
          <div class="flex flex-col gap-0.5">
            <p class="mb-1 truncate px-3 text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">
            ...
            {#if group.id === "workspace" && workspaces.length > 1}
              ...
              >
                {#each workspaces as w (w.id)}
                  <option value={w.id}>{w.name}{w.is_personal ? ` · ${t("ws.personalBadge")}` : ""}</option>
                {/each}
              </select>
            {/if}
...
{#if toast}
  <div class="toast toast-end z-50">
    <div class="alert py-2 text-sm shadow {toastKind === 'warning' ? 'alert-warning' : 'alert-info'}">
      {toast}
    </div>
  </div>
{/if}
