...
function buildInline(view: EditorView): DecorationSet {
  ...
  for (const vr of view.visibleRanges) {
    ...
    for (const span of spans) {
      ...
      if (span.contentFrom !== undefined && span.contentTo !== undefined &&
          span.contentFrom < span.contentTo) {
        const mark =
          span.kind === "link" ? linkMark(span) :
          span.kind === "wikilink" ? wikilinkMark(span) :
          markFor[span.kind];
      ...
      if (span.kind === "task") {
        ranges.push(
          Decoration.replace({ widget: new CheckboxWidget(span.checked === true) })
      ...
      if (span.kind === "math") {
        ...
        ranges.push(
          Decoration.replace({ widget: new InlineMathWidget(span.source ?? "") })
            .range(span.from, span.to),
        );
        continue;
      }
