...
const CODE_CONTEXT = /^(FencedCode|CodeBlock|InlineCode|CodeText|CodeInfo|CodeMark|HTMLBlock|CommentBlock)$/;
...
export function collectInlineSpans(
  ...
  spans.push(...wikis);
  const overlapsWiki = (a: number, b: number) =>
    wikis.some((w) => a < w.to && b > w.from);
  ...
  const overlapsMath = (a: number, b: number) =>
    maths.some((mt) => a < mt.to && b > mt.from);
    ...
    enter(node) {
      ...
      if (h) {
        ...
        if (marks.length > 0) {
          const lead = marks[0];
          const afterLead =
            doc.sliceString(lead.to, lead.to + 1) === " " ? lead.to + 1 : lead.to;
      ...
      if (
        (name === "StrongEmphasis" || name === "Emphasis" || name === "Strikethrough" ||
          name === "InlineCode") &&
        ...
        const markName =
          ...
          name === "InlineCode" ? "CodeMark" : "EmphasisMark";
        const hide = marksOf(node.node, markName);
        const inner = hide.length >= 2 ? { from: hide[0].to, to: hide[hide.length - 1].from } : null;
        spans.push({
          kind:
            name === "StrongEmphasis" ? "strong" :
            name === "Emphasis" ? "em" :
            name === "Strikethrough" ? "strike" : "code",
          from: node.from,
          to: node.to,
