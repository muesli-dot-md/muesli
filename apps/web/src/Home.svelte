<script lang="ts">
  ...
  let wizardResume: { workspaceId: string; outcome: "connected" | "error" } | null =
    $state(null);
  ...
  const sortFolders = (fs: FolderSummary[]): FolderSummary[] =>
    [...fs].sort(compareBy(sortKey, sortAsc, (f) => f.name, (f) => f.updated_at));
  ...
  function select(tgt: InfoTarget) {
    selectedRef = tgt.kind === "doc" ? { kind: "doc", id: tgt.doc.document_id } : { kind: "folder", id: tgt.folder.id };
  }
  ...
  function openModal(m: Modal) {
    ...
    if (m.kind === "move") {
      const cur =
        m.target.kind === "doc" ? m.target.doc.folder_id : m.target.folder.parent_id;
      moveDest = cur ?? "";
    }
  ...
  async function submitNewFolder(e: SubmitEvent) {
    ...
    try {
      ...
      modalError =
        e instanceof WorkspaceApiError && e.status === 409
          ? t("modal.folderExists")
          : errMsg(e);
    } finally {
      modalBusy = false;
    }
  ...
  const wizardHost: WizardHost = {
    ...
    startDriveOAuth: (id) => {
      ...
      window.location.href =
        `${httpBase}/api/workspaces/${encodeURIComponent(id)}/storage/google/start?wizard=1`;
    },
    ...
    const context: OnboardingContext =
      workspaces.length > 0
        ? {
            kind: "invited",
            workspaceName:
              (selectedWorkspaceId && workspaceNames[selectedWorkspaceId]) ||
              workspaces[0].name,
          }
        : { kind: "create" };
    onboardingHost = {
...
<div class="flex h-screen flex-col bg-base-200">
  <div class="flex min-h-0 flex-1">
    ...
    <aside class="flex w-64 shrink-0 flex-col gap-1 px-2 pt-2 pb-2">
      ...
      <nav class="mt-1 flex min-h-0 flex-1 flex-col overflow-y-auto" aria-label={t("home.workspaces")}>
        <div class="mb-1 flex items-center justify-between gap-1 px-2 pt-1">
          <p class="truncate text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">
            {t("home.workspaces")}
          </p>
          <button
      ...
      >
        ...
        {:else}
          ...
          <div class="flex items-center gap-2 px-6 pb-3">
            ...
            <button
              class="btn btn-sm gap-1.5"
              onclick={() => openModal({ kind: "newFolder" })}
            >
          ...
          {#if selectedWsUnbound}
            <div class="alert alert-warning mx-4 mt-3 flex items-center justify-between py-2 text-sm">
              ...
              {:else if listing.folders.length === 0 && listing.docs.length === 0}
                <div class="flex flex-1 flex-col items-center justify-center gap-3 pb-16">
                  ...
                  {:else}
                    ...
                    <button class="btn btn-primary btn-sm" onclick={() => openModal({ kind: "newDoc" })}>
                      {t("home.newFile")}
                    </button>
                  {/if}
                </div>
                      ...
                      <tr class="border-base-300/60 text-xs">
                        <th>
                          <button class="cursor-pointer" onclick={() => setSort("name")}>
                            {t("home.colName")} {@render sortArrow("name")}
                          </button>
                        </th>
                        <th class="w-36">
                          <button class="cursor-pointer" onclick={() => setSort("modified")}>
                            {t("home.colModified")} {@render sortArrow("modified")}
                          </button>
                        </th>
                    ...
                    <tbody>
                      {#each listing.folders as f (f.id)}
                        ...
                        >
                          ...
                          <td class="text-sm tabular-nums opacity-70" title={fullDateTime(f.updated_at)}>
                          ...
                          <td>
                            <div class="flex min-w-0 items-center gap-3">
                              ...
                              {#if d.starred}
                                ...
                                />
                              {/if}
                              <span class="hidden shrink-0 font-mono text-[11px] opacity-30 lg:inline"
                                >{d.slug}.md</span
                              >
                            </div>
                          </td>
                          <td class="text-sm tabular-nums opacity-70" title={fullDateTime(d.updated_at)}>
              ...
              {:else}
                ...
                >
                  {#if listing.folders.length > 0}
                    <h2 class="pt-1 pb-2 text-sm font-medium opacity-60">{t("home.foldersHeading")}</h2>
                  ...
                  {#if listing.docs.length > 0}
                    <h2 class="pt-4 pb-2 text-sm font-medium opacity-60">{t("home.documentsHeading")}</h2>
                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                      {#each listing.docs as d (d.document_id)}
                        {@const tgt = { kind: "doc", doc: d } as InfoTarget}
                        <!-- svelte-ignore a11y_click_events_have_key_events, a11y_no_static_element_interactions -->
                        <div
                          class="group cursor-pointer overflow-hidden rounded-xl border border-base-300 shadow-sm transition-shadow hover:shadow {isSelected(tgt)
                            ? 'bg-primary/10'
                        ...
                        >
                          ...
                          <div class="flex items-center gap-2 px-3 pt-2.5">
                            ...
                            <span class="min-w-0 flex-1 truncate text-sm font-medium" title={docName(d)}>
                              {docName(d)}
                            </span>
                            <!-- Starred indicator (migration 0011) -->
                            {#if d.starred}
                          ...
                          <p class="px-3 pt-0.5 pb-2.5 text-xs opacity-50" title={fullDateTime(d.updated_at)}>
                            {driveDate(d.updated_at)}
                          </p>
                        </div>
                      {/each}
  ...
  {#if modal}
    <div class="modal modal-open" role="dialog">
      <div class="modal-box {modal.kind === 'newWorkspace' || modal.kind === 'onboarding' ? 'w-[34rem]' : 'w-[26rem]'} max-w-[92vw]">
        {#if modal.kind === "newDoc"}
          ...
          <form class="flex flex-col gap-2" onsubmit={submitNewDoc}>
            ...
            {#if view === "folder" && crumbs.length > 0}
              <p class="text-xs opacity-50">{t("modal.willBeCreatedIn", { folder: crumbs[crumbs.length - 1].name })}</p>
            {/if}
            {#if modalError}
        ...
        {:else if modal.kind === "move"}
          ...
          <h3 class="mb-1 text-lg font-semibold">{t("modal.moveTitle", { name: itemName(target) })}</h3>
      ...
      <button class="modal-backdrop" aria-label={t("common.close")} onclick={() => (modal = null)}></button>
    </div>
  {/if}
