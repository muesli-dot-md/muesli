...
export function createWorkspaceApi(cfg: WorkspaceApiConfig) {
  ...
  return {
    ...
    getWorkspace: (id: string) => req<WorkspaceDetail>(`/api/workspaces/${encodeURIComponent(id)}`),
    ...
    trashDocument: (slug: string) =>
      req<{ trashed: boolean; document_id: string }>(`/api/documents/${encodeURIComponent(slug)}`, {
        method: "DELETE",
      }),
    ...
    createShareLink: (slug: string, role: ShareRole) =>
      req<{ url: string; role: ShareRole }>(`/api/documents/${encodeURIComponent(slug)}/share`, {
        method: "POST",
        body: { role },
      }),
