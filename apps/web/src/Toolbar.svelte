<script lang="ts">
  ...
  const active = $derived.by(() => {
    ...
    if (!v) {
      return { marks: new Set<InlineMark>(), block: "normal" as BlockStyle, list: null as ListKind | null };
    }
    return {
...
>
  <button class="btn btn-ghost btn-sm btn-square" title={t("toolbar.undo")} disabled={!canEdit} onclick={runUndo}>
    <Undo2 class="h-4 w-4" aria-hidden="true" />
  </button>
  <button class="btn btn-ghost btn-sm btn-square" title={t("toolbar.redo")} disabled={!canEdit} onclick={runRedo}>
  ...
  <div class="dropdown">
    ...
    <ul class="menu dropdown-content z-30 mt-1 w-44 rounded-box border border-base-300 bg-base-100 p-1 shadow">
      {#each styleOptions as style (style)}
        <li>
          <button class={active.block === style ? "menu-active" : ""} onclick={() => runStyle(style)}>
            {styleLabel[style]()}
          </button>
        </li>
      {/each}
    ...
    {#if linkOpen}
      <div class="absolute left-0 top-full z-30 mt-1 w-72 rounded-box border border-base-300 bg-base-100 p-3 shadow-lg">
        ...
        <div class="mt-2 flex justify-end gap-1">
          <button class="btn btn-ghost btn-xs" onclick={() => (linkOpen = false)}>{t("common.cancel")}</button>
  ...
  {#if imageOpen}
    <div class="relative">
      <div class="absolute left-0 top-full z-30 mt-1 w-72 rounded-box border border-base-300 bg-base-100 p-3 shadow-lg">
        ...
        <div class="mt-2 flex justify-end gap-1">
          <button class="btn btn-ghost btn-xs" onclick={() => (imageOpen = false)}>{t("common.cancel")}</button>
  ...
  <button class="btn btn-ghost btn-sm btn-square" title={t("toolbar.downloadMd")} onclick={download}>
    <Download class="h-4 w-4" aria-hidden="true" />
  </button>
  <div class="dropdown">
    <div tabindex="0" role="button" class="btn btn-ghost btn-sm gap-1 font-normal" title={t("toolbar.exportMenu")}>
  ...
  {#if collab.availability !== "volatile"}
    <div class="dropdown dropdown-end">
      <div tabindex="0" role="button" class="btn btn-ghost btn-sm gap-1.5 font-normal" title={t("toolbar.mode")}>
