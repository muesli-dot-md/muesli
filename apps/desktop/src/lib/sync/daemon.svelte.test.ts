...
import type { WorkspaceEventEnvelope } from "$lib/tauri";

// Capture the structure-event handler the store registers so the test can drive it.
let structureHandler: ((evt: WorkspaceEventEnvelope) => void) | null = null;
...
const onStructureEvent = vi.fn(async (h: (evt: WorkspaceEventEnvelope) => void) => {
  structureHandler = h;
  return unlisten;
});
