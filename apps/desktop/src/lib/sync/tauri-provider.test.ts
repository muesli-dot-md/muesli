...
/** Build a SyncStep1 frame (just like the Rust daemon sends when a client attaches). */
function makeSyncStep1Frame(doc: Y.Doc): Uint8Array {
...
describe("createTauriSession", () => {
  it("returns a well-shaped Session with ydoc, ytext, provider, awareness", async () => {
    const session = await createTauriSession({
      path: TEST_PATH,
      identity: { userId: "alice@example.com", name: "Alice", color: "#ff0000", colorLight: "#ff000033", kind: "human" },
    });
...
function makeInMemoryBus(): {
  transportA: { send: (f: Uint8Array) => void; subscribe: (h: (f: Uint8Array) => void) => () => void };
  transportB: { send: (f: Uint8Array) => void; subscribe: (h: (f: Uint8Array) => void) => () => void };
} {
...
describe("makeTauriProvider — two-provider convergence", () => {
  it("doc convergence: Provider A insert reaches Provider B's doc", () => {
    ...
    const identity: Parameters<typeof makeTauriProvider>[0]["identity"] = {
      userId: "alice@example.com", name: "Alice", color: "#f00", colorLight: "#f0033", kind: "human",
    };

    const provA = makeTauriProvider({
      doc: docA, path: "/test.md", identity, ...transportA,
    });
    const provB = makeTauriProvider({
      doc: docB, path: "/test.md", identity: { ...identity, name: "Bob" }, ...transportB,
    });
