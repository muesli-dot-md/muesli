...
describe("collabApi shim", () => {
  ...
  it("POSTs a reply / resolve / reopen with the thread id in the path", async () => {
    ...
    expect(calls[0]).toMatchObject({ method: "POST", path: "/api/documents/my%20doc/comments/t1/replies", body: { body: "yo" } });
    expect(calls[1]).toMatchObject({ method: "POST", path: "/api/documents/my%20doc/comments/t1/resolve" });
    expect(calls[2]).toMatchObject({ method: "POST", path: "/api/documents/my%20doc/comments/t1/reopen" });
  });
  ...
  it("handles suggestions: list, create, accept/reject, changeset accept/reject", async () => {
    ...
    expect(calls[0]).toMatchObject({ method: "GET", path: "/api/documents/my%20doc/suggestions", query: { status: "pending" } });
    expect(calls[1]).toMatchObject({ method: "POST", path: "/api/documents/my%20doc/suggestions", body: { edits: [{ start: 1, end: 2, insert: "x" }], note: "note" } });
    expect(calls[2]).toMatchObject({ method: "POST", path: "/api/documents/my%20doc/suggestions/s1/accept" });
    expect(calls[3]).toMatchObject({ method: "POST", path: "/api/documents/my%20doc/suggestions/s1/reject" });
    expect(calls[4]).toMatchObject({ method: "POST", path: "/api/documents/my%20doc/suggestions/changesets/c1/accept" });
    expect(calls[5]).toMatchObject({ method: "POST", path: "/api/documents/my%20doc/suggestions/changesets/c1/reject" });
  });
});
