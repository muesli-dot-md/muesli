...
<div class="flex flex-col gap-3 p-3">
  ...
  {:else}
    <p class="px-1 text-xs opacity-60">
      ...
      {:else}
        Switch the editor mode to Suggesting to propose edits without changing the document
        directly.
      {/if}
    </p>
    ...
    {#if collab.suggestMode && collab.drafts.length > 0}
      <div class="card border border-warning/50 bg-base-100 shadow-sm">
        <div class="card-body gap-2 p-3">
          ...
          {#each collab.drafts as draft, i (i)}
            ...
            <div class="rounded bg-base-200/60 px-2 py-1 text-xs">
              ...
              {#if !s.range}
                <span
                  ...
                  title="This edit is no longer anchored to text in the document"
                >
                  detached
                </span>
              {/if}
