<script lang="ts">
  // Ported from apps/web/src/SuggestionsPanel.svelte. Deltas:
  //   - explicit `store` prop instead of useDocSession().store
  //   - literal strings instead of t(...)
  //   - X icon from lucide-svelte (desktop) instead of @lucide/svelte
  import { X } from "lucide-svelte";
  import { authorName, relativeTime, type CollabStore, type SuggestionGroup } from "./collabStore.svelte";
  ...
  const draftLabel = (d: { from: number; to: number; insert: string; oldText: string }) =>
    d.oldText && d.insert ? "Replace" : d.insert ? "Insert" : "Delete";
        ...
        <div class="card-body gap-2 p-3">
          ...
          {#each collab.drafts as draft, i}
            <div class="flex items-start justify-between gap-2 text-xs">
              ...
              <button
                class="btn btn-ghost btn-xs shrink-0 active:scale-[0.96]"
                title="Remove"
                onclick={() => collab.removeDraft(i)}><X class="h-3 w-3" aria-hidden="true" /></button
              >
            </div>
          {/each}
          <input class="input input-xs w-full" placeholder="Add a note (optional)" bind:value={note} />
          <button class="btn btn-primary btn-xs active:scale-[0.96]" onclick={submit}>
            {collab.drafts.length > 1 ? "Submit suggestions" : "Submit suggestion"}
          </button>
        </div>
      </div>
    {/if}

    <div class="divider my-0 text-xs opacity-70">Pending</div>

    {#if collab.pendingGroups.length === 0}
      <p class="px-1 py-4 text-center text-sm opacity-50">No pending suggestions.</p>
    {/if}
        ...
        <div class="card-body gap-2 p-3">
          ...
          {#each group.items as s (s.id)}
            <div class="rounded bg-base-200/60 px-2 py-1 text-xs">
              ...
              {:else if !s.op.old_text}
                <span class="opacity-50">(empty edit)</span>
              {/if}
              {#if !s.range}
                <span class="ml-1 badge badge-ghost badge-xs" title="This edit is no longer anchored to text in the document">
                  detached
                </span>
              {/if}
            </div>
          ...
          {#if collab.conflicts[group.changeSetId]}
            <div class="alert alert-warning px-2 py-1 text-xs">
              Couldn't apply: {collab.conflicts[group.changeSetId]}
            </div>
          {/if}
          <div class="card-actions justify-end">
            <button
              class="btn btn-ghost btn-xs active:scale-[0.96]"
              onclick={(e) => {
                e.stopPropagation();
            ...
            >
            <button
              class="btn btn-success btn-xs active:scale-[0.96]"
              onclick={(e) => {
                e.stopPropagation();
