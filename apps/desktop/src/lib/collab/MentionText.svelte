<script lang="ts">
  ...
  // Unknown/removed users (id not in `knownIds`) render muted. Keep in sync with
  // apps/web/src/MentionText.svelte (desktop uses literal strings, no i18n).
...
<span class="whitespace-pre-wrap"
  >{#each segments as seg, i (i)}{#if seg.kind === "text"}{seg.text}{:else if seg.known}<span
