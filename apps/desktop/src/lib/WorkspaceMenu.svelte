<script lang="ts">
  ...
  const activeLetter = $derived((activeName.trim().match(/[\p{L}\p{N}]/u)?.[0] ?? "?").toUpperCase());
      ...
      {#if identity}
        ...
        <div class="flex items-center gap-2 px-1.5 py-1.5">
          {#if identity.avatarUrl}
            <img src={identity.avatarUrl} alt="" class="size-8 shrink-0 rounded-full object-cover" />
          {:else}
            <span
