  ...
  onMount(async () => {
    ...
    if (platform.transcription) {
      invoke("ensure_model")
        ...
        .catch(() => {
          /* model not present yet / offline — recording will cold-load */
        });
    }
    ...
    removeKeymap = installKeymap({
      ...
      openSearch: () => {
        if (workspace.root) showSearch = !showSearch;
      },
...
<div class="flex flex-col h-screen text-base-content" style="background: var(--floor-tint);">
  ...
  <div class="flex items-stretch h-[48px] bg-transparent shrink-0" data-tauri-drag-region>
      ...
      {:else}
        ...
        >
          {#if showSettings}
            ...
            <SettingsPanel initialSection={settingsSection} onclose={closeSettings} />
          {:else if showGraph}
  ...
  {#if showTranscript}
    <footer
      class="shrink-0 flex items-center px-3 py-0.5 border-t border-base-300 bg-transparent text-xs text-base-content/60"
    >
      <Mic size={12} class="mr-1 text-error" />
      <span class="mr-2">Recording…</span>
      <button class="hover:text-base-content transition-colors" onclick={stopTranscription}>
        Close transcript
      </button>
    </footer>
  {/if}
