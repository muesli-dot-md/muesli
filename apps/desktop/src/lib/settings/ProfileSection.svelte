...
{:else}
  ...
  <SettingsCard>
    <SettingRow title={identity?.display_name ?? 'Signed in'} description={identity?.email ?? undefined}>
      {#snippet leading()}
        {#if shownAvatar}
  ...
  <SettingsCard>
    ...
    <SettingRow title="Sign-in" description="You're signed in through your server's identity provider.">
      {#snippet control()}
