...
>
    ...
    {#snippet control()}
      <div class="w-full">
        <div class="mb-1.5 flex items-center justify-end">
          <span class="text-xs text-[var(--text-muted)] tabular-nums">{background.translucency}%</span>
        </div>
        <input
          type="range"
          min="0"
  ...
  <SettingRow title="Reset background" description="Restore translucency, tint, and hue to defaults.">
    {#snippet control()}
      <button class="btn btn-ghost btn-sm" onclick={() => background.reset()}>Reset to default</button>
    {/snippet}
  </SettingRow>
</SettingsCard>
