<script lang="ts">
  ...
  $effect(() => {
    ...
    if (mode === 'read') {
      ...
      const syncing =
        (daemonRunning || settings.syncEnabled) && !!workspaces.identity;
      docCollab.set({
    ...
    const { slug: collabSlug, isRemote: collabRemote } = docContext(
      path,
      workspace.root,
      syncing,
    );
    ...
    if (useTauriSync || useWsSync) {
      ...
      if (useTauriSync) {
        ...
        createTauriSession({ path, identity: presenceIdentity }).then((sess) => {
          if (destroyed) {
            sess.destroy();
            return;
          }
          ...
          view = createEditor({
            ...
            onSelection: () => { editorState.selectionEpoch++; },
          });
      ...
      <div bind:this={wrap} class="relative flex-1 min-h-0">
          ...
          >
            {#if !composerOpen}
              <button
                ...
                title={suggestMode ? 'Suggest an edit to the selection' : 'Comment on the selection'}
              >
              ...
              <div class="w-72 rounded-box border border-base-300 bg-base-100 p-3 shadow-lg">
                ...
                {#if suggestAction !== 'delete'}
                  <textarea
                    ...
                    rows="2"
                    placeholder={suggestAction === 'replace' ? 'Replacement text' : 'Text to insert'}
                ...
                <div class="mt-2 flex justify-end gap-1">
                  <button class="btn btn-ghost btn-xs active:scale-[0.96] transition-transform" onclick={closeComposer}>Cancel</button>
                  <button class="btn btn-primary btn-xs active:scale-[0.96] transition-transform" onclick={addSuggestDraft}>
                    Add to suggestion
                  </button>
                </div>
              </div>
        ...
        {#if suggestMode}
          <div class="absolute right-3 top-2 z-10">
            <span class="badge badge-warning badge-sm" title="You're in suggest mode — edits are queued as suggestions.">
              Suggesting
            </span>
          </div>
        {/if}
