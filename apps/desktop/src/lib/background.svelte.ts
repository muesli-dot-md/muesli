...
function createBackground() {
  ...
  return {
    get translucency() { return translucency; },
    set translucency(v: number) { translucency = clamp(v, 0, 100); apply(); persist(); },
    get hue() { return hue; },
    set hue(v: number) { hue = clamp(v, 0, 360); apply(); persist(); },
    get tint() { return tint; },
    set tint(v: number) { tint = clamp(v, 0, 100); apply(); persist(); },
