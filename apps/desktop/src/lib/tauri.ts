...
export const statPath = (path: string): Promise<PathInfo> =>
  invoke("stat_path", { path });
...
export const hasToken = (server: string): Promise<boolean> =>
  invoke("has_token", { server });
 ...
 */
export async function onEditorFrame(
  handler: (evt: EditorFrameEvent) => void,
): Promise<() => void> {
