<script lang="ts">
  ...
  let filtered = $derived(
    fuzzyFilter(commands.all(), query, (c) => c.title)
  );
      ...
      <ul class="overflow-y-auto max-h-72 py-1">
        ...
        {:else}
          {#each filtered as cmd, i}
            ...
            <li
              class="flex items-center justify-between px-4 py-2 text-sm cursor-pointer transition-colors {i === highlightIdx ? 'bg-base-200 text-base-content' : 'hover:bg-base-200/60 text-base-content/80'}"
