<script lang="ts">
  ...
  async function exportNode(node: WorkspaceNode) {
    try {
      ...
      const base = node.name.toLowerCase().endsWith('.md')
        ? node.name.slice(0, -3)
        : node.name;
  ...
  function menuItems(node: WorkspaceNode): MenuItem[] {
    if (node.isDir) {
      ...
      return [
        ...
        'separator',
        { label: 'Rename', icon: Pencil, action: () => { pendingRename = node.path; } },
        { label: 'Move to…', icon: FolderInput, action: () => { moveTarget = { path: node.path, name: node.name }; } },
        'separator',
        { label: 'Folder information', icon: Info, action: () => { infoTarget = { path: node.path, name: node.name }; } },
        'separator',
        { label: 'Move to trash', icon: Trash2, danger: true, action: () => { pendingDelete = node.path; } },
      ];
    }
    ...
    return [
      ...
      { label: 'File information', icon: Info, action: () => { infoTarget = { path: node.path, name: node.name }; } },
      'separator',
      { label: 'Move to trash', icon: Trash2, danger: true, action: () => { pendingDelete = node.path; } },
    ];
  }
