<script lang="ts">
  ...
  let filtered = $derived(
    fuzzyFilter(allFiles, query, (f) => f.display)
  );
    ...
    >
      ...
      <ul class="overflow-y-auto max-h-72 py-1">
        {#if allFiles.length === 0}
          <li class="px-4 py-2 text-sm text-base-content/50 italic">No workspace open or no notes found</li>
        ...
        {:else}
          {#each filtered as file, i}
            ...
            <li
              class="flex items-center px-4 py-2 text-sm cursor-pointer transition-colors {i === highlightIdx ? 'bg-base-200 text-base-content' : 'hover:bg-base-200/60 text-base-content/80'}"
