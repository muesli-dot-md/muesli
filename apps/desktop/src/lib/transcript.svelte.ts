...
export function createTranscriptStore() {
  ...
  function applyPartial(e: TranscriptEvent) {
    ...
    updated.set(key, { text: e.text, final: false, utteranceId: e.utteranceId, t0: e.t0, source: e.source });
    lines = updated;
  }
  ...
  function applyFinal(e: TranscriptEvent) {
    const key = `${e.source}:${e.utteranceId}`;
    const updated = new Map(lines);
    updated.set(key, { text: e.text, final: true, utteranceId: e.utteranceId, t0: e.t0, source: e.source });
    lines = updated;
  }
