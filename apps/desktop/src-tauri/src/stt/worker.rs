...
mod tests {
    ...
    struct CapturingEmitter {
        partials: Mutex<Vec<TranscriptEvent>>,
        finals: Mutex<Vec<TranscriptEvent>>,
    }
    impl Emitter for CapturingEmitter {
        fn partial(&self, e: &TranscriptEvent) {
            self.partials.lock().unwrap().push(e.clone());
        }
        fn final_(&self, e: &TranscriptEvent) {
            self.finals.lock().unwrap().push(e.clone());
        }
    }
    ...
    fn mid_utterance_finalize_on_disconnect() {
        ...
        let vad = ScriptedVad {
            script: vec![VadEvent::SpeechStarted, VadEvent::Speaking],
        };
        let emitter = Arc::new(CapturingEmitter::default());
        let em2 = emitter.clone();
        for _ in 0..2 {
        ...
        assert_eq!(
            finals.len(),
            1,
            "disconnect should finalize buffered speech"
        );
        assert_eq!(finals[0].utterance_id, 0);
    ...
    fn preroll_is_prepended_on_speech_start() {
        ...
        for _ in 0..6 {
            tx.send(vec![0.1f32; 1600]).unwrap();
        }
        drop(tx);
        run_worker(
            Source::Me,
    ...
    fn short_utterance_is_dropped() {
        ...
        let (tx, rx) = channel::<Vec<f32>>();
        let vad = ScriptedVad {
            script: vec![VadEvent::SpeechStarted, VadEvent::SpeechEnded],
        };
        let emitter = Arc::new(CapturingEmitter::default());
        let em2 = emitter.clone();
        // 1600 + 1600 = 3200 samples of speech < 4000 min → dropped.
        for _ in 0..2 {
            tx.send(vec![0.5f32; 1600]).unwrap();
        }
        drop(tx);
        run_worker(
            Source::Me,
    ...
    fn transcript_event_json_shape() {
        ...
        assert!(
            json.contains("\"source\":\"me\""),
            "source key/value: {json}"
        );
