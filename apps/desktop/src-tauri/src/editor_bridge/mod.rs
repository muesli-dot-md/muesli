...
impl EditorBridges {
    ...
    pub fn register(&self, path: PathBuf, to_daemon: UnboundedSender<Vec<u8>>) {
        self.map
            .lock()
            .unwrap()
            .insert(path, EditorChannels { to_daemon });
    }
...
mod tests {
    ...
    fn register_then_take_roundtrips_sender() {
        ...
        assert!(
            bridges.sender_for(&p).is_some(),
            "registered path resolves to a sender"
        );
        bridges.remove(&p);
        assert!(
            bridges.sender_for(&p).is_none(),
