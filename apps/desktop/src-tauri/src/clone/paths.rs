...
pub fn plan_layout(docs: &[DocInfo], folders: &[FolderInfo]) -> Vec<PlannedFile> {
    ...
    for doc in ordered {
        let dir: PathBuf = dir_segments(doc.folder_id.as_deref(), &by_id)
            .iter()
            .collect();
        let base = {
            let raw = doc
...
mod tests {
    ...
    fn duplicate_names_in_same_dir_get_suffixes_deterministically() {
        ...
        let by_slug: std::collections::HashMap<_, _> = plan
            .iter()
            .map(|p| (p.slug.as_str(), p.rel_path.clone()))
            .collect();
