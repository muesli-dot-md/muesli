...
{#snippet copyBlock(key: string, label: string, text: string)}
  <div class="flex flex-col gap-1">
    ...
    <div class="relative">
      <pre class="max-h-40 overflow-auto rounded-lg bg-base-200 p-3 pr-16 font-mono text-xs"
        style="white-space: pre-wrap; word-break: break-all;">{text}</pre>
      <button class="btn btn-xs absolute right-2 top-2" type="button"
...
{#if !standalone}
  <StepHeader {stepIndex} {totalSteps} title={t("wizard.spConnectTitle")}
    body={t("wizard.spLead")} {t} />
{/if}
...
{:else}
    ...
    <div class="flex flex-col gap-3">
      {#if setup.configured}
        <label class="flex cursor-pointer items-start gap-2">
          <input type="radio" class="radio radio-sm mt-0.5" checked={!sp.ownApp}
      ...
      {#if sp.ownApp}
        ...
        <label class="flex flex-col gap-1">
          ...
          <input class="input input-sm w-full font-mono" bind:value={sp.clientId}
            autocomplete="off" />
        </label>
        <div class="flex gap-4">
          <label class="flex cursor-pointer items-center gap-2 text-sm">
            <input type="radio" class="radio radio-sm" checked={sp.authMethod === "secret"}
              onchange={() => (sp.authMethod = "secret")} />
            {t("wizard.spAuthSecret")}
          </label>
          <label class="flex cursor-pointer items-center gap-2 text-sm">
            <input type="radio" class="radio radio-sm" checked={sp.authMethod === "certificate"}
        ...
        {#if sp.authMethod === "secret"}
          <label class="flex flex-col gap-1">
            ...
            <input class="input input-sm w-full" type="password" bind:value={sp.clientSecret}
              autocomplete="off" />
          </label>
        {:else}
          <label class="flex flex-col gap-1">
            <span class="text-sm">{t("wizard.spCertPem")}</span>
            <textarea class="textarea textarea-sm w-full font-mono" rows="4"
              bind:value={sp.certificatePem} placeholder="-----BEGIN CERTIFICATE-----"></textarea>
          </label>
          <div class="flex flex-col gap-1">
            ...
            {#if privateKeyFileName}
              <p class="text-xs text-base-content/60">
                ...
                <button class="btn btn-ghost btn-xs ml-1" type="button"
                  onclick={clearPrivateKeyFile}>
                  {t("wizard.spKeyPemRemove")}
                </button>
              </p>
            {:else}
              <input id="mws-sp-key-file" class="file-input file-input-sm w-full" type="file"
                accept=".pem,.key" onchange={(e) => void pickPrivateKeyFile(e)} />
            {/if}
          </div>
        {/if}
      {/if}
  ...
  {:else if sp.stage === "grant"}
    <div class="flex flex-col gap-3">
      ...
      {@render copyBlock("consent", t("wizard.spConsentUrl"),
        substitute(setup.consent_url_template, sp, setup))}
      {@render copyBlock("graph", t("wizard.spGrantGraph"),
        substitute(setup.grant_snippet_graph, sp, setup))}
      {@render copyBlock("pnp", t("wizard.spGrantPowershell"),
  ...
  {:else if sp.stage === "site"}
    <div class="flex flex-col gap-3">
      <div class="flex flex-wrap gap-3">
        <label class="flex min-w-40 flex-1 flex-col gap-1">
          ...
          <input class="input input-sm w-full font-mono" bind:value={sp.tenant}
            placeholder="contoso.onmicrosoft.com" autocomplete="off" />
        </label>
        <label class="flex min-w-40 flex-1 flex-col gap-1">
          <span class="text-sm">{t("wizard.spSiteUrl")}</span>
          <input class="input input-sm w-full" type="url" bind:value={sp.siteUrl}
    ...
    <div class="mt-4 flex justify-between">
      <button class="btn btn-ghost" type="button" onclick={back}>{t("wizard.back")}</button>
      <button class="btn btn-primary" type="button"
        disabled={busy || !siteComplete(sp)} onclick={() => void findLibraries()}>
