<script lang="ts">
  ...
  const concepts: { title: WizardKey; body: WizardKey; icon: ConceptIcon }[] = [
    { title: "onboarding.conceptWorkspace", body: "onboarding.conceptWorkspaceBody", icon: "workspace" },
    { title: "onboarding.conceptDocument", body: "onboarding.conceptDocumentBody", icon: "document" },
...
<div class="mws-root flex flex-col">
  {#if view.screen === "welcome"}
    <StepHeader stepIndex={view.screenIndex} totalSteps={view.totalScreens}
      title={t("onboarding.welcomeTitle")} body={t("onboarding.welcomeBody")} {t} />
  ...
  {:else if view.screen === "concepts"}
    <StepHeader stepIndex={view.screenIndex} totalSteps={view.totalScreens}
      title={t("onboarding.conceptsTitle")} {t} />
  ...
  {:else if host.context.kind === "create"}
    <StepHeader stepIndex={view.screenIndex} totalSteps={view.totalScreens}
      title={t("onboarding.createTitle")} body={t("onboarding.createBody")} {t} />
    <div class="mt-2 flex items-center justify-between">
      <div class="flex items-center gap-3">
        <button class="btn btn-ghost" type="button" onclick={back}>{t("wizard.back")}</button>
        {@render skipLink()}
  ...
  {:else if host.context.kind === "invited"}
    ...
    <div class="mb-5 flex flex-col gap-3">
      ...
      >
        {#each Array(view.totalScreens) as _, i}
          <span
  ...
  {:else}
    <StepHeader stepIndex={view.screenIndex} totalSteps={view.totalScreens}
      title={t("onboarding.desktopTitle")} body={t("onboarding.desktopBody")} {t} />
    <div class="flex flex-col gap-2">
      <ChoiceCard title={t("onboarding.localCard")} body={t("onboarding.localCardBody")}
        onclick={() => act("local")} />
      <ChoiceCard title={t("onboarding.serverCard")} body={t("onboarding.serverCardBody")}
        onclick={() => act("server")} />
    </div>
    <div class="mt-4 flex items-center justify-between">
      <button class="btn btn-ghost" type="button" onclick={back}>{t("wizard.back")}</button>
