...
export type CreateStorageBody =
  | { kind: "s3"; endpoint: string; bucket: string; region?: string; prefix?: string;
      access_key_id: string; secret_key: string }
  | { kind: "github"; api_base: string; owner: string; repo: string; branch: string;
      prefix?: string; token?: string }
  | ({ kind: "sharepoint"; tenant: string; site_url: string; site_id: string;
      drive_id: string; drive_name: string; prefix?: string } & SharePointCredentials);
