...
<StepHeader {stepIndex} {totalSteps} title={t("wizard.s3ConnectTitle")}
  body={t("wizard.s3PolicyLead")} {t} />
...
<form class="flex flex-col gap-3" onsubmit={submit}>
  ...
  <label class="flex flex-col gap-1">
    ...
    <input class="input input-sm w-full" type="url" bind:value={endpoint}
      placeholder="https://s3.example.com" required />
  </label>
  <div class="flex flex-wrap gap-3">
    <label class="flex min-w-40 flex-1 flex-col gap-1">
    ...
    <label class="flex min-w-40 flex-1 flex-col gap-1">
      ...
      <input class="input input-sm w-full" type="password" bind:value={secretKey}
