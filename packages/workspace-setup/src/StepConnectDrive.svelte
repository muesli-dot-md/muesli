<script lang="ts">
  ...
  let {
    host, t, workspaceId, ensureWorkspace, onconnected, onback, resumeError = false,
  ...
  const hint = $derived(
    elapsed >= 90 ? t("wizard.driveHint90")
    : elapsed >= 45 ? t("wizard.driveHint45")
    : elapsed >= 15 ? t("wizard.driveHint15")
    : "",
  );
...
<StepHeader {stepIndex} {totalSteps} title={t("wizard.driveConnectTitle")}
  body={t("wizard.driveLead")} {t} />
