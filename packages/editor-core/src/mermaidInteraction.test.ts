...
describe("attachMermaidInteraction", () => {
  ...
  it("applies a translate transform when the diagram is dragged", () => {
    ...
    holder.dispatchEvent(new PointerEvent("pointermove", { clientX: 40, clientY: 25, bubbles: true }));
    expect(layer.style.transform).toContain("translate(30px, 15px)");
    holder.dispatchEvent(new PointerEvent("pointerup", { clientX: 40, clientY: 25, bubbles: true }));
  });

  it("zooms on wheel, scaling the layer", () => {
    const { holder, layer } = build();
    holder.dispatchEvent(
      new WheelEvent("wheel", { deltaY: -100, clientX: 100, clientY: 50, bubbles: true, cancelable: true }),
    );
  ...
  it("does not exceed the max zoom clamp on repeated wheel-in", () => {
    ...
    for (let i = 0; i < 60; i++) {
      holder.dispatchEvent(
        new WheelEvent("wheel", { deltaY: -200, clientX: 100, clientY: 50, bubbles: true, cancelable: true }),
      );
    }
