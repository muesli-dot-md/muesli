...
function enclosingMark(
  ...
  for (
    let n = tree.resolveInner(from, 1) as ReturnType<typeof tree.resolveInner> | null;
    n;
    n = n.parent
  ) {
...
export function activeInlineMarks(state: EditorState): Set<InlineMark> {
  ...
  for (
    let n = tree.resolveInner(head, -1) as ReturnType<typeof tree.resolveInner> | null;
    n;
    n = n.parent
  ) {
...
export function toggleList(state: EditorState, kind: ListKind): TransactionSpec {
  ...
  return newHead === null ? { changes } : { changes, selection: EditorSelection.cursor(newHead) };
}
