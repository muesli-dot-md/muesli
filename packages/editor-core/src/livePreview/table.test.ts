...
//
// Widget-interaction tests for the WYSIWYG TableWidget (sub-project ⑥ B). The
// table renders as a contenteditable grid while the cursor is outside it; cell
// edits and structural mutations dispatch regenerated GFM back into the doc, and
// formula cells render their computed value (not the literal `=…` text).
//
// jsdom can't do real layout, but the DOM structure, dispatch wiring and formula
// rendering are all exercisable. We drive cells/buttons directly and assert on
...
describe("column resize handle is reachable and resizes the whole column", () => {
  ...
  it("attaches a resize handle per column as a child of the table grid (not just <th>)", () => {
    ...
    // One handle per column, spanning the full table height (a direct child of
    // the <table>, NOT nested inside a header cell — that was the bug).
  ...
  it("pointerdown + move on a handle sets a px width on every cell in that column", () => {
    ...
    // Pointer capture is a no-op in jsdom (guarded); the drag still tracks via
    // the handle's own move/up listeners.
    ...
    // Every cell in column 0 (header + body) got an explicit px width.
...
function menuItem(label: string): HTMLButtonElement {
  ...
  if (!hit)
    throw new Error(
      `menu item not found: ${label} (have: ${items.map((b) => b.textContent).join(", ")})`,
    );
  return hit;
}
...
describe("right-click context menu", () => {
  ...
  it("opens on contextmenu and lists insert/delete actions", () => {
    ...
    const labels = Array.from(menu!.querySelectorAll(".cm-live-table-menu-item")).map(
      (b) => b.textContent,
    );
...
describe("cell edit dispatches regenerated markdown", () => {
  ...
  it("editing a body cell rewrites the table source", () => {
    const cells = view.dom.querySelectorAll<HTMLElement>("tbody td");
    const cell = cells[0];
...
describe("context-menu insert/delete dispatches correct markdown", () => {
  ...
  it("insert column left of the clicked column", () => {
    ...
    const doc = view.state.doc.toString();
    expect(doc).toContain("| 1 |  | 2 |"); // new empty col between a and b
  });
...
describe("formula cells", () => {
  ...
  it("renders the computed value, not the literal formula", () => {
    ...
    expect(valueEl!.textContent).toBe("5"); // 2 + 3
    // the literal formula text must not appear in the rendered widget
