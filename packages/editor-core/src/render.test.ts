...
describe("renderMarkdown", () => {
  ...
  it("computes a formula cell in the reading view (not the literal text)", () => {
    ...
    const md = ["| item | qty |", "| --- | --- |", "| a | 2 |", "| b | 3 |", "| total | =SUM(B1:B2) |"].join(
      "\n",
    );
...
describe("finding 32 regression guards", () => {
  ...
  it("PURIFY_CONFIG strips scripts but keeps KaTeX MathML", () => {
    expect(purify.sanitize("<math><semantics><annotation encoding='x'>t</annotation></semantics></math><script>1</script>", PURIFY_CONFIG)).not.toContain(
      "<script",
    );
