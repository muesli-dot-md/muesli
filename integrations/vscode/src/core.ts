...
export function credentialsPath(): string {
  return (
    process.env.MUESLI_CREDENTIALS_PATH ?? path.join(configDir(), "muesli", "credentials.json")
  );
}
...
export class PresenceSession {
  ...
  setCursor(anchor: number | null, head?: number): void {
    ...
    const rel = (n: number) =>
      Y.relativePositionToJSON(Y.createRelativePositionFromTypeIndex(this.ytext, clamp(n)));
  ...
  participants(): Participant[] {
    ...
    for (const [clientId, state] of this.provider.awareness.getStates()) {
      ...
      const user = ((state as Record<string, unknown>).user ?? {}) as Record<string, unknown>;
      out.push({
        clientId,
        ...
        cursor: this.decodeCursor((state as Record<string, unknown>).cursor),
      });
    }
    return out;
  }
