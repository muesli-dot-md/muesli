...
function ytextOffsetToEditorPos(
...
function renderPresence(): void {
  ...
  for (const p of remotes) {
    ...
    const color =
      PALETTE.includes(p.color) || /^#[0-9a-fA-F]{6}$/.test(p.color) ? p.color : PALETTE[0];
...
async function showParticipants(): Promise<void> {
  if (!active) return;
  const a = active;
  const items: (vscode.QuickPickItem & { open?: boolean })[] = a.session
    .participants()
    .map((p: Participant) => ({
  ...
  items.push({
    ...
    description: webOriginFor(a.link.server),
    open: true,
  });
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("muesli.showParticipants", showParticipants),
  );
